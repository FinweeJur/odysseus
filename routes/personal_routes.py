...
def setup_personal_routes(personal_docs_manager, rag_manager, rag_available):
    ...
    def _resolve_allowed_personal_dir(directory: str) -> str:
        ...
        if not directory:
            raise HTTPException(400, "Caminho do diretório é obrigatório")
        ...
        try:
            ...
            in_base = False
        if not in_base:
            raise HTTPException(403, "Diretório deve estar dentro dos documentos pessoais")
        return resolved
    ...
    async def add_directory_to_rag(
        ...
        try:
            ...
            if not os.path.exists(directory):
                raise HTTPException(404, f"Diretório não encontrado: {directory}")

            if not os.path.isdir(directory):
                raise HTTPException(400, f"Caminho não é um diretório: {directory}")
            ...
            if rag:
                ...
                if result["success"]:
                    ...
                    return {
                        "success": True,
                        "message": f"Indexados com sucesso {result['indexed_count']} trechos de {directory}",
                        "indexed_count": result["indexed_count"],
                        "failed_count": result.get("failed_count", 0),
                        "directory": directory
                    }
                else:
                    raise HTTPException(500, result.get("message", "Falha ao indexar diretório"))
            else:
                raise HTTPException(503, "Sistema RAG não está disponível")
                
        except HTTPException:
            raise
        except Exception as e:
            logger.error(f"Error adding directory to RAG: {e}")
            raise HTTPException(500, f"Falha ao adicionar diretório: {str(e)}")
    ...
    async def remove_directory_from_rag(directory: str = Query(...), owner: str = Depends(require_user), _admin: None = Depends(require_admin)):
        ...
        try:
            ...
            return {
                "success": True,
                "message": f"Removido com sucesso {directory} do índice RAG",
                "directory": directory
            }
            ...
            raise HTTPException(500, f"Falha ao remover diretório: {str(e)}")
    ...
    async def upload_files_to_rag(request: Request, files: List[UploadFile] = File(...)):
        ...
        rag = _rag()
        if not rag:
            raise HTTPException(503, "Sistema RAG não está disponível — o serviço de embedding está em execução?")
    ...
    async def delete_file_from_rag(filepath: str = Query(...), owner: str = Depends(require_user), _admin: None = Depends(require_admin)):
        ...
        try:
            ...
            raise HTTPException(500, f"Falha ao excluir arquivo: {str(e)}")
