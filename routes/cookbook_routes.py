...
def setup_cookbook_routes() -> APIRouter:
    ...
    def _diagnose_serve_output(text: str) -> dict | None:
        ...
        patterns = [
            (
                ...
                "Sem memória de GPU disponível para o cache KV após carregar o modelo.",
                [
                    {"label": "tentar novamente com utilização de memória de GPU 0.95", "op": "replace", "flag": "--gpu-memory-utilization", "value": "0.95"},
                    {"label": "tentar novamente com contexto 2048", "op": "replace", "flag": "--max-model-len", "value": "2048"},
                ],
            ),
            (
                ...
                "A GPU ficou sem memória durante a inicialização ou o aquecimento.",
                [
                    {"label": "tentar novamente com contexto 4096", "op": "replace", "flag": "--max-model-len", "value": "4096"},
                    {"label": "tentar novamente com utilização de memória de GPU 0.80", "op": "replace", "flag": "--gpu-memory-utilization", "value": "0.80"},
                    {"label": "tentar novamente com --enforce-eager", "op": "append", "arg": "--enforce-eager"},
                ],
            ),
            (
                r"not divisib|must be divisible|attention heads.*divisible",
                "O tamanho do paralelismo de tensor é incompatível com o modelo.",
                [
                    {"label": "tentar novamente com tamanho de paralelismo de tensor 1", "op": "replace", "flag": "--tensor-parallel-size", "value": "1"},
                    {"label": "tentar novamente com tamanho de paralelismo de tensor 2", "op": "replace", "flag": "--tensor-parallel-size", "value": "2"},
                ],
            ),
            (
                r"KV cache.*too (small|large)|max_model_len.*exceeds|maximum.*context",
                "O tamanho do contexto é muito grande para a memória de GPU disponível.",
                [
                    {"label": "tentar novamente com contexto 8192", "op": "replace", "flag": "--max-model-len", "value": "8192"},
                    {"label": "tentar novamente com contexto 4096", "op": "replace", "flag": "--max-model-len", "value": "4096"},
                ],
            ),
            (
                r"enable-auto-tool-choice requires --tool-call-parser",
                "A escolha automática de ferramenta requer um parser de chamada de ferramenta explícito.",
                [{"label": "tentar novamente com o parser de ferramenta Hermes", "op": "append", "arg": "--tool-call-parser hermes"}],
            ),
            (
                ...
                "O modelo requer código personalizado ou suporte a um modelo mais novo.",
                [{"label": "tentar novamente com --trust-remote-code", "op": "append", "arg": "--trust-remote-code"}],
            ),
            (
                r"Either a revision or a version must be specified|transformers\.integrations\.hub_kernels|kernels/layer",
                "Incompatibilidade entre os pacotes do vLLM/Transformers e kernels.",
                [{"label": "atualizar vLLM, Transformers e kernels neste servidor", "op": "dependency", "package": "vllm transformers kernels"}],
            ),
            (
                r"Address already in use|bind.*address.*in use",
                "A porta já está em uso.",
                [{"label": "tentar novamente na porta 8001", "op": "replace", "flag": "--port", "value": "8001"}],
            ),
            (
                r"No CUDA GPUs are available|no GPU.*found|CUDA_VISIBLE_DEVICES.*invalid",
                "Nenhuma GPU está visível para o processo de serviço.",
                [{"label": "limpar a seleção de GPU do Cookbook ou escolher GPUs disponíveis", "op": "settings", "field": "gpus", "value": ""}],
            ),
            (
                r"Failed to infer device type|NVML Shared Library Not Found|No module named 'amdsmi'|platform is not available",
                "O vLLM não conseguiu encontrar uma GPU suportada (CUDA ou ROCm). "
                "Esta máquina pode ter apenas gráficos integrados ou não suportados.",
                [
                    {"label": "trocar para llama.cpp (CPU/Metal, funciona sem GPU dedicada)", "op": "manual"},
                    {"label": "trocar para Ollama (CPU/Metal, funciona sem GPU dedicada)", "op": "manual"},
                ],
            ),
            (
                r"vllm.*command not found|No module named vllm|ERROR: vLLM is not installed",
                "O vLLM não está instalado ou não está no PATH neste servidor.",
                [{"label": "instalar o vLLM em Cookbook → Dependências", "op": "dependency", "package": "vllm"}],
            ),
            (
                r"sglang.*command not found|No module named sglang|SGLang is not installed",
                "O SGLang não está instalado ou não está no PATH neste servidor.",
                [{"label": "instalar o SGLang em Cookbook → Dependências", "op": "dependency", "package": "sglang[all]"}],
            ),
            (
                ...
                "As dependências do llama.cpp / llama-cpp-python estão faltando.",
                [{"label": "instalar as dependências do llama.cpp ou llama-cpp-python[server]", "op": "dependency", "package": "llama-cpp-python[server]"}],
            ),
            (
                r"No GGUF found on this host|no \.gguf file|No GGUF file found",
                "Nenhum arquivo GGUF encontrado para este modelo neste host. O backend llama.cpp precisa de um arquivo .gguf.",
                [{"label": "baixar uma versão GGUF deste modelo (o repositório geralmente termina em -GGUF, arquivo tipo Q4_K_M.gguf)", "op": "manual"}],
            ),
            (
                r"No module named 'torch'|No module named torch|No module named 'diffusers'|No module named diffusers",
                "O serviço de difusão requer PyTorch e diffusers.",
                [{"label": "instalar diffusers[torch] em Cookbook → Dependências", "op": "dependency", "package": "diffusers[torch]"}],
            ),
            (
                r"403 Forbidden|401 Unauthorized|Access to model.*is restricted|gated repo|not in the authorized list|awaiting a review",
                "O acesso ao modelo está restrito ou não autorizado.",
                [{"label": "definir o token HF e solicitar acesso ao modelo no HuggingFace", "op": "manual"}],
            ),
        ]
        ...
        if re.search(r"Traceback \(most recent call last\)", tail, re.I) and not re.search(
            ...
            return {
                "message": "Traceback do Python detectado durante a inicialização do serviço.",
                "suggestions": [{"label": "inspecionar o traceback e tentar novamente com backend/configurações ajustados", "op": "manual"}],
            }
        return None
    ...
    async def generate_cookbook_ssh_key(request: Request):
        ...
        if not key_path.exists():
            ...
            if proc.returncode != 0:
                ...
                return {"ok": False, "error": detail or "Falha ao gerar chave SSH"}
    ...
    def _missing_binary_message(binary: str, target: str) -> str:
        if binary == "tmux":
            return (
                f"O tmux é necessário para downloads/serviços em segundo plano do Cookbook em {target}. "
                "Instale-o com o gerenciador de pacotes do seu sistema operacional, ou execute a configuração do servidor do Cookbook para esse servidor."
            )
        if binary == "docker":
            return (
                f"O Docker é necessário para este comando de inicialização do Cookbook em {target}, mas o CLI do docker não foi encontrado. "
                "Instale o Docker e certifique-se de que este usuário pode executar `docker`, depois tente novamente."
            )
        return f"{binary} é necessário em {target}, mas não foi encontrado."
    ...
    async def model_cached(request: Request, host: str | None = None, model_dir: str | None = None, ssh_port: str | None = None, platform: str | None = None):
        ...
        if ssh_port is not None and ssh_port != "" and not _SSH_PORT_RE.fullmatch(ssh_port):
            raise HTTPException(400, "ssh_port inválido")
        TMUX_LOG_DIR.mkdir(parents=True, exist_ok=True)
    ...
    async def model_serve(request: Request, req: ServeRequest):
        ...
        if is_pip_install:
            ...
            if not req.repo_id or not re.fullmatch(
                ...
                raise HTTPException(400, "Nome de pacote pip inválido")
        else:
        ...
        if is_windows and remote:
            ...
            if "ollama" in req.cmd:
                ...
                ps_lines.append('  Write-Host "Ollama não encontrado. Instale em https://ollama.com/download/windows"')
    ...
    async def server_setup(request: Request, req: SetupRequest):
        ...
        if not host:
            raise HTTPException(400, "host é obrigatório")
        port = req.ssh_port
        ...
        try:
            ...
            return {"ok": False, "error": "A configuração expirou (120s)", "platform": platform}
        except Exception as e:
    ...
    async def _run_gpu_shell(cmd_text: str, host: str | None, ssh_port: str | None, timeout: int = 8):
        ...
        try:
            ...
            proc.kill()
            return None, "A sondagem de GPU expirou"
        if proc.returncode != 0:
            err = (stderr.decode("utf-8", errors="replace") or "").strip()[:200]
            return None, err or f"Falha na sondagem de GPU ({proc.returncode})"
        return stdout.decode("utf-8", errors="replace"), None
    ...
    async def list_gpus(request: Request, host: str | None = None, ssh_port: str | None = None):
        ...
        try:
            ...
            nvidia_error = "nvidia-smi não encontrado"
            gpu_out = ""
        except Exception as e:
        ...
        return {"ok": False, "error": nvidia_error or "Nenhuma sondagem de memória de GPU disponível", "gpus": []}
    ...
    async def kill_pid(request: Request, req: KillPidRequest):
        ...
        require_admin(request)
        if req.pid < 100:
            raise HTTPException(400, f"Recusando sinalizar o PID {req.pid} (<100, provavelmente processo do sistema)")
        sig = (req.signal or "TERM").upper()
        if sig not in ("TERM", "KILL", "INT"):
            raise HTTPException(400, "signal deve ser TERM, KILL ou INT")
        host = _validate_remote_host(req.host)
        ...
        try:
            if host:
                ...
                if not pid_alive(req.pid):
                    return {"ok": False, "error": f"O PID {req.pid} não está em execução"}
            ...
            if proc.returncode != 0:
                err = (stderr.decode("utf-8", errors="replace") or "").strip()[:200]
                return {"ok": False, "error": err or f"kill retornou {proc.returncode}"}
            return {"ok": True, "pid": req.pid, "signal": sig}
        except asyncio.TimeoutError:
            return {"ok": False, "error": "O comando kill expirou"}
        except Exception as e:
    ...
    def _cookbook_tasks_status_sync():
        ...
        for task in tasks:
            ...
            if download_zero_files:
                diagnosis = {"message": "Nenhum arquivo correspondente foi baixado. O repositório do modelo ou o padrão de nome de arquivo/quantização pode estar incorreto (por exemplo, uma tag ':Q4_K_M' que não existe no repositório). Verifique o repositório e o padrão de inclusão/quantização."}
