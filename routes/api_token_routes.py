...
def _normalize_scopes(scopes: str | list[str] | None = None, profile: str | None = None) -> list[str]:
    ...
    if profile_key:
        if profile_key not in TOKEN_PROFILES:
            raise HTTPException(400, "Perfil de token desconhecido")
    ...
    for scope in requested:
        if scope not in ALLOWED_SCOPES:
            raise HTTPException(400, f"Escopo de token desconhecido: {scope}")
...
def setup_api_token_routes() -> APIRouter:
    ...
    def create_token(
        ...
        if not name:
            raise HTTPException(400, "O nome do token é obrigatório")
