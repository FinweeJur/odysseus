...
def _verify_session_owner(request: Request, session_id: str, session_manager=None):
    ...
    if not user:
        raise HTTPException(403, "Autenticação necessária")
    db = SessionLocal()
    try:
...
def _reject_raw_endpoint_url_for_non_admin(
    ...
    if user and not _current_user_is_admin(request, user):
        raise HTTPException(403, "Escolha um endpoint de modelo registrado")
...
def setup_session_routes(session_manager: SessionManager, config: dict, webhook_manager=None):
    ...
    def create_session(
        ...
        if endpoint_id and endpoint_id.strip():
            ...
            try:
                ...
                if not endpoint_row:
                    raise HTTPException(400, "O endpoint de modelo não existe mais")
        ...
        if not endpoint_url and not skip_val:
            raise HTTPException(400, "endpoint_url é obrigatório (escolha em /api/models)")
        ...
        if skip_val:
            ...
            if not ids:
                raise HTTPException(400, "Não foi possível acessar /v1/models")
            ...
            if model_to_use not in avail:
                ...
                if not found:
                    raise HTTPException(400,
                                        f"Modelo não encontrado no servidor. Disponíveis: {', '.join(avail)}")
                model_to_use = found
    ...
    def rename_session(
        ...
        try:
            ...
            raise HTTPException(404, f"Sessão {sid} não encontrada")
        result = {"id": sid}
        if name is not None:
        ...
        try:
            ...
            try:
                ...
                if db_sess and db_sess.is_important:
                    raise HTTPException(
                        status_code=403,
                        detail={"error": "SESSION_STARRED", "message": "Remova a estrela da sessão antes de excluí-la"}
                    )
            finally:
                db.close()
            ...
            raise HTTPException(
                ...
                detail={
                    ...
                    "message": "Falha ao excluir a sessão"
                }
            )
    ...
    def delete_all_sessions(request: Request):
        ...
        try:
            ...
            raise HTTPException(500, "Falha ao excluir as sessões")
        finally:
            db.close()
    ...
    def archive_session(request: Request, sid: str):
        ...
        try:
            ...
            try:
                ...
                raise HTTPException(500, "Falha ao arquivar a sessão")
            finally:
                db.close()
    ...
    def unarchive_session(request: Request, sid: str):
        ...
        try:
            ...
            raise HTTPException(500, "Falha ao desarquivar a sessão")
        finally:
            db.close()
    ...
    def create_session_openai(
        ...
        if not OPENAI_API_KEY:
            raise HTTPException(400, "Servidor sem OPENAI_API_KEY configurada")
        sid = str(uuid.uuid4())
        user = effective_user(request)
        session = session_manager.create_session(
            session_id=sid,
    ...
    async def mark_session_important(request: Request, session_id: str, important: bool = Form(True)):
        ...
        try:
            ...
            try:
                ...
                raise HTTPException(500, "Falha ao atualizar a importância da sessão")
            finally:
                db.close()
    ...
    async def compact_session(request: Request, session_id: str):
        ...
        if len(history) < 6:
            raise HTTPException(400, "Mensagens insuficientes para compactar")
        ...
        if not older:
            raise HTTPException(400, "Nada antigo o suficiente para compactar")
        ...
        if not url or not model:
            raise HTTPException(400, "Nenhum modelo configurado para compactação")
        ...
        try:
            ...
            raise HTTPException(500, "Falha na compactação")
        ...
        if not session_manager.replace_messages(session_id, new_history):
            raise HTTPException(500, "Falha ao salvar o histórico compactado")
    ...
    def auto_sort_sessions(request: Request, skip_llm: bool = False):
        ...
        if len(session_list) < 2:
            ...
            return {"status": "skipped", "reason": "Nenhuma sessão não arquivada para organizar"}
        ...
        if not url:
            raise HTTPException(503, "Nenhum endpoint de modelo disponível para a triagem automática")
        ...
        try:
            ...
            if result is None:
                ...
                raise HTTPException(502, "A IA retornou um JSON inválido para a triagem automática — o modelo pode não seguir instruções JSON; tente outro modelo utilitário nas Configurações.")
        except HTTPException:
            raise
        except Exception as e:
            logger.error(f"Auto-sort LLM call failed: {e}")
            raise HTTPException(502, f"Falha na triagem automática: {str(e)}")

        folders = result.get("folders", {})
        if not folders:
            return {"status": "skipped", "reason": "A IA não encontrou agrupamentos"}
        ...
        try:
            ...
            raise HTTPException(500, "Falha ao aplicar as atribuições de pasta")
        finally:
            db.close()
