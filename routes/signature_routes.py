...
def setup_signature_routes() -> APIRouter:
    ...
    async def create_signature(request: Request, req: SignatureCreate) -> Dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(400, "Os dados da assinatura devem ser bytes PNG codificados em base64")

        sig = Signature(
            id=str(uuid.uuid4()),
            owner=user,
            name=(req.name or "Assinatura").strip() or "Assinatura",
            data_png=b64,
            width=req.width,
            height=req.height,
            svg=req.svg,
        ...
        try:
            ...
            raise HTTPException(500, f"Falha ao salvar assinatura: {e}")
        finally:
            db.close()
    ...
    async def delete_signature(sig_id: str, request: Request) -> Dict[str, Any]:
        ...
        try:
            ...
            if user and sig.owner != user:
                raise HTTPException(403, "Esta assinatura não é sua")
            db.delete(sig)
            db.commit()
            ...
            db.rollback()
            raise HTTPException(500, f"Falha ao excluir assinatura: {e}")
        finally:
            db.close()
