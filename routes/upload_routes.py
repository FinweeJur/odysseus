...
def setup_upload_routes(upload_handler):
    ...
    async def api_upload(request: Request, files: List[UploadFile] = File(...)):
        ...
        if recent_uploads >= upload_handler.max_concurrent_uploads:
            raise HTTPException(
                status_code=429,
                detail=f"Número máximo de envios simultâneos ({upload_handler.max_concurrent_uploads}) excedido"
            )
        ...
        if not out:
            raise HTTPException(500, "Falha em todos os envios de arquivo")
    ...
    async def upload_stats(request: Request):
        ...
        try:
            ...
            raise HTTPException(500, "Falha ao obter estatísticas de envio")
    ...
    async def download_file(request: Request, file_id: str, thumb: int = 0):
        ...
        if not upload_handler.validate_upload_id(file_id):
            raise HTTPException(400, "ID de arquivo inválido")
    ...
    async def get_vision_text(request: Request, file_id: str, force: int = 0):
        ...
        if not mime.startswith("image/"):
            raise HTTPException(400, "Não é uma imagem")
        ...
        try:
            ...
            raise HTTPException(500, f"Falha na análise de visão: {e}")
        try:
    ...
    async def put_vision_text(request: Request, file_id: str):
        ...
        if not isinstance(text, str):
            raise HTTPException(400, "o campo text deve ser uma string")
