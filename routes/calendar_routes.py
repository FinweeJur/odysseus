...
def setup_calendar_routes() -> APIRouter:
    ...
    async def test_connection(request: Request):
            ...
            if not pw:
                ...
                if pw:
                    try:
                        ...
                        pass
        if not (url and user and pw):
            return {"ok": False, "error": "URL, usuário ou senha ausentes"}
        from src.caldav_sync import validate_caldav_url
        try:
        ...
        try:
            ...
            if r.status_code == 401:
                return {"ok": False, "error": "Falha na autenticação — verifique o usuário/senha"}
            if r.status_code == 403:
                return {"ok": False, "error": "Acesso negado — o usuário não pode acessar essa URL"}
            if r.status_code == 404:
                return {"ok": False, "error": "Não encontrado — verifique o caminho da URL"}
            if 300 <= r.status_code < 400:
                return {"ok": False, "error": "Redirecionamentos não são seguidos por segurança CalDAV; use a URL final"}
            return {"ok": False, "error": f"HTTP {r.status_code}"}
        except httpx.ConnectError as e:
            return {"ok": False, "error": f"Conexão recusada: {e}"[:200]}
        except httpx.TimeoutException:
            return {"ok": False, "error": "Tempo de conexão esgotado"}
    ...
    async def list_calendars(request: Request):
        ...
        try:
            ...
            raise HTTPException(500, "Falha ao listar os calendários")
        finally:
            db.close()
    ...
    async def list_events(request: Request, start: str, end: str, calendar: str = ""):
        ...
        try:
            ...
            raise HTTPException(500, "Falha ao listar os eventos")
        finally:
            db.close()
    ...
    async def create_event(request: Request, data: EventCreate):
        ...
        try:
            ...
            raise HTTPException(500, "Falha ao criar o evento")
        finally:
            db.close()
    ...
    async def update_event(request: Request, uid: str, data: EventUpdate):
        ...
        try:
            ...
            raise HTTPException(500, "Falha ao atualizar o evento")
        finally:
            db.close()
    ...
    async def delete_event(request: Request, uid: str):
        ...
        try:
            ...
            raise HTTPException(500, "Falha ao excluir o evento")
        finally:
            db.close()
    ...
    async def create_calendar(request: Request, name: str = "Imported", color: str = "#5b8abf"):
        ...
        try:
            ...
            raise HTTPException(500, "Falha ao criar o calendário")
        finally:
            db.close()
    ...
    async def update_calendar(request: Request, cal_id: str, name: str = None, color: str = None):
        ...
        try:
            ...
            raise HTTPException(500, "Falha ao atualizar o calendário")
        finally:
            db.close()
    ...
    async def import_ics(request: Request, file: UploadFile = File(...), calendar_name: str = ""):
        ...
        try:
            ...
            try:
                ...
                raise HTTPException(400, f"Arquivo ICS inválido: {e}")
            ...
            raise HTTPException(500, "Falha ao importar o ICS")
        finally:
            db.close()
    ...
    async def export_ics(request: Request, cal_id: str):
        ...
        try:
            ...
            raise HTTPException(500, "Falha ao exportar o ICS")
        finally:
            db.close()
    ...
    async def quick_parse(request: Request):
        ...
        if not url or not model:
            return {"ok": False, "error": "Nenhum endpoint de LLM configurado"}
        ...
        try:
            ...
            return {"ok": False, "error": f"Falha na chamada do LLM: {e}"}

        cleaned = strip_think(raw or "", prose=False, prompt_echo=True)
        cleaned = _re.sub(r"^```(?:json)?\s*|\s*```$", "", cleaned, flags=_re.MULTILINE).strip()
        m = _re.search(r"\{[\s\S]*\}", cleaned)
        if not m:
            return {"ok": False, "error": "Não foi possível extrair o JSON", "raw": cleaned[:400]}
        try:
            parsed = _json.loads(m.group())
        except Exception as e:
            return {"ok": False, "error": f"JSON inválido: {e}", "raw": cleaned[:400]}
        ...
        if not dtstart:
            return {"ok": False, "error": "O modelo não gerou um horário de início", "raw": cleaned[:400]}
        if not dtend:
