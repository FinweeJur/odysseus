...
def setup_document_routes(session_manager, upload_handler=None) -> APIRouter:
        ...
        try:
            ...
            if req.session_id:
                ...
                if user and session.owner and session.owner != user:
                    raise HTTPException(403, "Não é possível criar documento na sessão de outro usuário")
            ...
            raise HTTPException(500, f"Falha ao criar documento: {e}")
        finally:
            db.close()
    ...
    async def import_pdf(
        ...
        if session_id:
            ...
            try:
                ...
                if user and sess.owner and sess.owner != user:
                    raise HTTPException(403, "Não é possível importar para a sessão de outro usuário")
            finally:
                db.close()

        if upload_handler is None:
            raise HTTPException(500, "Gerenciador de upload não configurado")
        ...
        if not pdf_path:
            raise HTTPException(500, "Não foi possível localizar o PDF salvo")
        ...
        if not doc_id:
            raise HTTPException(500, "Falha ao criar documento para o PDF")

        db = SessionLocal()
        try:
            doc = db.query(Document).filter(Document.id == doc_id).first()
            if not doc:
                raise HTTPException(500, "Documento criado não encontrado")
    ...
    async def documents_library(
        ...
        try:
            ...
            raise HTTPException(500, f"Falha ao buscar biblioteca de documentos: {e}")
        finally:
            db.close()
    ...
    async def list_documents(request: Request, session_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            if not user:
                raise HTTPException(403, "Autenticação obrigatória")
    ...
    async def extract_pdf_text(request: Request, doc_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not upload_id:
                raise HTTPException(400, "Documento não é um PDF — nenhum marcador pdf_source encontrado")

            pdf_path = _locate_current_user_upload(request, upload_id, user)
            if not pdf_path:
                raise HTTPException(404, "Não foi possível localizar o PDF de origem")
            ...
            if not body_text:
                return {"ok": True, "id": doc_id, "extracted": False, "reason": "Nenhum conteúdo legível"}
    ...
    async def documents_export_zip(request: Request):
        ...
        ids = data.get("ids") or []
        if not ids:
            raise HTTPException(400, "Nenhum documento especificado")
        _ext = {
    ...
    async def update_document(request: Request, doc_id: str, req: DocumentUpdate) -> Dict[str, Any]:
        ...
        try:
            ...
            db.rollback()
            raise HTTPException(500, f"Falha ao atualizar documento: {e}")
        finally:
            db.close()
    ...
    async def tidy_documents(request: Request) -> Dict[str, Any]:
        ...
        try:
            ...
            for doc in docs:
                ...
                if not content or content in ("", "# Sem título"):
                ...
                if not title_raw or title_raw == "Sem título":
                    new_title = _derive_title(content)
                    if new_title and new_title != "Sem título":
                        doc.title = new_title
                        fixed_titles += 1
            ...
            return {
                ...
                "deleted": deleted,
                "message": f"{fixed_titles} título{'s' if fixed_titles != 1 else ''} corrigido(s), {deleted} documento{'s' if deleted != 1 else ''} vazio(s) removido(s)",
            }
        except Exception as e:
            db.rollback()
            logger.error(f"Document tidy failed: {e}")
            raise HTTPException(500, f"Falha na limpeza: {e}")
        finally:
            db.close()
    ...
    async def ai_tidy_documents(request: Request) -> Dict[str, Any]:
        ...
        if not url or not model:
            raise HTTPException(500, "Nenhum endpoint configurado para limpeza por IA")
        ...
        try:
            ...
            if not to_review:
                return {"deleted": 0, "reviewed": 0, "message": "Todos os documentos já foram revisados"}
            ...
            if not match:
                raise HTTPException(500, "A IA retornou uma resposta inválida")
            ...
            return {
                ...
                "message": f"{reviewed} revisado(s), {deleted} documento{'s' if deleted != 1 else ''} inútil(eis) removido(s)",
            }
        except HTTPException:
            raise
        except Exception as e:
            ...
            raise HTTPException(500, f"Falha na limpeza por IA: {e}")
        finally:
            db.close()
    ...
    async def export_pdf_preview(doc_id: str, request: Request) -> Dict[str, Any]:
        ...
        try:
            ...
            if not upload_id:
                raise HTTPException(400, "Documento não está vinculado a um PDF de origem")

            pdf_path = _locate_current_user_upload(request, upload_id, user)
            if not pdf_path:
                raise HTTPException(404, f"PDF de origem {upload_id} não encontrado nos uploads")

            fields = load_field_sidecar(pdf_path)
            if not fields:
                raise HTTPException(404, "Esquema de campos não encontrado para o PDF de origem")
    ...
    async def render_pages(doc_id: str, request: Request) -> Dict[str, Any]:
        ...
        try:
            ...
            pdf_path = _locate_current_user_upload(request, upload_id, user)
            if not pdf_path:
                raise HTTPException(404, f"PDF de origem {upload_id} não encontrado")
    ...
    async def render_page_png(doc_id: str, page_no: int, request: Request):
        ...
        try:
            ...
            pdf_path = _locate_current_user_upload(request, upload_id, user)
            if not pdf_path:
                raise HTTPException(404, "PDF de origem não encontrado")
        finally:
            db.close()
    ...
    async def ai_fill_annotations(doc_id: str, request: Request) -> Dict[str, Any]:
        ...
        if not instruction:
            raise HTTPException(400, "instruction é obrigatório")
        ...
        try:
            ...
            raise HTTPException(503, f"Nenhum modelo de visão disponível: {e}")
    ...
    async def render_pdf(doc_id: str, request: Request):
        ...
        try:
            ...
            try:
                ...
                _cleanup_temps()
                raise HTTPException(500, f"Falha ao renderizar PDF: {e}")
    ...
    async def export_pdf(doc_id: str, request: Request):
        ...
        try:
            ...
            try:
                ...
                _cleanup_temps()
                raise HTTPException(500, f"Falha ao preencher o PDF: {e}")
    ...
    async def prepare_signed_reply(doc_id: str, request: Request):
        ...
        try:
            ...
            if not (doc.source_email_uid and doc.source_email_folder):
                raise HTTPException(400, "Documento não possui e-mail de origem — não é possível responder")
