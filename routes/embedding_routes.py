...
def setup_embedding_routes():
    ...
    def list_models():
        ...
        try:
            ...
            raise HTTPException(503, "fastembed não está instalado")
    ...
    async def download_model(model_name: str):
        ...
        if model_name not in catalog:
            raise HTTPException(404, f"Modelo desconhecido: {model_name}")
        ...
        try:
            ...
            raise HTTPException(500, f"Falha no download: {str(e)}")
        finally:
    ...
    def delete_model(model_name: str):
        ...
        if model_name == _active_model():
            raise HTTPException(400, "Não é possível excluir o modelo de embedding ativo")

        if model_name in _downloading:
            raise HTTPException(400, "O modelo está sendo baixado atualmente")
        ...
        hf_src = catalog[model_name].get("sources", {}).get("hf", "")
        if not hf_src:
            raise HTTPException(400, "Nenhuma fonte de cache para este modelo")

        model_path = os.path.join(_cache_dir(), _model_cache_name(hf_src))
        if not os.path.isdir(model_path):
            return {"deleted": False, "message": "Modelo não está em cache"}
    ...
    def set_endpoint(url: str = Form(...), model: str = Form("")):
        ...
        try:
            ...
            raise HTTPException(400, f"Endpoint inacessível: {e}")
