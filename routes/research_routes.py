...
def setup_research_routes(research_handler, session_manager=None) -> APIRouter:
    ...
    def _validate_session_id(session_id: str) -> None:
        if not _SESSION_ID_RE.fullmatch(session_id):
            raise HTTPException(400, "Formato de ID de sessão inválido")
    ...
    async def research_status(session_id: str, request: Request):
        ...
        if not _owns_in_memory(session_id, user):
            raise HTTPException(404, "Nenhuma pesquisa encontrada para esta sessão")
    ...
    async def research_result(session_id: str, request: Request):
        ...
        _validate_session_id(session_id)
        if not _owns_in_memory(session_id, user):
            raise HTTPException(404, "Nenhum resultado de pesquisa disponível")
    ...
    async def research_report(session_id: str, request: Request):
        ...
        try:
            ...
            raise HTTPException(500, f"Falha ao gerar o relatório: {e}")
        if html_content is None:
            logger.warning(f"No report data found for session {session_id}")
            raise HTTPException(404, "Nenhum relatório visual disponível para esta sessão")
    ...
    async def research_detail(session_id: str, request: Request):
        ...
        try:
            ...
            raise HTTPException(500, f"Falha ao ler a pesquisa: {e}")
    ...
    async def research_archive(session_id: str, request: Request, archived: bool = Query(True)):
        ...
        try:
            ...
            raise HTTPException(500, f"Falha ao atualizar a pesquisa: {e}")
    ...
    async def research_start(body: ResearchStartRequest, request: Request):
                ...
                if auth_mgr is not None and getattr(auth_mgr, "is_configured", False):
                    try:
                        ...
                        if not privs.get("can_use_research", True):
                            raise HTTPException(403, f"Sua conta não tem permissão para usar pesquisa.")
                    except HTTPException:
                        raise
                    except Exception:
                        pass
        ...
        if body.endpoint_id:
            ...
            try:
                ...
                if not ep:
                    raise HTTPException(404, "Endpoint não encontrado ou desabilitado")
            ...
            if not ep_url:
                ...
                try:
                    ...
                    db.close()
            if not ep_url:
                raise HTTPException(400, "Nenhum endpoint configurado. Adicione um nas Configurações primeiro.")
            if body.model:
                ep_model = body.model
    ...
    async def research_spinoff(session_id: str, request: Request):
        ...
        if session_manager is None:
            raise HTTPException(500, "session_manager não configurado")
        ...
        if not result:
            raise HTTPException(404, "Nenhum resultado de pesquisa disponível para esta sessão")
