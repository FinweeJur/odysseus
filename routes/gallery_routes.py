...
def setup_gallery_routes() -> APIRouter:
    ...
    async def gallery_upload(request: Request):
        ...
        if not file or not hasattr(file, 'filename'):
            raise HTTPException(400, "Nenhum arquivo fornecido")
        ...
        try:
            ...
            if existing:
                return {"ok": False, "duplicate": True, "filename": existing.filename,
                        "id": existing.id, "message": "Foto duplicada ignorada"}
            ...
            if ext not in VIDEO_EXTS and ext not in IMAGE_EXTS:
                raise HTTPException(400, f"Tipo de arquivo não suportado: .{ext}")
    ...
    async def gallery_replace(request: Request, image_id: str):
        ...
        try:
            ...
            if not user or img.owner != user:
                raise HTTPException(403, "Esta imagem não é sua")

            form = await request.form()
            file = form.get("image")
            if not file or not hasattr(file, 'read'):
                raise HTTPException(400, "Nenhuma imagem fornecida")
            ...
            try:
                ...
                db.rollback()
                raise HTTPException(500, f"Falha ao gravar no banco de dados: {e}")
    ...
    async def gallery_rotate(request: Request, image_id: str):
        ...
        try:
            ...
            raise HTTPException(400, "Ângulo inválido")
        if angle not in (90, -90, 180, 270):
            raise HTTPException(400, "O ângulo deve ser 90, -90, 180 ou 270")
        ...
        try:
            ...
            if not img_path.exists():
                raise HTTPException(404, "Arquivo de imagem não encontrado")
    ...
    async def gallery_ai_upscale(request: Request):
        ...
        form = await request.form()
        file = form.get("image")
        if not file: raise HTTPException(400, "Nenhuma imagem")
        ...
        if not ep:
            raise HTTPException(400, "Nenhum endpoint de geração de imagem configurado. Adicione um em Configurações → Adicionar Modelos.")
        ...
        try:
            async with httpx.AsyncClient(timeout=120) as client:
                ...
                return {"error": f"Endpoint de upscale não disponível ({resp.status_code})"}
        except Exception as e:
            return {"error": str(e)}
    ...
    async def gallery_style_transfer(request: Request):
        ...
        if not ep:
            raise HTTPException(400, "Nenhum endpoint de geração de imagem configurado.")
        ...
        try:
            async with httpx.AsyncClient(timeout=180) as client:
                ...
                return {"error": f"Falha na transferência de estilo ({resp.status_code})"}
        except Exception as e:
            return {"error": str(e)}
    ...
    async def gallery_library(
        ...
        try:
            ...
            raise HTTPException(500, f"Falha ao buscar biblioteca da galeria: {e}")
        finally:
            db.close()
    ...
    async def gallery_download_zip(request: Request):
        ...
        ids = data.get("ids") or []
        if not ids:
            raise HTTPException(400, "Nenhuma imagem especificada")
        db = SessionLocal()
        try:
            ...
            if not used:
                raise HTTPException(404, "Nenhum arquivo de imagem encontrado no disco")
    ...
    async def inpaint_proxy(request: Request):
        ...
        if base:
            ...
            if not ok:
                raise HTTPException(400, f"URL do endpoint rejeitada: {reason}")
        ...
        if not base:
            ...
            try:
                ...
                if not eps:
                    raise HTTPException(400, "Nenhum endpoint de geração de imagem configurado. Sirva um modelo de difusão via Cookbook primeiro.")
        ...
        if is_openai:
            ...
            if not api_key:
                raise HTTPException(400, "O endpoint OpenAI não possui api_key armazenada — edite-o nas configurações de Endpoints.")
            import base64, io
            try:
                from PIL import Image
            except ImportError:
                raise HTTPException(500, "Pillow não está instalado no servidor")
            ...
            try:
                ...
                raise HTTPException(400, f"Falha ao preparar requisição OpenAI: {e}")
            ...
            if "dall-e-3" in oa_model:
                raise HTTPException(400, "dall-e-3 não suporta edição de imagens — escolha gpt-image-1 ou dall-e-2")
            data = {
                "model": oa_model,
            ...
            try:
                async with httpx.AsyncClient(timeout=120) as client:
                    ...
                    if r.status_code != 200:
                        raise HTTPException(r.status_code, f"Falha na edição via OpenAI: {r.text[:300]}")
                    result = r.json()
                    raw_b64 = None
                    if result.get("data"):
                        item = result["data"][0]
                    ...
                    if not raw_b64:
                        raise HTTPException(502, "A OpenAI não retornou nenhuma imagem")
                ...
                raise HTTPException(504, "O inpaint OpenAI expirou (120s)")
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=120) as client:
                ...
                if r.status_code != 200:
                    raise HTTPException(r.status_code, f"Falha no inpaint: {r.text[:200]}")
                return r.json()
        except httpx.TimeoutException:
            raise HTTPException(504, "A requisição de inpaint expirou (120s)")
        except HTTPException:
            raise
        except Exception as e:
            raise HTTPException(502, f"Erro no inpaint: {str(e)}")
    ...
    async def harmonize_image(request: Request):
        ...
        if "api.openai.com" in base:
            raise HTTPException(400,
                "Harmonizar requer um servidor de difusão que suporte img2img "
                "(SD WebUI / Forge / Comfy). A API da OpenAI não expõe "
                "isso. Cookbook → Modelos pode servir um modelo compatível com SD "
                "localmente em poucos cliques.")
        ...
        async with httpx.AsyncClient(timeout=240) as client:
            for path, kind, payload in candidates:
                ...
                try:
                    ...
                    if isinstance(data, dict):
                        ...
                        if data.get("error") and not data.get("image"):
                            raise HTTPException(502,
                                f"Erro do servidor de difusão em {path}: {data['error']}")
                        if data.get("image"):
                    ...
                    raise HTTPException(502, f"Não foi possível acessar o servidor de difusão em {base}: {e}")
                except httpx.TimeoutException:
                    raise HTTPException(504, "A harmonização expirou (240s) — reinicie o servidor de difusão ou reduza Color match / desative Seam fix")
        raise HTTPException(502,
            f"Nenhuma das rotas img2img funcionou em {base}. "
            f"Última resposta: {last_err or 'desconhecida'}. "
            "Seu servidor de difusão precisa expor uma das rotas /v1/images/harmonize, "
            "/v1/images/img2img, /v1/images/variations ou /sdapi/v1/img2img.")
    ...
    async def denoise_image(request: Request):
        ...
        try:
            ...
            raise HTTPException(500, f"Dependência ausente no servidor: {e}")
        ...
        try:
            ...
            return {"error": "realesrgan não está instalado. Instale-o no Cookbook → Dependências (busque 'realesrgan')."}
        try:
            ...
            return {"error": f"Falha na redução de ruído: {e}"}
    ...
    async def upscale_image_local(request: Request):
        ...
        try:
            ...
            return {"error": f"Falha no upscale: {e}"}
    ...
    async def remove_background(request: Request):
        ...
        try:
            ...
            try:
                ...
                return {"error": "Nenhum modelo de remoção de fundo disponível. Instale o rembg: pip install rembg"}
    ...
    async def enhance_face(request: Request):
        ...
        try:
            ...
            raise HTTPException(500, f"Falha na melhoria facial: {str(e)}")
    ...
    async def ai_tag_image(request: Request, image_id: str):
        ...
        try:
            ...
            if not vl_settings.get("vision_enabled", True):
                return {"error": "A visão está desativada — ative-a em Configurações → Visão"}
            configured = vl_settings.get("vision_model", "")
            try:
                chat_url, model_name, headers = _resolve_vl_model(configured)
            except ValueError:
                return {"error": "Nenhum modelo de visão configurado — defina um em Configurações → Visão"}
            if not chat_url:
                return {"error": "Nenhum endpoint com suporte a visão configurado"}
            ...
            async with httpx.AsyncClient(timeout=60) as client:
                ...
                if resp.status_code != 200:
                    ...
                    return {"error": f"O modelo de visão retornou {resp.status_code}: {body[:200]}"}
                data = resp.json()
