...
def setup_chat_routes(
    ...
    async def chat_endpoint(request: Request, chat_request: ChatRequest) -> Dict[str, str]:
        ...
        try:
            ...
            raise HTTPException(404, f"Sessão '{session}' não encontrada")
        owner = get_current_user(request)
        if _clear_orphaned_session_endpoint(sess, owner=owner):
            raise HTTPException(400, "O endpoint de modelo selecionado foi removido. Escolha outro modelo nas Configurações.")
        ...
        if not getattr(sess, "model", "").strip():
            raise HTTPException(
                400,
                "Nenhum modelo selecionado para este chat. Abra o seletor de modelo e escolha um antes de enviar.",
            )
    ...
    async def chat_stream(request: Request) -> StreamingResponse:
        ...
        try:
            ...
            raise HTTPException(400, f"Erro ao processar a requisição: {e}")
        ...
        try:
            ...
            raise HTTPException(400, "Parâmetros de requisição inválidos")
        ...
        async def stream_with_save() -> AsyncGenerator[str, None]:
            ...
            if _is_image_generation_session(sess, owner=_user):
                ...
                if not get_setting("image_gen_enabled", True):
                    yield f'data: {json.dumps({"delta": "A geração de imagens está desabilitada pelo administrador."})}\n\n'
                    yield "data: [DONE]\n\n"
                    _active_streams.pop(session, None)
                    return
    ...
    async def chat_resume(request: Request, session_id: str) -> StreamingResponse:
        ...
        if not agent_runs.is_active(session_id):
            raise HTTPException(404, "Nenhuma execução ativa para esta sessão")
    ...
    async def chat_stream_status(request: Request, session_id: str) -> Dict[str, Any]:
        ...
        if rec is None:
            ...
            raise HTTPException(404, "Nenhum stream ativo para esta sessão")
        return rec
    ...
    async def rewrite_message(request: Request) -> StreamingResponse:
        ...
        if not session_id or not original_text or not instruction:
            raise HTTPException(400, "session_id, original_text e instruction são obrigatórios")
