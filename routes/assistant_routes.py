...
def setup_assistant_routes(task_scheduler) -> APIRouter:
    ...
    async def _get_or_create(owner: str) -> CrewMember:
        ...
        if not owner or owner in _SYNTHETIC_OWNERS:
            raise HTTPException(status_code=400, detail=f"Não é possível criar assistente para {owner!r}")
        db = SessionLocal()
        try:
    ...
    async def get_assistant_session(request: Request):
        ...
        if not crew or not crew.session_id:
            raise HTTPException(status_code=500, detail="Não foi possível resolver a sessão do assistente")
        return {
    ...
    async def get_assistant_settings(request: Request):
        ...
        crew = await _get_or_create(owner)
        if not crew:
            raise HTTPException(status_code=500, detail="Assistente não disponível")
        db = SessionLocal()
        try:
    ...
    async def run_check_in_now(task_id: str, request: Request):
        ...
        try:
            ...
            if not crew:
                raise HTTPException(status_code=400, detail="Não é uma tarefa do assistente")
        finally:
            db.close()
