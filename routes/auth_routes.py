...
def setup_auth_routes(auth_manager: AuthManager) -> APIRouter:
    ...
    async def first_run_setup(body: SetupRequest, request: Request):
        ...
        if auth_manager.is_configured:
            raise HTTPException(400, "Já configurado")
        ...
        if not ok:
            raise HTTPException(500, "Falha na configuração")
    ...
    async def signup(body: SignupRequest, request: Request):
        ...
        if not auth_manager.is_configured:
            raise HTTPException(400, "Execute a configuração inicial primeiro")
        if not auth_manager.signup_enabled:
            raise HTTPException(403, "O cadastro está desabilitado. Peça uma conta a um administrador.")
        if len(body.password) < 8:
            raise HTTPException(400, "A senha deve ter pelo menos 8 caracteres")
        if len(body.username.strip()) < 1:
            raise HTTPException(400, "Nome de usuário é obrigatório")
    ...
    async def login(body: LoginRequest, request: Request, response: Response):
        ...
        if auth_manager.totp_enabled(username):
            ...
            if not auth_manager.totp_verify(username, body.totp_code):
                raise HTTPException(401, "Código de 2FA inválido")
    ...
    async def totp_setup(request: Request):
        ...
        if auth_manager.totp_enabled(user):
            raise HTTPException(400, "O 2FA já está habilitado")
        secret = auth_manager.totp_generate_secret(user)
        if not secret:
            raise HTTPException(500, "Falha ao gerar o segredo")
    ...
    async def update_user_privileges(username: str, request: Request):
        ...
        if not ok:
            raise HTTPException(404, "Usuário não encontrado ou é administrador")
    ...
    async def rename_user(username: str, body: RenameUserRequest, request: Request):
        ...
        try:
            ...
            raise HTTPException(500, "Falha ao renomear os dados do usuário")
        ...
        if not ok:
            raise HTTPException(400, "Não é possível renomear o usuário")
        return {"ok": True, "username": new_username, "renamed_self": old_username == user}
    ...
    async def admin_delete_user(body: DeleteUserRequest, request: Request):
        ...
        if not ok:
            raise HTTPException(400, "Não é possível excluir o usuário")
    ...
    async def test_integration_route(integration_id: str, request: Request):
            ...
            try:
                ...
                if r.is_success:
                    ...
                    return {
                        "ok": True,
                        "message": (
                            f"Enviado para {full_url} — no seu app ntfy, "
                            f"inscreva-se no tópico \"{topic}\" com o servidor "
                            f"\"{base}\" (ou cole a URL completa: {full_url})."
                        ),
                    }
                return {"ok": False, "message": f"ntfy retornou HTTP {r.status_code} de {full_url}: {r.text[:200]}"}
            except Exception as e:
                hint = ""
                if parsed.hostname not in ("127.0.0.1", "localhost"):
                    hint = " Se este for o ntfy do Docker Compose, defina NTFY_BIND para esse host/IP do Tailscale e NTFY_BASE_URL para a mesma URL do servidor no .env, depois recrie o ntfy."
                return {"ok": False, "message": f"Falha ao publicar no ntfy em {full_url}: {e}.{hint}"[:500]}
        ...
        return {"ok": False, "message": (result.get("error") or "Falha na conexão")[:300]}
