    ...
    async def transcribe_audio(file: UploadFile = File(...)):
        ...
        try:
            if not stt_service.available:
                raise HTTPException(
                    status_code=503,
                    detail={"message": "Serviço STT não disponível ou configurado para modo navegador"}
                )

            audio_bytes = await read_upload_limited(file, STT_MAX_AUDIO_BYTES, "Audio file")
            if not audio_bytes:
                raise HTTPException(status_code=400, detail={"message": "Arquivo de áudio vazio"})

            text = stt_service.transcribe(audio_bytes)
            if text is None:
                raise HTTPException(
                    status_code=500,
                    detail={"message": "Falha na transcrição"}
                )
            ...
            raise HTTPException(
                status_code=500,
                detail={"message": f"Falha na transcrição: {str(e)}"}
            )
