...
def setup_task_routes(task_scheduler) -> APIRouter:
    ...
    async def _generate_task_name(prompt: str) -> str:
        ...
        try:
            ...
            first = prompt.split('\n')[0].split('.')[0].strip()
            return first[:50] if first else "Tarefa Sem Título"
    ...
    async def create_task(request: Request, req: TaskCreate):
        ...
        if req.task_type in ("llm", "research") and not req.prompt:
            raise HTTPException(400, "O prompt é obrigatório para tarefas de LLM/pesquisa")
        if req.task_type == "action" and not req.action:
            raise HTTPException(400, "O nome da ação é obrigatório para tarefas de ação")
        ...
        if req.task_type == "action" and req.action in _ADMIN_ONLY_ACTIONS and not _is_admin(user):
            raise HTTPException(403, f"A ação '{req.action}' requer privilégios de administrador")
        if req.trigger_type == "schedule" and not req.schedule:
            raise HTTPException(400, "O agendamento é obrigatório para tarefas disparadas por agendamento")
        if req.trigger_type == "schedule" and req.schedule == "cron" and not req.cron_expression:
            raise HTTPException(400, "A expressão cron é obrigatória para agendamento cron")
        if req.trigger_type == "schedule" and req.schedule == "cron" and req.cron_expression:
            try:
                from croniter import croniter
                croniter(req.cron_expression)
            except Exception:
                raise HTTPException(400, "Expressão cron inválida")
        if req.trigger_type == "event" and not req.trigger_event:
            raise HTTPException(400, "O nome do evento é obrigatório para tarefas disparadas por evento")
        if req.trigger_type == "event" and not req.trigger_count:
            raise HTTPException(400, "A contagem de disparos é obrigatória para tarefas disparadas por evento")
        ...
        if not name:
            if req.task_type == "action":
                ...
                name = BUILTIN_ACTION_INFO.get(req.action, req.action or "Tarefa de Ação")
            elif req.prompt:
                name = await _generate_task_name(req.prompt)
            else:
                name = "Tarefa Sem Título"
        ...
        if req.trigger_type == "schedule":
            if req.schedule == "once" and req.scheduled_date:
                try:
                    ...
                    raise HTTPException(400, "Formato de scheduled_date inválido")
    ...
    async def clear_task_cache(request: Request, task_id: str):
        ...
        if not tables:
            raise HTTPException(400, "Esta tarefa não possui cache que possa ser limpo")
    ...
    async def revert_task(request: Request, task_id: str):
        ...
        try:
            ...
            if not defs:
                raise HTTPException(400, "Não é uma tarefa integrada")
            task.name = defs["name"]
    ...
    async def run_task_now(request: Request, task_id: str, force: bool = False):
        ...
        if not started:
            raise HTTPException(409, "A tarefa já está em execução")
        return {"ok": True, "message": "Tarefa disparada" + (" em paralelo" if force else "")}
    ...
    async def stop_task_now(request: Request, task_id: str):
        ...
        if not stopped:
            raise HTTPException(404, "A tarefa não está em execução")
        return {"ok": True, "message": "Tarefa interrompida"}
    ...
    async def list_output_targets(request: Request):
        ...
        _owner(request)
        targets = [
            {"value": "session", "label": "Sessão", "description": "Salvar resultado em uma sessão de chat"},
            {"value": "notification", "label": "Notificação", "description": "Enviar uma notificação no navegador com o resultado (também salvo na sessão para histórico)"},
            {"value": "email", "label": "Enviar e-mail para mim", "description": "Enviar resultado pela sua conta SMTP configurada"},
        ]
    ...
    async def list_events(request: Request):
        ...
        _owner(request)
        return {"events": [
            {"name": "session_created", "description": "Disparado quando uma nova sessão de chat é criada"},
            {"name": "message_sent", "description": "Disparado quando um usuário envia uma mensagem"},
            {"name": "document_created", "description": "Disparado quando um documento é criado"},
            {"name": "memory_added", "description": "Disparado quando uma memória é adicionada"},
            {"name": "research_completed", "description": "Disparado quando um relatório de pesquisa é concluído"},
            {"name": "email_received", "description": "Disparado quando um novo e-mail é observado na caixa de entrada"},
            {"name": "skill_added", "description": "Disparado quando uma nova skill é criada"},
        ]}
    ...
    async def webhook_trigger(task_id: str, token: str):
        ...
        if not started:
            raise HTTPException(409, "A tarefa já está em execução")
        return {"ok": True, "message": "Tarefa disparada via webhook"}
    ...
    async def parse_task(request: Request) -> Dict[str, Any]:
        ...
        if not desc:
            return {"success": False, "message": "Nada para analisar"}
        ...
        try:
            ...
            if not (url and model):
                return {"success": False, "message": "Nenhum endpoint de modelo configurado"}
            ...
            if not out.get("prompt"):
                return {"success": False, "message": "Não foi possível extrair uma instrução de tarefa"}
