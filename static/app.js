// ============================================
// Cutia UI — Orquestrador Principal da Aplicação
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportCopyBtn) {
    exportCopyBtn.addEventListener('click', async (e) => {
      ...
      if (!transcript.trim()) { uiModule.showToast('Nada para copiar ainda'); return; }
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'Chat do Cutia';
  ...
  if (exportDocBtn) {
    exportDocBtn.addEventListener('click', async (e) => {
      ...
      try {
        ...
        uiModule.showToast('Salvo em documentos');
      } catch (err) {
  ...
  function _syncGroupIndicator(active) {
    ...
    if (active) {
      ...
      if (welcomeSub) {
        ...
        welcomeSub.textContent = 'Pesquisa profunda multietapas com coleta de fontes e síntese.';
      }
      if (tipEl) {
    ...
    sortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        ...
        if (current === mode) {
          ...
          uiModule.showToast('Ordem manual');
        } else {
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        ...
        uiModule.showToast('Modelos ordenados: ' + opt.textContent.trim().toLowerCase());
      });
    });
  }
  ...
  if (saveAiName) {
    saveAiName.addEventListener('click', async () => {
      ...
      try {
        ...
        if (result.success) {
          uiModule.showToast(`IA renomeada para ${newName}`);
          renameAiModal.classList.add('hidden');
          aiNameInput.value = '';
        }
      } catch (e) {
  ...
  if (saveSessionName) {
    saveSessionName.addEventListener('click', async () => {
      ...
      try {
        ...
        const result = await response.json();
        if (response.ok) {
          uiModule.showToast(`Sessão renomeada para ${newName}`);
          renameSessionModal.classList.add('hidden');
          ...
          if (meta) {
            ...
            el('current-meta').textContent = `Sessão: ${meta.name}${meta.model ? ' ' + meta.model.split('/').pop() : ''}${meta.rag ? ' [RAG]' : ''}${ver}`;
          }
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_HIDE_WIDTH ? '' : 'Mensagem para o Cutia...');
      }
  ...
  if (overflowGroupBtn) {
    overflowGroupBtn.addEventListener('click', async () => {
      ...
      if (turningOn) {
        ...
        uiModule.showToast(`Chat em grupo pronto — ${picked.length} modelos`);
      } else {
    ...
    incognitoBtn.addEventListener('click', () => {
      ...
      if (chk.checked) {
        ...
        if (welcomeSub) {
          ...
          welcomeSub.textContent = "Quem sou eu? Eu sou ninguém.";
          welcomeSub.style.display = '';
        }
        if (tipEl) { tipEl.dataset.originalTip = tipEl.textContent; tipEl.textContent = 'Sess\u00e3o tempor\u00e1ria \u2014 n\u00e3o ser\u00e1 salva e sem ativa\u00e7\u00e3o de mem\u00f3ria.'; tipEl.style.opacity = '0.5'; tipEl.style.marginTop = '8px'; }
  ...
  document.addEventListener('keydown', (e) => {
    ...
    applyUIVis(state);
    syncRearrangeChecks();
    uiModule.showToast('Reorganização desativada');
  }, true);
  ...
  // Characters/Personas button in overflow menu
  const overflowCharactersBtn = el('overflow-characters-btn');
  if (overflowCharactersBtn) {
    overflowCharactersBtn.addEventListener('click', () => {
    ...
    railDelete.addEventListener('click', async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          uiModule.showToast('Sessão excluída');
        } else {
...
function startOdysseusApp() {
  ...
  chatContainer.addEventListener('drop', (e) => {
    ...
    fileHandlerModule.renderAttachStrip();
    uiModule.showToast(`${files.length} arquivo${files.length > 1 ? 's' : ''} adicionado${files.length > 1 ? 's' : ''} ao chat`);
  });
  ...
  const _showCmpShield = () => {
    if (!_cmpDropShield) {
      ...
      _box.textContent = 'Solte arquivos para anexar';
  ...
  window.addEventListener('drop', (e) => {
    ...
    fileHandlerModule.addFiles(files);
    fileHandlerModule.renderAttachStrip();
    uiModule.showToast(`${files.length} arquivo${files.length > 1 ? 's' : ''} adicionado${files.length > 1 ? 's' : ''} para anexar`);
  }, true);
  ...
  modelsModule.refreshModels(true).then(() => {
    ...
    if (!hasModels) {
      ...
      if (tip) tip.textContent = 'Adicione um endpoint de IA em Configurações na barra lateral, ou cole um endpoint/chave de API no chat.';
    }
  }).catch(() => {});
