...
import * as Modals from './modalManager.js';
  ...
  async function _resolveComposeSendAccountId() {
    ...
    if (uiModule) uiModule.showToast('A conta de e-mail selecionada é somente recebimento; usando sua conta SMTP.');
    return null;
  }
  ...
  function renderTabs() {
    ...
    let html = '';
    html += '<button class="doc-tab-arrow doc-tab-arrow-left" id="doc-tab-left" title="Rolar para a esquerda">&#x2039;</button>';
    ...
    for (const [id, doc] of docs) {
      ...
      const menuBtn = `<button class="doc-tab-menu-btn" data-doc-id="${id}" title="Ações do documento"><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="5" r="2.5"/><circle cx="12" cy="12" r="2.5"/><circle cx="12" cy="19" r="2.5"/></svg></button>`;
      const ver = doc.version || doc.version_count || 1;
      const verChip = `<span class="doc-tab-version" data-doc-id="${id}" title="Histórico de versões">v${ver}</span>`;
      ...
      html += `<div class="doc-tab${isActive ? ' active' : ''}" draggable="true" data-doc-id="${id}" title="${title}">
        ...
        <button class="doc-tab-close" data-doc-id="${id}" title="Desvincular do chat (mantido na Biblioteca)">&times;</button>
      </div>`;
    }
    ...
    if (!_anyTab && isOpen && !activeDocId) {
      html += `<div class="doc-tab active doc-tab-ghost" title="Novo documento — comece a digitar"><span class="doc-tab-title">Sem título</span></div>`;
    }
    html += `<button class="doc-tab-new" id="doc-tab-new-btn" title="Novo documento"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></button>`;
    html += '</div>';
    html += '<button class="doc-tab-arrow doc-tab-arrow-right" id="doc-tab-right" title="Rolar para a direita">&#x203A;</button>';
  ...
  function showEmptyState() {
    ...
    if (textarea) textarea.placeholder = 'Comece a digitar ou cole um texto para criar um documento...';
  ...
  async function _downloadFilledPdf() {
    ...
    try {
      ...
      if (uiModule) uiModule.showError('Falha na exportação: ' + e.message);
      else alert('Falha na exportação: ' + e.message);
    }
  }
  ...
  async function _openExportPdfModal() {
    ...
    overlay.innerHTML = `
      <div class="modal-content" style="width:min(780px,94vw);max-height:86vh;">
        <div class="modal-header">
          <h4>Exportar PDF preenchido</h4>
          <button id="pdf-export-close" class="modal-close" title="Fechar">×</button>
        </div>
        <div id="pdf-export-summary" style="font-size:0.78rem;opacity:0.7;margin:0 0 6px;">Carregando valores dos campos…</div>
        <div id="pdf-export-body" class="modal-body" style="font-size:0.85rem;">
          <div style="opacity:0.6;">Buscando mapeamento…</div>
        </div>
        <div class="modal-footer" style="display:flex;justify-content:flex-end;gap:8px;padding-top:8px;border-top:1px solid var(--border);margin-top:6px;align-items:center;">
          ...
          <button id="pdf-export-download" class="confirm-btn confirm-btn-primary" disabled>Baixar PDF</button>
        </div>
      </div>
    `;
    ...
    try {
      ...
      overlay.querySelector('#pdf-export-body').innerHTML =
        `<div style="color:#c00;">Falha ao carregar prévia: ${(e && e.message) || e}</div>`;
    }
  }
  ...
  async function _undoPdfPaneAction() {
    ...
    try {
      ...
      _setPdfSaveStatus('error', e.message || 'Falha ao desfazer');
      return true;
    }
  }
  ...
  async function _renderPdfPane() {
    ...
    pane.innerHTML = '<div style="color:#bbb;font-size:13px;text-align:center;padding:40px;">Carregando PDF…</div>';
    ...
    try {
      ...
      pane.innerHTML = `<div style="color:#fbb;padding:40px;text-align:center;">Falha ao carregar PDF: ${_escHtml(e.message || String(e))}</div>`;
      if (savedPill) pane.appendChild(savedPill);
      return;
    }
  ...
  async function _aiFillAnnotations() {
    ...
    const instruction = window.prompt(
      'O que a IA deve preencher?\n(ex: "Meu nome é Maria Silva, endereço Rua Principal 123, nasc. 1990-01-15")'
    );
    ...
    try {
      ...
      if (!proposed.length) {
        ...
        if (uiModule && uiModule.showToast) uiModule.showToast('A IA não encontrou nada para preencher');
        return;
      }
      ...
      _setPdfSaveStatus('saved');
      if (uiModule && uiModule.showToast) uiModule.showToast(`A IA adicionou ${proposed.length} anotações`);
      _renderPdfPane();
    } catch (e) {
      console.error('AI fill failed:', e);
      _setPdfSaveStatus('error', `Falha ao preencher com IA: ${e.message || e}`);
    } finally {
  ...
  function _setPdfSaveStatus(status, msg) {
    ...
    const palette = {
      ...
      dirty:  { txt: 'Editando…',   bg: 'var(--panel)',          fg: 'var(--fg)' },
      saving: { txt: 'Salvando…',    bg: 'var(--panel)',          fg: 'var(--fg)' },
      saved:  { txt: 'Salvo',      bg: 'rgba(34,197,94,0.85)',  fg: '#fff' },
      error:  { txt: msg || 'Falha ao salvar', bg: 'var(--red)',    fg: 'var(--bg)' },
    };
    ...
    if (status === 'saved') {
      setTimeout(() => {
        if (pill.textContent === 'Salvo') _setPdfSaveStatus('idle');
      }, 1200);
    }
  }
  ...
  async function _savePdfPaneToMarkdown(opts = {}) {
    ...
    try {
      ...
      if (!res.ok) {
        const t = await res.text().catch(() => res.statusText);
        _setPdfSaveStatus('error', `Falha ao salvar: ${res.status}`);
      ...
      _setPdfSaveStatus('error', e.message || 'Falha ao salvar');
      ...
      if (fields.attachments && fields.attachments.length > 0 && fields.sourceUid) {
        ...
        for (const att of fields.attachments) {
          ...
          if (isPdf) {
            ...
            chip.addEventListener('click', () => _withSpinner(chip, async () => {
              try {
                ...
                if (data.doc_id) {
                  ...
                  uiModule.showError(data.error || 'Falha ao abrir o PDF');
                ...
                if (uiModule) uiModule.showError('Falha ao abrir o PDF');
              }
            }));
            ...
            chip.title = `Baixar ${att.filename}`;
            chip.innerHTML = chipHtml;
            chip.addEventListener('click', () => _withSpinner(chip, async () => {
              try {
                const folderQs = encodeURIComponent(fields.sourceFolder || 'INBOX');
                ...
                if (uiModule) uiModule.showError('Falha no download: ' + e.message);
              }
            }));
            attDiv.appendChild(chip);
          }
    ...
    for (const file of list) {
      try {
        ...
        if (data.success) {
          ...
          if (uiModule) uiModule.showError(`Falha ao enviar ${file.name}: ${data.error || ''}`);
        }
      } catch (err) {
        if (uiModule) uiModule.showError(`Falha ao enviar ${file.name}`);
      }
    }
    _renderComposeAttachments();
  }
  ...
  async function _sendEmail() {
    ...
    try {
      ...
      if (canceled) {
        ...
        if (uiModule) uiModule.showToast('Envio cancelado');
        return;
      }
      ...
      try {
        ...
        data = { success: false, error: `Falha no envio (${res.status})` };
      }
      if (!res.ok && data && !data.error) data.error = `Falha no envio (${res.status})`;
      if (data.success) {
        if (uiModule) {
          uiModule.showToast('Mensagem enviada', {
            duration: 7000,
            leadingIcon: 'check',
            action: 'Ver mensagem',
            onAction: () => {
        ...
        _restoreDetachedEmailDoc(detachedEmailDoc);
        detachedEmailDoc = null;
        if (uiModule) uiModule.showError(data.error || 'Falha ao enviar');
      }
    } catch (e) {
      _restoreDetachedEmailDoc(detachedEmailDoc);
      detachedEmailDoc = null;
      if (uiModule) uiModule.showError(e?.message ? `Falha ao enviar e-mail: ${e.message}` : 'Falha ao enviar e-mail');
    } finally {
  ...
  async function _saveDraft() {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        if (uiModule) uiModule.showToast('Rascunho salvo na caixa de e-mail');
      } else {
        if (uiModule) uiModule.showError(data.error || 'Falha ao salvar rascunho');
      }
    } catch (e) {
      const timedOut = e && e.name === 'AbortError';
      if (uiModule) uiModule.showError(timedOut ? 'Tempo esgotado ao salvar rascunho' : 'Falha ao salvar rascunho');
    } finally {
      clearTimeout(timeout);
      if (btn) { btn.disabled = false; btn.textContent = 'Rascunho'; }
    }
  }
  ...
  async function _aiReply() {
    ...
    if (btn) { btn.disabled = true; btn.innerHTML = '<svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor" style="vertical-align:-1px;margin-right:3px"><path d="M12 0L14.59 8.41L23 12L14.59 15.59L12 24L9.41 15.59L1 12L9.41 8.41Z"/></svg>Gerando…'; }
    ...
    try {
      ...
      if (data.success && data.reply) {
        ...
        if (uiModule) uiModule.showToast(`Rascunho da IA inserido (${data.model_used || 'IA'})`);
      } else {
        if (uiModule) uiModule.showError(data.error || 'Falha ao gerar resposta');
      }
    } catch (e) {
      if (uiModule) uiModule.showError('Falha ao gerar resposta da IA');
    } finally {
      if (btn) { btn.disabled = false; btn.innerHTML = '<svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor" style="vertical-align:-1px;margin-right:3px"><path d="M12 0L14.59 8.41L23 12L14.59 15.59L12 24L9.41 15.59L1 12L9.41 8.41Z"/></svg>Resposta IA'; }
    }
  }
  ...
  async function _scheduleSend(anchorEl = null) {
    ...
    if (!to || !body) {
      if (uiModule) uiModule.showError('Destinatário e corpo são obrigatórios');
      return;
    }
    if (inReplyTo && !_emailReplyOwnText(body)) {
      if (uiModule) uiModule.showError('O corpo da resposta está vazio');
      return;
    }
    if (attachments.length === 0 && _bodyMentionsAttachment(body)) {
      const proceed = await _confirmMissingAttachment();
    ...
    overlay.innerHTML = `
      <div class="modal-content schedule-send-modal" style="width:400px;max-width:92vw;">
        <div class="modal-header">
          <h4>Agendar envio</h4>
          <button class="close-btn" id="sched-close" title="Fechar"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
        </div>
        <div class="modal-body schedule-send-body">
          <label class="schedule-send-label">Atalhos rápidos</label>
          <div class="schedule-send-presets">
            <button class="memory-toolbar-btn" data-preset="1h">Em 1 hora</button>
            <button class="memory-toolbar-btn" data-preset="3h">Em 3 horas</button>
            <button class="memory-toolbar-btn" data-preset="tomorrow">Amanhã 9h</button>
            <button class="memory-toolbar-btn" data-preset="monday">Segunda 9h</button>
          </div>
          <label class="schedule-send-label" for="sched-datetime">Ou escolha um horário específico</label>
        ...
        <div class="modal-footer schedule-send-footer">
          ...
          <button class="memory-toolbar-btn schedule-send-confirm" id="sched-confirm"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>Agendar</button>
        </div>
      </div>
    `;
    document.body.appendChild(overlay);
    ...
    overlay.querySelector('#sched-confirm').addEventListener('click', async () => {
      ...
      if (!localDt) { if (uiModule) uiModule.showError('Escolha um horário'); return; }
      ...
      try {
        ...
        const data = await res.json();
        if (data.success) {
          if (uiModule) uiModule.showToast(`Agendado para ${new Date(localDt).toLocaleString()}`);
          cleanup();
          // Close the document
          _closeWithoutDeleting(true);
        } else {
          if (uiModule) uiModule.showError(data.error || 'Falha ao agendar');
        }
      } catch (e) {
        if (uiModule) uiModule.showError('Falha ao agendar');
      }
    });
  }
  ...
  function _detachDocFromSession(docId, { toast = false } = {}) {
    ...
    if (hasContent) {
      fetch(`${API_BASE}/api/document/${docId}`, {
        ...
        if (toast && uiModule) uiModule.showToast('Documento desvinculado da sessão');
      }).catch(() => {});
    } else {
    ...
    if (diffToggleBtn) diffToggleBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (versions.length < 2) {
          if (uiModule) uiModule.showToast('Não há versão anterior para comparar');
          return;
        }
        // versions are sorted desc — [0] is latest, [1] is previous
        const prevContent = versions[1].content || '';
        if (prevContent === current) {
          if (uiModule) uiModule.showToast('Sem alterações em relação à versão anterior');
          return;
        }
        enterDiffMode(prevContent, current);
      } catch {
        if (uiModule) uiModule.showError('Falha ao carregar histórico de versões');
      }
    });
  ...
  export async function replaceEmailReplyBody(docId, replyText) {
    ...
    if (ownText && !/^(\[AI reply draft will appear here\]|Drafting AI reply)/i.test(ownText)) {
      if (uiModule) uiModule.showToast('Resposta da IA pronta, mas o rascunho foi editado');
      return;
    }
    ...
    try {
      ...
      if (uiModule) {
        const msg = e.message === 'Not found'
          ? 'Documento não encontrado — tente abri-lo pela Biblioteca.'
  ...
  function enterDiffMode(oldContent, newContent) {
    ...
    if (_diffChunks.length === 0) {
      ...
      if (uiModule) uiModule.showToast('Sem alterações');
      return;
    }
  ...
  async function copyDocument() {
    ...
    if (uiModule) uiModule.showToast('Copiado para a área de transferência');
  }
  ...
  async function _sendSignedReply(docId) {
    ...
    if (uiModule) uiModule.showToast('Preparando resposta assinada…');
    let result;
    try {
      ...
      if (!res.ok || !result.ok) {
        ...
        if (uiModule) uiModule.showError(`Não foi possível preparar a resposta assinada: ${msg}`);
        return;
      }
    } catch (e) {
      console.error('prepare-signed-reply failed:', e);
      if (uiModule) uiModule.showError("Não foi possível preparar a resposta assinada");
      return;
    }
    ...
    for (const [, d] of docs) {
      if (d.language === 'email' && d._draftForMessageId === mid && mid) {
        ...
        _renderComposeAttachments();
        if (uiModule) uiModule.showToast(`"${att.filename}" adicionado ao rascunho de resposta`);
        return;
      }
    }
    ...
    const content = headerLines.join('\n') + '\n---\n\nOlá' + (reply.to_name ? ' ' + reply.to_name.split(/\s+/)[0] : '') + ',\n\nSegue em anexo a cópia assinada.\n\nAtenciosamente,\n';
    ...
    try {
      ...
      const cRes = await fetch(`${API_BASE}/api/document`, {
        ...
        body: JSON.stringify({
          session_id: sessionId,
          title: reply.subject || 'Resposta assinada',
          language: 'email',
          content,
        }),
      });
      ...
      if (uiModule) uiModule.showError("Não foi possível criar o rascunho de resposta");
      return;
    }
    ...
    _renderComposeAttachments();
    if (uiModule) uiModule.showToast(`Rascunho de resposta pronto — "${att.filename}" anexado`);
  }
  ...
  export async function saveDocument({ silent = false } = {}) {
    ...
    try {
      ...
      _syncDocIndicator();
      if (!silent && uiModule) uiModule.showToast('Documento salvo');
    } catch (e) {
      console.error('Failed to save document:', e);
      if (!silent && uiModule) uiModule.showError('Falha ao salvar documento');
    }
  }
  ...
  function _importFromDevice() {
    ...
    const fi = document.createElement('input');
      ...
      try {
        ...
        if (uiModule && uiModule.showError) uiModule.showError('Falha na importação: ' + (err.message || err));
      } finally {
        fi.value = '';
        fi.remove();
      }
  ...
  function showExportMenu(e, anchorRect) {
    ...
    options.push({ label: 'Importar da biblioteca', fn: () => openLibrary() });
    options.push({ label: 'Importar do dispositivo', fn: () => _importFromDevice(), _divider: true });
    if (isForm) options.push({ label: 'PDF preenchido (.pdf)', fn: _downloadFilledPdf });
    options.push(
      { label: 'Exportar Markdown', fn: exportDocument },
  ...
  async function exportAsPdf() {
    ...
    try {
      ...
      if (uiModule) uiModule.showError('Falha ao carregar biblioteca de PDF');
      return;
    }
    const lang = document.getElementById('doc-language-select')?.value || '';
    const text = textarea.value || '';
    ...
    if (uiModule) uiModule.showToast('Exportando PDF…');
  }
  ...
  async function exportAsDocx() {
    ...
    try {
      ...
      if (uiModule) uiModule.showError('Falha ao carregar biblioteca de DOCX');
      return;
    }
    const text = textarea.value || '';
  ...
  async function deleteActiveDocument() {
    if (!activeDocId) return;
    const doc = docs.get(activeDocId);
    const name = doc ? doc.title : 'este documento';
    const ok = uiModule && uiModule.styledConfirm
      ? await uiModule.styledConfirm(`Excluir "${name}"?`, { confirmText: 'Excluir', danger: true })
    ...
    try {
      ...
      if (uiModule) uiModule.showError('Falha ao excluir documento');
    }
  }
