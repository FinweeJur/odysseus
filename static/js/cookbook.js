...
export function _buildServeCmd(f, modelName, backend) {
  ...
  if (backend === 'vllm') {
    ...
    if (!_isWindows()) {
      ...
      cmd += `MODEL_FILE=${ggufPath} && { [ -n "$MODEL_FILE" ] && [ -f "$MODEL_FILE" ]; } || { echo "ERRO: Nenhum GGUF encontrado neste host. Baixe o modelo aqui, ou mude para o servidor onde ele está em cache."; exit 1; } && `;
    }
...
async function _fetchDependencies() {
  ...
  try {
    ...
    label.textContent = 'Carregando pacotes…';
    label.style.cssText = 'text-align:center;opacity:0.5;font-size:11px;margin-top:6px;';
    list.appendChild(label);
  } catch {
    list.innerHTML = '<div class="hwfit-loading">Carregando pacotes…</div>';
  }
  try {
    ...
    if (!pkgs.length) { list.innerHTML = '<div class="hwfit-loading">Nenhum pacote encontrado</div>'; return; }
    const _winUnsupported = new Set(['diffusers', 'hf_transfer', 'vllm', 'rembg', 'gfpgan']);

    const _statusTag = (pkg, isLocal, isSystemDep, winBlocked) => {
      if (winBlocked) return `<span class="cookbook-dep-tag cookbook-dep-na">N/A</span>`;
      if (pkg.installed && isSystemDep) return `<span class="cookbook-dep-tag cookbook-dep-installed" title="Encontrado no servidor selecionado">Instalado</span>`;
      if (pkg.installed && pkg.pip_update_available === false) {
        const tip = esc(pkg.update_note || pkg.status_note || 'Encontrado externamente; atualize fora do Cutia.');
        return `<span class="cookbook-dep-tag cookbook-dep-installed" title="${tip}">Instalado</span>`;
      }
      if (pkg.installed) return `<button class="cookbook-dep-tag cookbook-dep-installed cookbook-dep-installed-btn" title="Instalado — clique para ações"><span class="cookbook-dep-installed-label">Instalado</span><span class="cookbook-dep-caret">&#9662;</span></button>`;
      if (isSystemDep) {
        const depTip = esc(pkg.install_hint || 'Instale este pacote do sistema no servidor selecionado.');
        const depLabel = pkg.applicable === false ? 'N/D ?' : 'Faltando';
    ...
    const _depRow = (pkg) => {
      ...
      const _rebuildBtn = (pkg.name === 'llama_cpp')
        ? `<button type="button" class="cookbook-dep-tag cookbook-dep-rebuild" id="cookbook-rebuild-engine" title="Limpa o build em cache do llama.cpp para que o próximo serve recompile a partir do código-fonte (use após instalar um toolkit CUDA/ROCm para transformar um build CPU-only em um build GPU).">Reconstruir</button>`
        : '';
    ...
    list.innerHTML = [
      _viewingRemote ? '' : _section('App do Cutia', 'Execute dentro do próprio app do Cutia.', _appDeps),
      _section('Servidor', 'Executa no servidor escolhido acima (Local, ou uma máquina remota via SSH).', _serverDeps),
    ].join('');
    ...
    // `statusEl`, when given, shows "Instalando…/Atualizando…" and is disabled.
    async function _installDep(pipName, pkgName, isLocalOnly, upgrade, statusEl) {
      ...
      const targetHost = isLocalOnly ? 'este servidor' : (_envState.remoteHost || 'local');
      ...
      try {
        ...
        if (!res.ok || !data.ok) {
          ...
          uiModule.showToast('Falha ao instalar: ' + String(reason).slice(0, 200));
          return;
        }
        ...
        if (statusEl) { statusEl.textContent = upgrade ? 'Atualizando…' : 'Instalando…'; statusEl.disabled = true; }
        uiModule.showToast(`${upgrade ? 'Atualizando' : 'Instalando'} ${pkgName} em ${targetHost}...`);
      } catch (err) {
        uiModule.showToast('Falha ao instalar: ' + err.message);
      }
    }
    ...
    function _showDepMenu(anchor) {
      ...
      it.innerHTML = `<span class="dropdown-icon">${upIco}</span><span>Atualizar</span>`;
      it.title = `Atualiza ${pkgName} para a versão mais recente (pip install -U)`;
    ...
    list.innerHTML = `<div class="hwfit-loading">Erro ao carregar pacotes: ${esc(err.message)}</div>`;
  }
}
...
function _wireTabEvents(body) {
    ...
    cacheServer.addEventListener('change', () => {
      ...
      if (dirsEl) {
        ...
        dirsEl.innerHTML = dirs.map(d => `<span class="cookbook-serve-dir-pill">${esc(d)}</span>`).join('') +
          '<span class="cookbook-serve-dir-edit" title="Editar em Configurações">editar</span>';
  ...
  if (rebuildBtn && !rebuildBtn._wired) {
    ...
    rebuildBtn.addEventListener('click', async () => {
      ...
      const where = host || 'este servidor';
      if (!confirm(`Reconstruir o motor llama.cpp em ${where}?\n\nIsso limpa o build em cache do llama-server para que o próximo serve recompile a partir do código-fonte (com CUDA/HIP se houver toolchain presente). Não baixa nem instala nada.`)) return;
      const _label = rebuildBtn.textContent;
      rebuildBtn.disabled = true;
      rebuildBtn.textContent = 'Limpando...';
      try {
        ...
        if (!res.ok || !data.ok) {
          const reason = data.detail || data.error || `HTTP ${res.status}`;
          uiModule.showToast('Falha ao reconstruir: ' + String(reason).slice(0, 200));
        } else {
          uiModule.showToast(`Build do llama.cpp limpo em ${where}. Reinicie a tarefa de serve para reconstruir com suporte a GPU.`);
        }
      } catch (err) {
        uiModule.showToast('Falha ao reconstruir: ' + err.message);
      } finally {
  ...
  if (selectBtn && bulkBar) {
    selectBtn.addEventListener('click', () => {
      ...
      selectBtn.textContent = active ? 'Cancelar' : 'Selecionar';
    ...
    function _updateBulkCount() {
      ...
      if (countEl) countEl.textContent = count + ' selecionado(s)';
    }

    document.getElementById('serve-bulk-cancel')?.addEventListener('click', () => {
      selectBtn.classList.remove('active');
      selectBtn.textContent = 'Selecionar';  // reset label so the button doesn't stay reading "Cancelar" after exit
    ...
    document.getElementById('serve-bulk-delete')?.addEventListener('click', async () => {
      ...
      if (!(await uiModule.styledConfirm(`Excluir ${repos.length} modelo(s)? Isso remove os arquivos em cache.`, { confirmText: 'Excluir', danger: true }))) return;
      for (const repo of repos) {
        const item = document.querySelector(`.memory-item[data-repo="${repo}"]`);
        if (item) await _deleteCachedModel(repo, item, true);
      }
      selectBtn.classList.remove('active');
      selectBtn.textContent = 'Selecionar';  // same reset as bulk-cancel
      bulkBar.classList.add('hidden');
      document.querySelectorAll('.serve-select-cb').forEach(dot => { dot.style.display = 'none'; dot.classList.remove('selected'); });
    });
  }
  ...
  if (dlBtn && dlInput) {
    ...
    const triggerDownload = () => {
      ...
      if (!/^[^\s/]+\/[^\s/]+$/.test(repo)) {
        uiModule.showToast('Informe um repo ID completo do HuggingFace, como "org/model-name" (ou cole a URL completa do HF).');
        dlInput.focus();
        return;
      }
    ...
    async function _loadLatest() {
      ...
      try {
        ...
        if (!models.length) {
          ...
          const msg = lastErr
            ? `Não foi possível carregar os modelos em alta (${esc(lastErr)})`
            : 'Nenhum modelo em alta encontrado';
        ...
        hfList.innerHTML = '<div class="hwfit-loading">Falha ao carregar</div>';
      }
    }
...
export function _serverEntryHtml(s, i, defaultServer, forceRemote, isNew) {
  ...
  const envOpts = ['none', 'venv'].map(e => `<option value="${e}"${s.env === e ? ' selected' : ''}>${e === 'none' ? 'Nenhum' : e}</option>`).join('');
  let html = '';
  html += `<div class="cookbook-server-entry" data-idx="${i}" data-platform="${esc(s.platform || '')}">`;
  const _srvTitle = s.name || (isLocal ? 'Local' : (s.host || `Servidor ${i + 1}`));
  const _srvKey = isLocal ? 'local' : (s.host || '');
  const _isDefaultSrv = (defaultServer || '') === _srvKey;
  const _pIco = _platformIcon(s.platform);
  const _keyBtn = `<button class="cookbook-server-key-btn" title="Configurar chave SSH para este servidor" style="height:22px;box-sizing:border-box;display:inline-flex;align-items:center;position:relative;top:-2px;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:4px;flex-shrink:0;"><circle cx="7.5" cy="15.5" r="5.5"/><path d="M12 11l8-8"/><path d="M17 6l3 3"/></svg>Chave</button>`;
  const _checkBtn = `<button class="cookbook-server-check-btn" title="Verificar conexão SSH" style="height:22px;box-sizing:border-box;display:inline-flex;align-items:center;position:relative;top:-2px;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:4px;flex-shrink:0;"><polyline points="20 6 9 17 4 12"/></svg>Verificar</button>`;
  ...
  if (isNew) {
    ...
    html += `<span style="margin-left:auto;display:inline-flex;gap:4px;align-items:center;">${_checkBtn}${_keyBtn}<button class="cookbook-server-cancel-btn" title="Descartar este novo servidor" style="height:22px;box-sizing:border-box;display:inline-flex;align-items:center;position:relative;top:-2px;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:4px;flex-shrink:0;"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>Cancelar</button></span>`;
  } else {
    html += `<span style="margin-left:auto;display:inline-flex;gap:4px;align-items:center;">${!isLocal ? _checkBtn + _keyBtn : ''}<span class="cookbook-srv-default${_isDefaultSrv ? ' active' : ''}" title="${_isDefaultSrv ? 'Servidor padrão — o Cutia abre aqui' : 'Tornar este o servidor padrão'}" data-srv-key="${esc(_srvKey)}">${_isDefaultSrv ? _MODELDIR_CHECK_ON : _MODELDIR_CHECK_OFF}<span class="cookbook-srv-default-label">padrão</span></span></span>`;
  }
  html += `</span>`;
  html += `<div class="cookbook-server-row">`;
  html += `<input type="text" class="hwfit-sf cookbook-srv-name" value="${esc(s.name || (isLocal ? 'Local' : ''))}" placeholder="Nome (opcional)" style="width:92px;flex-shrink:0;" />`;
  html += `<input type="text" class="hwfit-sf cookbook-srv-host" value="${isLocal ? '' : esc(s.host || '')}" placeholder="ex: user@ip" style="width:214.5px;flex-shrink:0;box-sizing:border-box;" ${isLocal ? 'readonly' : ''} />`;
  html += `<input type="text" class="hwfit-sf cookbook-srv-port" value="${esc(s.port || '')}" placeholder="Porta" title="Porta SSH (padrão 22)" style="width:48px;flex-shrink:0;" ${isLocal ? 'readonly' : ''} />`;
  html += `<select class="hwfit-sf cookbook-srv-env">${envOpts}</select>`;
  html += `<input type="text" class="hwfit-sf cookbook-srv-path" value="${esc(s.envPath || '')}" placeholder="${s.platform === 'windows' ? 'caminho do venv' : '~/venv'}" />`;
  ...
  html += `<span style="width:100%;font-size:13px;font-weight:600;margin-bottom:3px;">Diretório de modelos <span style="font-weight:400;opacity:0.5;font-size:11px;">— marque para onde os downloads devem ir</span></span>`;
  for (let j = 0; j < modelDirs.length; j++) {
    ...
    const dlBtn = `<span class="cookbook-modeldir-dl${isTarget ? ' active' : ''}" title="${isTarget ? 'Os downloads vão para aqui' : 'Enviar downloads para aqui'}" data-dl-dir="${esc(dirVal)}">${isTarget ? _MODELDIR_CHECK_ON : _MODELDIR_CHECK_OFF}</span>`;
    const rmBtn = isDefault ? '' : ' <span class="cookbook-modeldir-rm" title="Remover">✖</span>';
  ...
  html += `<button class="cookbook-modeldir-add" title="Adicionar diretório de modelos">+ Adicionar</button>`;
  ...
  if (isNew) {
    ...
    html += `<button class="cookbook-server-save-btn" title="Salvar este servidor" style="${_btnStyle}"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:4px;flex-shrink:0;"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg>Salvar</button>`;
  } else if (!isLocal) {
    html += `<button class="cookbook-server-rm cookbook-server-rm-btn" title="Excluir este servidor" style="${_btnStyle}"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:4px;flex-shrink:0;"><path d="M3 6h18"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/></svg>Excluir</button>`;
  }
  html += `</div>`;
  if (!isLocal) {
    html += `<div class="cookbook-server-key-panel hidden" style="margin-top:6px;flex-direction:column;gap:5px;">`;
    html += `<div style="display:flex;gap:4px;align-items:center;">`;
    html += `<button type="button" class="memory-toolbar-btn cookbook-server-key-gen" style="height:23px;">Gerar chave</button>`;
    ...
    html += `<span style="font-size:10px;opacity:0.55;line-height:1.25;">Docker: execute este comando no seu terminal uma vez.</span>`;
    html += `</div>`;
    html += `<textarea class="memory-search-input cookbook-server-key-command" readonly rows="3" style="min-height:58px;resize:vertical;font-family:var(--mono,monospace);font-size:10px;line-height:1.35;">Informe user@host, depois gere a chave.</textarea>`;
    html += `</div>`;
  }
  html += `</div>`;
  return html;
...
function _renderRecipes() {
  ...
  html += '<button class="cookbook-tab" data-backend="Dependencies"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" style="vertical-align:-1px;margin-right:3px;"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg>Dependências</button>';
  html += '<button class="cookbook-tab" data-backend="Settings"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" style="vertical-align:-1px;margin-right:3px;"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>Configurações</button>';
  html += '</div>';
  ...
  html += '<p class="memory-desc doclib-desc" style="margin-top:6px;">Baixe do <a href="https://huggingface.co/models" target="_blank" rel="noopener" style="color:var(--accent,var(--red));text-decoration:none;"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:1px;"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>HuggingFace</a> colando o link do modelo, ou baixe diretamente na seção de Scan abaixo.</p>';
  ...
  html += `<button class="memory-toolbar-btn cookbook-dl-add-server" title="Adicionar servidor em Configurações" style="height:28px;">adicionar servidor</button>`;
  html += `</div>`;
  html += `<div class="cookbook-dl-input" style="margin-top:0;">`;
  html += `<input type="text" class="cookbook-dl-repo" id="cookbook-dl-repo" placeholder="org/model-name, URL do HF, ou org/model:QUANT_TAG" />`;
  ...
  html += `<span style="pointer-events:none;">Modelos em alta que cabem no seu hardware</span>`;
  html += `</button>`;
  html += `<button type="button" class="memory-toolbar-btn" id="cookbook-hf-latest-refresh" title="Atualizar" style="height:26px;width:26px;padding:0;border-radius:4px;">\u21BB</button>`;
  html += `</div>`;
  ...
  html += '</div>';
  html += '<p class="memory-desc doclib-desc" style="margin-top:6px;">Escaneia seu hardware para saber quais modelos você consegue rodar. O hardware fica em cache; clique no botão de scan para reescanear após mudar GPUs.</p>';
  ...
  html += '<option value="general" selected>Padrão</option><option value="coding">Código</option>';
  html += '<option value="reasoning">Raciocínio</option><option value="chat">Chat</option>';
  ...
  html += '<option value="multimodal">Visão</option></select>';
  ...
  html += '<select class="cookbook-field-input hwfit-engine" id="hwfit-engine" style="height:28px;" title="Filtrar por engine de serving">';
  ...
  html += '</select>';
  html += '<span class="hwfit-help-chip hwfit-help-chip-inline hwfit-engine-help" title="Regra geral: GGUF em GPU única / CPU+RAM → llama.cpp (ou Ollama). Safetensors em multi-GPU NVIDIA → vLLM. SGLang é uma alternativa de classe vLLM, às vezes mais rápida em big-MoE / contexto longo.">?</span>';
  html += '</span>';
  ...
  html += '<option value="" selected>Quant: Todos</option>';
  ...
  html += '<span class="hwfit-help-chip hwfit-help-chip-inline hwfit-quant-help" title="Níveis de quant mais baixos (Q2/Q3/Q4 / AWQ-4bit) são menores, mais rápidos e mais baratos de rodar, com alguma perda de qualidade. Níveis mais altos (Q8 / FP8 / FP16 / BF16) preservam mais qualidade mas precisam de mais VRAM. “Todos” mostra o quant com melhor pontuação por modelo — escolha um específico para filtrar.">?</span>';
  html += '</span>';
  ...
  html += '<label class="hwfit-ctx-control" title="Tamanho de contexto para estimativas de encaixe. Reduza para encontrar mais modelos que cabem no seu hardware.">';
  html += '<span>Contexto</span><span class="hwfit-help-chip hwfit-help-chip-inline" title="Tamanho de contexto. Reduza para encontrar mais modelos que cabem no seu hardware; aumente quando precisar de chats ou documentos mais longos.">?</span><input type="range" id="hwfit-context" min="0" max="5" step="1" value="3" />';
  ...
  html += '<button type="button" class="hwfit-gpu-btn" id="hwfit-rescan" title="Reescanear hardware" style="flex-shrink:0;position:relative;top:-3px;left:-1px;">↻ RESCAN</button>';
  html += '<button type="button" class="hwfit-gpu-btn hwfit-hw-manual-btn" id="hwfit-hw-manual-btn" title="Definir hardware manualmente" style="flex-shrink:0;position:relative;top:-3px;left:-1px;display:inline-flex;align-items:center;gap:3px;"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>EDITAR</button>';
  ...
  html += '<option value="speed">Velocidade</option><option value="params">Params</option>';
  html += '<option value="context">Contexto</option></select>';
  html += '</div>';
  html += '<div class="hwfit-manual-panel hidden" id="hwfit-manual-panel">';
  html += '<span class="hwfit-manual-note" style="font-size:10px;opacity:0.6;width:100%;margin-bottom:2px;">Simulador — estes valores SUBSTITUEM o hardware detectado.</span>';
  ...
  html += '<button type="button" class="hwfit-hw-manual-clear">× Limpar</button>';
  html += '</div>';
  ...
  html += '<span class="cookbook-serve-dir-edit" title="Editar em Configurações">editar</span>';
  html += '</div>';
  ...
  html += '<input type="text" class="memory-search-input" id="serve-search" placeholder="Buscar modelos em cache\u2026" style="flex:1;min-width:120px;" />';
  ...
  html += '</select>';
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Pacotes opcionais que estendem as capacidades do Cutia.</p>';
  ...
  if (_es.hfTokenConfigured) {
    html += `<span class="hwfit-hf-check" title="Token salvo" style="font-weight:800;color:var(--green,#50fa7b);font-size:15px;line-height:1;flex-shrink:0;position:relative;top:2px;">✓</span>`;
  }
  ...
  html += '<button class="cal-add-btn cal-add-btn-text" id="cookbook-server-add" title="Adicionar servidor" style="margin-left:auto;"><span class="cal-add-plus">+</span><span class="cal-add-label">Adicionar</span></button>';
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Configure servidores SSH, instale chaves do Cutia, escolha diretórios de modelos, ed set the default server. Local is this machine.</p>';
