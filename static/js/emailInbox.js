...
const _emailSetupHint = () => '<div style="margin-top:6px;opacity:0.72;font-size:11px;">Configurar: <span style="color:var(--accent,var(--red));">Configurações &rsaquo; Integrações</span></div>';
...
const _replySeparator = '---------- Mensagem anterior ----------';
...
export function folderDisplayName(folder) {
  ...
  if (f === 'inbox') return 'CAIXA DE ENTRADA';
  if (f.includes('all mail')) return 'Arquivo / Todos os e-mails';
  if (f.includes('archive')) return 'Arquivo';
  if (f.includes('spam')) return 'Spam';
  if (f.includes('junk')) return 'Lixo eletrônico';
  if (f.includes('trash') || f.includes('bin') || f.includes('deleted')) return 'Lixeira';
  if (f.includes('sent')) return 'Enviados';
  if (f.includes('draft')) return 'Rascunhos';
  return raw;
}
...
function _renderList() {
  ...
  if (_senderFilter) {
    ...
    chip.innerHTML = `<span class="email-filter-chip-label">De: ${_esc(_senderFilterLabel || _senderFilter)}</span><button class="email-filter-chip-clear" title="Limpar filtro">&times;</button>`;
  ...
  if (_emails.length === 0) {
    ...
    empty.textContent = _senderFilter ? `Nenhum e-mail de ${_senderFilterLabel || _senderFilter}` : 'Nenhum e-mail';
...
function _createEmailItem(em) {
    ...
    if (us && us.per_uid && em.uid != null) {
      ...
      for (const k of Object.keys(us.per_uid)) {
        if (k.endsWith(suffix)) {
          ...
          if (score >= 3) { _unreadColor = 'var(--color-error, #e06c75)'; _unreadTitle = 'Urgente — ' + (v.reason || 'precisa de resposta agora'); }
          else if (score === 2) { _unreadColor = '#f0ad4e'; _unreadTitle = 'Responder em breve — ' + (v.reason || ''); }
          break;
        }
      }
    }
  ...
  const spamTag = em.is_spam_verdict
    ? `<span class="email-tag email-tag-spam" title="A IA marcou como spam — clique em ✓ para desmarcar">spam <button class="email-spam-unflag" data-uid="${em.uid}" title="Não é spam">\u2713</button></span>`
    : '';
  ...
  item.innerHTML = `
    ...
    <div class="email-item-content">
      <div class="email-item-top">
        <span class="email-sender email-sender-clickable" style="color:${color}" data-from-addr="${_esc(senderAddr)}" data-from-name="${_esc(senderName)}" title="Mostrar todos os e-mails de ${_esc(senderName)}">${_esc(senderName)}</span>
    ...
    <div class="email-menu-wrap">
      <button class="hamburger email-menu-btn" title="Ações">
...
async function _openEmail(em, itemEl, preloadedData = null, mode = 'reply') {
  ...
  try {
    ...
    if (wantsAiReply) {
      if (data.cached_ai_reply) {
        ...
        draftToastTimer = setTimeout(() => {
          import('./ui.js').then(m => m.showToast && m.showToast('Gerando resposta com IA', { duration: 3000, leadingIcon: 'spinner' })).catch(() => {});
        }, 450);
        try {
          let currentModel = '';
          ...
          if (result.success && result.reply) {
            ...
            import('./ui.js').then(m => m.showError && m.showError('Falha na resposta com IA: ' + _msg)).catch(() => {});
            return;
          }
        } catch (e) {
          if (draftToastTimer) clearTimeout(draftToastTimer);
          console.error('AI reply generation failed:', e);
          import('./ui.js').then(m => m.showError && m.showError('Falha na resposta com IA: ' + (e.message || e))).catch(() => {});
          return;
        }
      }
    }
    ...
    if (_docModule) {
      ...
      if (existingDocId) {
        ...
        if (!docRes.ok) {
          ...
          import('./ui.js').then(m => m.showError && m.showError('Falha ao criar rascunho de resposta (' + docRes.status + ')')).catch(() => {});
          return;
        }
    ...
    import('./ui.js').then(m => m.showError && m.showError('Falha ao responder: ' + msg)).catch(() => {});
  } finally {
...
function _showEmailMenu(em, anchor, itemEl) {
  ...
  const actions = [
    ...
    { label: 'Lembrar de responder', icon: _bellIcon, submenu: 'remind' },
    { label: 'Arquivar', icon: _archiveIcon, action: () => _archiveEmail(em) },
    { label: 'Excluir', icon: _deleteIcon, danger: true, action: () => _deleteEmail(em) },
  ];
...
function _showRemindSubmenu(em, parentDropdown) {
  ...
  header.innerHTML = '<span>Lembrar</span>';
  ...
  const presets = [
    { label: 'Mais tarde hoje', sub: laterToday.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }), date: laterToday },
    { label: 'Amanhã', sub: tomorrow.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }), date: tomorrow },
    { label: 'Próxima semana', sub: nextWeek.toLocaleDateString([], { weekday: 'short' }) + ' ' + nextWeek.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }), date: nextWeek },
  ];
  ...
  customItem.innerHTML = '<span>Escolher data e hora…</span>';
...
async function _createReplyReminder(em, dueDate) {
  ...
  const from = em.from || em.sender || 'alguém';
  const payload = {
    title: `Responder: ${em.subject || '(sem assunto)'}`,
  ...
  try {
    ...
    showToast(`Lembrete definido para ${fmt}`);
    ...
    showError('Falha ao criar lembrete');
  }
}
...
async function _createEmailChat(emailData) {
  try {
    ...
    if (url && model) {
      ...
      if (meta) meta.textContent = `E-mail: ${(emailData.subject || '').slice(0, 60)}`;
    }
  } catch (e) {
...
async function _composeNew() {
  ...
  try {
    ...
    if (!sid) {
      await _createEmailChat({ subject: 'Novo e-mail' });
      try { sid = sessionModule?.getCurrentSessionId?.() || ''; } catch (_) {}
    }
    ...
    if (!sid) {
      ...
      import('./ui.js').then(m => m.showError && m.showError('Não foi possível iniciar um novo e-mail (sem sessão).')).catch(() => {});
      return;
    }
    ...
    if (!res.ok) {
      ...
      import('./ui.js').then(m => m.showError && m.showError('Falha ao criar novo e-mail (' + res.status + ')')).catch(() => {});
      return;
    }
