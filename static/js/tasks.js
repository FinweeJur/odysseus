...
let _tasks = [];
let _tasksFetched = false;   // first-fetch sentinel — `false` → show loading row instead of "Nenhuma tarefa ainda"
...
async function _runFirstOpenOnboarding() {
  try {
    ...
    console.warn('Onboarding de tarefas falhou:', e);
  }
}
...
async function _pauseTask(id) {
  ...
  if (!res.ok) throw new Error('Falha ao pausar tarefa');
}
...
async function _resumeTask(id) {
  ...
  if (!res.ok) throw new Error('Falha ao retomar tarefa');
}
...
async function _runNow(id, force = false) {
  ...
  if (!res.ok) {
    ...
    let msg = `Falha ao acionar tarefa (${res.status})`;
    try {
      const data = await res.json();
      if (data && data.detail) msg = data.detail;
    } catch (_) {}
    if (res.status === 409) msg = 'Tarefa já está em execução';
    throw new Error(msg);
  }
}
...
async function _stopTask(id) {
  ...
  if (!res.ok) {
    let msg = `Falha ao parar tarefa (${res.status})`;
    try {
      const data = await res.json();
      if (data && data.detail) msg = data.detail;
    } catch (_) {}
...
async function _fetchOutputTargets() {
  ...
  try {
    ...
    _outputTargets = [{ value: 'session', label: 'Sessão' }];
  }
...
function _scheduleLabel(task) {
  ...
  if (task.schedule === 'once') {
    if (task.scheduled_date) {
      ...
      return `Uma vez em ${d.toLocaleDateString()} às ${d.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`;
    }
    return 'Uma vez';
  }
  const localTime = _utcTimeToLocal(t);
  if (task.schedule === 'daily') return `Diariamente às ${localTime}`;
  if (task.schedule === 'weekly') {
    const day = DAYS_OF_WEEK[task.scheduled_day ?? 0];
    return `Semanalmente, ${day} às ${localTime}`;
  }
  if (task.schedule === 'monthly') {
    const d = task.scheduled_day ?? 1;
    const suffix = ordinalSuffix(d);
    void suffix;
    return `Mensalmente, dia ${d} às ${localTime}`;
  }
...
function _relativeTime(iso) {
  ...
  const past = diff < 0;
  if (abs < 60000) return past ? 'agora mesmo' : 'em um instante';
  if (abs < 3600000) {
...
function _taskEnterSelect() {
  ...
  if (_sb) { _sb.classList.add('active'); _sb.textContent = 'Cancelar'; }
...
function _taskExitSelect() {
  ...
  const _sb = document.getElementById('tasks-select-btn');
  if (_sb) { _sb.classList.remove('active'); _sb.textContent = 'Selecionar'; }
...
function _taskUpdateBulkCount() {
  ...
  if (c) c.textContent = `${_taskSelected.size} selecionada${_taskSelected.size !== 1 ? 's' : ''}`;
...
async function _taskBulkDelete() {
  ...
  const ok = uiModule?.styledConfirm
    ? await uiModule.styledConfirm(`Excluir ${ids.length} tarefa${ids.length > 1 ? 's' : ''}? Isso não pode ser desfeito.`, { confirmText: 'Excluir', danger: true })
    : confirm(`Excluir ${ids.length} tarefa(s)?`);
  if (!ok) return;
  const results = await Promise.allSettled(ids.map(id => _deleteTask(id)));
  const deletedIds = ids.filter((_, i) => results[i].status === 'fulfilled');
  await _animateTaskRemoval(deletedIds);
  if (uiModule) uiModule.showToast(`${deletedIds.length} tarefa${deletedIds.length > 1 ? 's' : ''} excluída${deletedIds.length > 1 ? 's' : ''}`);
  await _fetchTasks();
...
function _renderTaskChips() {
  ...
  mkChip(`todas (${_tasks.length})`, null, !_taskFilter);
...
function _renderList() {
  ...
  if (_headCount) _headCount.textContent = _tasks.length ? `${_tasks.length} tarefa${_tasks.length !== 1 ? 's' : ''}` : '';
  ...
  if (_tasks.length === 0) {
    ...
    if (!_tasksFetched) {
      list.appendChild(spinnerModule.createLoadingRow('Carregando…'));
    } else {
      list.innerHTML = '<div style="opacity:0.4;font-size:12px;text-align:center;padding:24px 0;">Nenhuma tarefa ainda. Crie uma para começar.</div>';
    }
    return;
  }
  ...
  if (visible.length === 0) {
    list.innerHTML = '<div style="opacity:0.4;font-size:12px;text-align:center;padding:24px 0;">Nenhuma tarefa corresponde.</div>';
    return;
  }
  ...
  for (const task of visible) {
    ...
    const statusBadge = task.status === 'paused'
      ? `<span class="task-status-badge task-state-badge task-paused-badge" data-task-status-action="resume" title="Pausada - clique para retomar" style="position:relative;top:4px;"><svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor"><polygon points="7 4 19 12 7 20 7 4"/></svg><span class="task-state-label">pausada</span></span>`
      : task.status === 'active'
        ? `<span class="task-status-badge task-state-badge task-active-badge" data-task-status-action="pause" title="Ativa - clique para pausar" style="position:relative;top:4px;"><svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="5" width="4" height="14" rx="1"/><rect x="14" y="5" width="4" height="14" rx="1"/></svg><span class="task-state-label">ativa</span></span>`
        : '';
    const builtinBadge = task.is_builtin
      ? `<span class="task-builtin-badge${task.is_modified ? ' modified' : ''}" title="${task.is_modified ? 'Tarefa integrada — editada a partir do padrão' : 'Tarefa integrada'}">integrada${task.is_modified ? ' · editada' : ''}</span>`
      : '';
    ...
    menuBtn.addEventListener('click', (e) => {
      ...
      if (task.status !== 'completed') items.push({ label: 'Executar agora', icon: '<polyline points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/>', action: () => _doRunNow(task.id) });
      items.push({ label: 'Editar', icon: '<path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.12 2.12 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>', action: () => _showForm(task) });
      if (task.status === 'active') items.push({ label: 'Pausar', icon: '<rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/>', action: () => _doPause(task.id) });
      else if (task.status === 'paused') items.push({ label: 'Retomar', icon: '<polygon points="5 3 19 12 5 21 5 3"/>', action: () => _doResume(task.id) });
      items.push({ label: 'Histórico', icon: '<circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/>', action: () => _showRunHistory(task.id, task.name) });
      if (task.is_builtin && task.is_modified) {
        items.push({ label: 'Restaurar padrão', icon: '<polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/>', action: () => _doRevert(task.id) });
      }
      if (_taskClearCacheLabel(task)) {
        items.push({ label: 'Limpar cache', icon: '<path d="M3 6h18"/><path d="M8 6V4h8v2"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v5"/><path d="M14 11v5"/>', action: () => _doClearTaskCache(task.id, _taskClearCacheLabel(task)) });
      }
      items.push({ label: 'Excluir', icon: '<polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/>', action: () => _doDelete(task.id), danger: true });
    ...
    if (task.status !== 'completed') {
      ...
      runBtn.title = 'Executar agora';
      runBtn.style.cssText = 'position:relative;top:1px;margin-right:4px;';
      runBtn.innerHTML = '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><polyline points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg><span>Executar</span>';
    ...
    if (task.next_run && task.status === 'active') metaParts.push('Próxima: ' + _relativeTime(task.next_run));
    if (task.run_count > 0) metaParts.push(task.run_count + ' execuç' + (task.run_count !== 1 ? 'ões' : 'ão'));
    ...
    const extra = [];
    if (task.last_run) extra.push('Última: ' + _relativeTime(task.last_run));
    ...
    if (task.last_run_status) {
      ...
      lr.innerHTML = `<span style="font-weight:600;color:${color};">${isErr ? '✗' : '✓'}</span> <span style="opacity:0.9;">${_esc(prev) || (isErr ? 'Falhou (sem detalhes)' : 'Sucesso (sem saída)')}</span>`;
      lr.title = 'Abrir histórico completo';
