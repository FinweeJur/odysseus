    ...
    const overlay = _modal(`
      <div class="modal-content" style="width:min(560px,94vw);">
        <div class="modal-header">
          ...
          <button class="sig-close modal-close" title="Fechar">×</button>
        </div>
        <div class="modal-body">
          ...
          <div style="margin-top:10px;display:flex;align-items:center;gap:10px;font-size:0.78rem;">
            <label for="sig-smoothness" style="white-space:nowrap;opacity:0.8;">Suavidade</label>
          ...
          <input class="sig-name" type="text" placeholder="Nome (opcional, ex.: 'Completa' ou 'Iniciais')" style="margin-top:10px;">
        </div>
        <div class="modal-footer" style="display:flex;gap:8px;justify-content:flex-end;padding-top:8px;border-top:1px solid var(--border);margin-top:6px;">
          <button class="sig-clear confirm-btn confirm-btn-secondary">Limpar</button>
          <button class="sig-undo confirm-btn confirm-btn-secondary">Desfazer</button>
          <span style="flex:1;"></span>
          <button class="sig-cancel confirm-btn confirm-btn-secondary">Cancelar</button>
          <button class="sig-save confirm-btn confirm-btn-primary" disabled>Salvar</button>
        </div>
      </div>
    `);
    ...
    saveBtn.onclick = async () => {
      ...
      try {
        ...
        const sig = await _saveSignature({
          ...
          name: (nameInput.value || '').trim() || 'Assinatura',
        });
        const out = {
          id: sig.id,
        ...
        alert('Falha ao salvar assinatura: ' + e.message);
  ...
  return new Promise(async (resolve) => {
    ...
    const tiles = sigs.map((s) => `
      <div class="sig-tile" data-id="${s.id}">
        ...
        <button class="sig-tile-del" data-id="${s.id}" title="Excluir">×</button>
      </div>
    `).join('');
    ...
    const overlay = _modal(`
      <div class="modal-content" style="width:min(560px,94vw);">
        <div class="modal-header">
          <h4>Escolha uma assinatura</h4>
          <button class="sig-close modal-close" title="Fechar">×</button>
        </div>
        <div class="modal-body">
          <button class="sig-new-tile confirm-btn confirm-btn-primary" style="width:100%;margin-bottom:12px;padding:8px;">+ Desenhar nova assinatura</button>
          ${tiles ? `<div style="display:grid;grid-template-columns:repeat(3, 1fr);gap:10px;">${tiles}</div>` : '<div style="opacity:0.6;font-size:0.8rem;text-align:center;padding:8px 0;">Nenhuma assinatura salva ainda — desenhe uma acima.</div>'}
        </div>
      </div>
    `);
    ...
    overlay.querySelectorAll('.sig-tile-del').forEach((btn) => {
      btn.addEventListener('click', async (e) => {
        ...
        if (!await window.styledConfirm('Excluir esta assinatura?', { confirmText: 'Excluir', danger: true })) return;
