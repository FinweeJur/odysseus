...
export async function loadPersonalDocs() {
  ...
  try {
    ...
    if (files.length === 0) {
      ...
      placeholder.textContent = 'Solte arquivos acima para adicionar ao RAG';
    ...
    error.textContent = 'Falha ao carregar arquivos';
...
async function _deleteFile(filepath, displayName) {
  if (!await uiModule.styledConfirm(`Remover "${displayName}" do RAG?`, { confirmText: 'Remover', danger: true })) return;
  try {
    ...
    alert('Falha ao excluir arquivo: ' + e.message);
  }
}
...
export async function uploadRagFiles(fileList) {
  ...
  if (zone) zone.textContent = 'Enviando…';
  ...
  try {
    ...
    const data = await res.json();
    if (zone) zone.textContent = 'Solte arquivos aqui ou clique para enviar';
    await loadPersonalDocs();
    return data;
  } catch (e) {
