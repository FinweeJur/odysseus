...
function _showNotesFirstOpenHint(pane) {
  ...
  hint.innerHTML = `
    <div class="tour-hint-text"><b>Notas</b> é sua lista de tarefas básica, e também onde os lembretes são gerenciados.</div>
...
function _undoArchive(note, prevIdx) {
  ...
  _patchNote(note.id, { archived: false }).catch(() => {
    ...
    _renderNotes();
    uiModule.showError('Falha ao desfazer');
  });
}
...
function _formatDueDate(dateStr) {
  ...
  if (hasTime && d < now) return 'atrasado';
  if (!hasTime && diffDays < 0) return 'atrasado';
  if (diffDays === 0) return hasTime ? timeStr : 'hoje';
  if (diffDays === 1) return hasTime ? `amanhã ${timeStr}` : 'amanhã';
...
const _ORDINALS = ['1ª', '2ª', '3ª', '4ª', '5ª'];
const _DAYS = ['Domingo', 'Segunda-feira', 'Terça-feira', 'Quarta-feira', 'Quinta-feira', 'Sexta-feira', 'Sábado'];
function _nthWeekdayLabel(d) {
  const n = Math.ceil(d.getDate() / 7); // 1..5
  return `${_ORDINALS[n - 1] || `${n}ª`} ${_DAYS[d.getDay()]}`;
}
...
function _formatRepeatLabel(repeat, originalDate) {
  ...
  if (norm === 'daily') return 'Diariamente';
  if (norm === 'yearly') return 'Anualmente';
  const parts = norm.split(':');
  if (parts[0] === 'weekly') {
    const wd = parseInt(parts[1], 10);
    if (isNaN(wd)) return 'Semanalmente';
    return `Toda(o) ${_DAYS[wd]}`;
  }
  if (parts[0] === 'monthly') {
    if (parts[1] === 'day') return `Mensalmente no dia ${parts[2]}`;
    if (parts[1] === 'nth') {
      const n = parseInt(parts[2], 10);
      const wd = parseInt(parts[3], 10);
      return `Mensalmente na ${_ORDINALS[n - 1] || `${n}ª`} ${_DAYS[wd]}`;
    }
    if (parts[1] === 'last') {
      const wd = parseInt(parts[2], 10);
      return `Mensalmente na última ${_DAYS[wd]}`;
    }
  }
  return norm;
}
...
export function openPanel() {
  ...
  pane.innerHTML = `
    ...
    <div class="notes-pane-header">
      ...
      <button id="notes-archive-toggle" class="doc-action-icon-btn notes-header-text-btn" title="Ver arquivo" style="opacity:0.8;gap:5px;">
        ...
        <span class="notes-header-btn-label">Arquivo</span>
      </button>
      <button id="notes-view-toggle" class="doc-action-icon-btn notes-header-text-btn" title="Alternar visualização" style="opacity:0.8;gap:5px;">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg>
        <span class="notes-header-btn-label">Alternar</span>
      </button>
    ...
    <div class="notes-search-bar">
      ...
      <button id="notes-select-btn" class="notes-select-trigger" type="button">Selecionar</button>
    </div>
    <div id="notes-bulk-bar" class="memory-bulk-bar hidden">
      ...
      <span id="notes-selected-count">0 Selecionado(s)</span>
      <span style="flex:1"></span>
      <button id="notes-bulk-archive" class="memory-toolbar-btn" disabled>
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px;margin-right:4px;"><rect x="2" y="3" width="20" height="5" rx="1"/><path d="M4 8v11a2 2 0 002 2h12a2 2 0 002-2V8"/><path d="M10 12h4"/></svg>Arquivar
      </button>
      <button id="notes-bulk-delete" class="memory-toolbar-btn danger" disabled>
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px;margin-right:4px;"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/></svg>Excluir
      </button>
    </div>
  ...
  if (viewBtn) {
    ...
    const _setViewLabel = () => {
      ...
      if (lbl) lbl.textContent = _viewMode === 'grid' ? 'Lista' : 'Grade';
    };
    _setViewLabel();
...
function _exitSelectMode() {
  ...
  if (btn) { btn.classList.remove('active'); btn.textContent = 'Selecionar'; }
...
function _renderLabels(root = document) {
  ...
  let html = `<button class="notes-label-chip${allActive ? ' active' : ''}" data-action="all">Todos</button>`;
  html += `<button class="notes-label-chip${_activeFilter === 'default' ? ' active' : ''}" data-action="default" title="Mostrar notas sem tags">Padrão <span class="notes-label-chip-count">${defaultCount}</span></button>`;
  if (todayCount > 0) {
    ...
    html += `<button class="notes-label-chip notes-label-chip-today${isOn ? ' active' : ''}" data-action="today" title="Próximo passo de cada meta">${icon}Hoje <span class="notes-label-chip-count">${todayCount}</span></button>`;
  }
  if (goalCount > 0) {
    ...
    html += `<button class="notes-label-chip notes-label-chip-goals${isOn ? ' active' : ''}" data-action="goals" title="Mostrar apenas metas">${icon}Metas <span class="notes-label-chip-count">${goalCount}</span></button>`;
  }
  ...
  html += `<button class="${reminderCls}" data-action="reminders" title="${isReminderOn ? 'Mostrando apenas lembretes — clique para mostrar todos' : isReminderOff ? 'Ocultando lembretes — clique para mostrar apenas lembretes' : 'Clique para filtrar lembretes'}">${reminderIcon}Lembretes <span class="notes-label-chip-count">${reminderCount}</span></button>`;
  const showingReminders = _activeFilter === 'reminders';
  if (showingReminders && pastReminderCount > 0) {
    html += `<button class="notes-label-chip notes-label-clear-past" data-action="clear-past-reminders" title="Excluir lembretes cujo tempo passou"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>Limpar passado <span class="notes-label-chip-count">${pastReminderCount}</span></button>`;
  }
...
function _renderNotes() {
  ...
  for (const note of sorted) {
    ...
    html += `<div class="note-card${note.pinned ? ' note-card-pinned' : ''}${cc}${sel}${goalClass}${reminderGlowClass}${_selectMode ? ' note-card-selectmode' : ''}" draggable="${(_selectMode || _isNotesMobileMode()) ? 'false' : 'true'}" data-note-id="${note.id}"${cardStyle}>
      ...
      ${goalPill}
      <button class="note-card-pin${note.pinned ? ' active' : ''}" data-note-id="${note.id}" title="${note.pinned ? 'Desafixar' : 'Afixar'}">
      ...
      ${_showingArchived
        ? `<button class="note-card-corner-trash" data-note-id="${note.id}" title="Excluir permanentemente" aria-label="Excluir permanentemente">
          ...
          <button class="note-card-corner-unarchive" data-note-id="${note.id}" title="Restaurar" aria-label="Restaurar nota">
        ...
        : `<button class="note-card-done" data-note-id="${note.id}" title="Marcar como concluído" aria-label="Marcar como concluído">
          ...
          ${_hasItems(note) ? `<button class="note-card-copy note-card-copy-corner" data-note-id="${note.id}" title="Copiar todos os itens" aria-label="Copiar todos os itens">
      ...
      ${contentHtml}
      ${_hasItems(note) ? `<div class="note-cl-quickadd"><input type="text" class="note-cl-quickadd-input" placeholder="+ Adicionar item" data-note-id="${note.id}" /></div>` : ''}
      ${reminderTagHtml}
      ${noteTags.length ? `<div class="note-card-label">${noteTags.map(t => `<button type="button" class="note-card-label-chip" data-note-label-filter="${_esc(t)}" title="Filtrar #${_esc(t)}">#${_esc(t)}</button>`).join(' ')}</div>` : ''}
      ${note.agent_session_id ? `<button class="note-agent-tag" data-note-id="${note.id}" data-session-id="${_esc(note.agent_session_id)}" title="Abrir o chat do agente para esta nota">
      ...
      <div class="note-card-actions">
        ...
        <span style="flex:1"></span>
        ${_showingArchived ? `
        <button class="note-card-action note-card-delete" data-note-id="${note.id}" title="Excluir permanentemente">
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/></svg>
        </button>
        <button class="note-card-action note-card-unarchive" data-note-id="${note.id}" title="Restaurar">
        ...
        ${_hasItems(note) ? `
        <button class="note-card-action note-card-copy" data-note-id="${note.id}" title="Copiar todos os itens">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
        </button>` : ''}
        <button class="note-card-action note-card-archive" data-note-id="${note.id}" title="Salvar (arquivar)">
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg>
        </button>
        <button class="note-card-action note-card-delete" data-note-id="${note.id}" title="Excluir">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
        </button>
        <button class="note-card-action note-card-corner-menu" data-note-id="${note.id}" title="Mais" aria-label="Mais ações">
