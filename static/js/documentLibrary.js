...
// ---- Library animation helpers ----
  ...
  async function _copyChatById(sessionId) {
    try {
      ...
      if (uiModule && uiModule.showError) uiModule.showError('Falha ao copiar conversa');
    }
  }
  ...
  function libraryRelativeTime(isoString) {
    ...
    if (diffD < 14) return diffD + 'd atrás';
    const diffW = Math.floor(diffD / 7);
    if (diffW < 8) return diffW + 'sem atrás';
  ...
  function libraryRenderGrid() {
    ...
    if (_libraryDocs.length === 0) {
      if (_librarySearch || _libraryActiveLanguage) {
        grid.innerHTML = '<div class="doclib-empty">Nenhum documento corresponde à sua busca.</div>';
      } else {
        ...
        grid.innerHTML =
          '<div class="doclib-empty" style="display:flex;align-items:center;justify-content:center;gap:8px;flex-wrap:wrap;">' +
            ...
            '<span style="opacity:0.7;font-size:11px;">' +
              ...
              ' &middot; ou crie um em uma sessão' +
            '</span>' +
          '</div>';
    ...
    if (shownCount < _libraryTotal) {
      ...
      btn.textContent = `Carregar mais (${shownCount} de ${_libraryTotal})`;
  ...
  function libraryCreateCard(doc) {
    ...
    const _langSvg = doc.language && doc.language !== 'text'
      ...
      : '';
    titleEl.innerHTML = (_langSvg || _GEN_DOC_ICON) + _hlSearch(doc.title || 'Sem título');
    ...
    if (doc.session_id) {
      ...
      openItem.title = 'Abrir no editor';
    ...
    cloneItem.title = 'Clonar para a sessão ativa';
    ...
    exportItem.addEventListener('click', async (e) => {
      ...
      } catch { if (uiModule) uiModule.showError('Falha ao exportar documento'); }
    });
    ...
    archiveItem.innerHTML = _di(_archiveIco) + `<span>${_libraryArchivedView ? 'Restaurar' : 'Arquivar'}</span>`;
    archiveItem.title = _libraryArchivedView ? 'Restaurar para documentos ativos' : 'Arquivar (ocultar da lista principal)';
    archiveItem.addEventListener('click', async (e) => {
      ...
      try {
        ...
        libraryRenderGrid();
        if (uiModule) uiModule.showToast(toArchived ? 'Arquivado' : 'Restaurado');
      } catch { if (uiModule) uiModule.showError('Falha ao ' + (toArchived ? 'arquivar' : 'restaurar')); }
    });
    ...
    deleteItem.innerHTML = _di(_deleteIco) + '<span>Excluir</span>';
    ...
    cloneBtn.title = 'Clonar — copiar para a sessão ativa';
    ...
    deleteBtn.innerHTML = '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:3px;"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg>Excluir';
    ...
    archiveBtn.innerHTML = '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:3px;"><polyline points="21 8 21 21 3 21 3 8"/><rect x="1" y="3" width="22" height="5"/><line x1="10" y1="12" x2="14" y2="12"/></svg>' + (_libraryArchivedView ? 'Restaurar' : 'Arquivar');
    archiveBtn.title = _libraryArchivedView ? 'Restaurar para documentos ativos' : 'Arquivar (ocultar da lista principal)';
  ...
  async function libraryExpandCard(card, doc) {
    ...
    try {
      ...
      if (!existingPre) {
        preview.innerHTML = '<div style="padding:8px;color:var(--color-error);font-size:10px;">Falha ao carregar</div>';
      }
      if (actionsBar && !preview.contains(actionsBar)) preview.appendChild(actionsBar);
    }
  }
  ...
  async function libraryImportDocument(doc) {
    ...
    if (!sessionId) {
      ...
      if (!sessionId) {
        if (uiModule) uiModule.showError('Não foi possível criar uma sessão');
        return;
      }
    }
    try {
      ...
      let baseTitle = src.title || doc.title || 'Sem título';
      ...
      _syncDocIndicator();
      if (uiModule) uiModule.showToast('Documento clonado para a sessão');
    } catch (e) {
      console.error('Failed to import document:', e);
      if (uiModule) uiModule.showError('Falha ao importar documento');
    }
  }
  ...
  function libraryExitSelectMode() {
    ...
    if (selectBtn) { selectBtn.classList.remove('active'); selectBtn.textContent = 'Selecionar'; }
  ...
  function libraryUpdateBulkCount() {
    ...
    if (countEl) countEl.textContent = `${_librarySelectedIds.size} selecionado(s)`;
    ...
    if (archiveBtn) {
      ...
      archiveBtn.textContent = _libraryArchivedView ? 'Restaurar' : 'Arquivar';
    }
  }

  async function libraryDeleteSingle(docId, card) {
    if (uiModule && uiModule.styledConfirm) {
      const ok = await uiModule.styledConfirm('Excluir este documento?', { confirmText: 'Excluir', danger: true });
      if (!ok) return;
    } else if (!confirm('Excluir este documento?')) {
      return;
    }
    try {
      ...
      if (uiModule) uiModule.showToast('Documento excluído');
    } catch (e) {
      if (uiModule) uiModule.showError(`Falha ao excluir documento: ${e.message || e}`);
    }
  }
  ...
  async function libraryBulkDelete() {
    ...
    if (uiModule && uiModule.styledConfirm) {
      const ok = await uiModule.styledConfirm(
        `Excluir ${count} documento${count !== 1 ? 's' : ''}?`,
        { confirmText: 'Excluir', danger: true }
      );
      if (!ok) return;
    } else if (!confirm(`Excluir ${count} documento${count !== 1 ? 's' : ''}?`)) {
      return;
    }
    ...
    if (uiModule) {
      const msg = failed > 0
        ...
        : `${deleted} documento${deleted !== 1 ? 's' : ''} excluído${deleted !== 1 ? 's' : ''}`;
  ...
  async function libraryBulkArchive() {
    ...
    await libraryFetch(false);
    if (uiModule) {
      const verb = toArchived ? 'Arquivado(s)' : 'Restaurado(s)';
      const msg = failed > 0 ? `${verb} ${done} · ${failed} falhou(aram)` : `${done} documento${done !== 1 ? 's' : ''} ${verb.toLowerCase()}`;
      (failed > 0 ? uiModule.showError : uiModule.showToast)(msg);
    }
  }
  ...
  async function libraryBulkClone() {
    ...
    if (uiModule) {
      const msg = failed > 0
        ? `${done} clonado(s) · ${failed} falhou(aram)`
        : `${done} documento${done !== 1 ? 's' : ''} clonado${done !== 1 ? 's' : ''}`;
      (failed > 0 ? uiModule.showError : uiModule.showToast)(msg);
    }
  }
  ...
  async function libraryBulkExport() {
    ...
    if (_librarySelectedIds.size > 5) {
      const ids = [..._librarySelectedIds];
      try {
        if (uiModule) uiModule.showToast(`Compactando ${ids.length} documentos…`);
        ...
        if (uiModule) uiModule.showToast(`${ids.length} documentos exportados (zip)`);
      } catch (e) {
        if (uiModule) uiModule.showError('Falha ao criar zip');
      }
      return;
    }
    const extMap = {
    ...
    if (uiModule) uiModule.showToast(`${_librarySelectedIds.size} documento${_librarySelectedIds.size !== 1 ? 's' : ''} exportado${_librarySelectedIds.size !== 1 ? 's' : ''}`);
  }
  ...
  export function openLibrary(opts) {
    ...
    modal.innerHTML = `
      <div class="modal-content doclib-modal-content" style="width:min(640px, 92vw);max-height:85vh;background:var(--bg);">
        ...
        <div class="lib-tabs" id="doclib-lib-tabs" style="padding:0 10px;">
          <button class="lib-tab" data-doclib-tab="chats"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:3px;"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>Conversas</button>
          ...
          <button class="lib-tab" data-doclib-tab="research"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:-1px;margin-right:3px;"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/><line x1="11" y1="8" x2="11" y2="14"/><line x1="8" y1="11" x2="14" y2="11"/></svg>Pesquisa</button>
          <button class="lib-tab" data-doclib-tab="archive"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:3px;"><polyline points="21 8 21 21 3 21 3 8"/><rect x="1" y="3" width="22" height="5"/><line x1="10" y1="12" x2="14" y2="12"/></svg>Arquivo</button>
        </div>
        <div class="modal-body" style="display:flex;flex-direction:column;gap:10px;overflow:hidden;">
          <div id="doclib-panel-chats" data-doclib-panel="chats" class="admin-card" style="display:none;flex:1;flex-direction:column;overflow:hidden;">
            <div style="display:flex;align-items:baseline;gap:8px;margin-bottom:2px;">
              <h2 style="margin:0;padding:0;line-height:1;">Conversas <span id="doclib-chats-stats" class="memory-count" style="font-size:0.6em;opacity:0.6;font-weight:normal"></span></h2>
            </div>
            <p class="memory-desc doclib-desc">Todas as sess\u00f5es de chat ativas. Clique para abrir.</p>
            <div class="memory-toolbar">
              <div class="memory-category-filters">
                ...
                <button class="memory-toolbar-btn" id="doclib-chats-select-btn">Selecionar</button>
                <button class="memory-toolbar-btn" id="doclib-chats-tidy-btn" title="Organiza\u00e7\u00e3o com IA: excluir sess\u00f5es in\u00fateis e organizar em pastas"><svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor" style="vertical-align:-1px;margin-right:2px;"><path d="M12 0L14.59 8.41L23 12L14.59 15.59L12 24L9.41 15.59L1 12L9.41 8.41Z"/></svg> Organizar</button>
              </div>
              <input type="text" id="doclib-chats-search" placeholder="Buscar conversas\u2026" class="memory-search-input" />
            ...
            <div id="doclib-chats-bulk" class="memory-bulk-bar hidden" style="margin-bottom:5px;">
              ...
              <span id="doclib-chats-selected-count">0 selecionado(s)</span>
              <button class="memory-toolbar-btn" id="doclib-chats-bulk-archive" style="position:relative;top:-3px;left:2px;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:3px;"><polyline points="21 8 21 21 3 21 3 8"/><rect x="1" y="3" width="22" height="5"/><line x1="10" y1="12" x2="14" y2="12"/></svg>Arquivar</button>
              <button class="memory-toolbar-btn danger" id="doclib-chats-bulk-delete" style="position:relative;left:2px;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:3px;"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/></svg>Excluir</button>
          ...
          <div id="doclib-panel-archive" data-doclib-panel="archive" class="admin-card" style="display:none;flex:1;flex-direction:column;overflow:hidden;">
            <div style="display:flex;align-items:baseline;gap:8px;margin-bottom:2px;">
              <h2 style="margin:0;padding:0;line-height:1;position:relative;top:2px;">Arquivo <span id="doclib-arc-stats" class="memory-count" style="font-size:0.6em;opacity:0.6;font-weight:normal"></span></h2>
            </div>
            <p class="memory-desc doclib-desc" style="position:relative;top:0.5px;">Sess\u00f5es arquivadas. Restaure para tornar ativas de novo.</p>
            <div class="memory-toolbar">
              <div class="memory-category-filters">
                ...
                <button class="memory-toolbar-btn" id="doclib-arc-select-btn">Selecionar</button>
              </div>
              <input type="text" id="doclib-arc-search" placeholder="Buscar no arquivo\u2026" class="memory-search-input" />
            ...
            <div id="doclib-arc-bulk" class="memory-bulk-bar hidden" style="margin-bottom:5px;">
              ...
              <span id="doclib-arc-selected-count">0 selecionado(s)</span>
          ...
          <div id="doclib-panel-research" data-doclib-panel="research" class="admin-card" style="display:none;flex:1;flex-direction:column;overflow:hidden;">
            <div style="display:flex;align-items:baseline;gap:8px;margin-bottom:2px;margin-top:10px;">
              <h2 style="margin:0;padding:0;line-height:1;">Pesquisa <span id="doclib-research-stats" class="memory-count" style="font-size:0.6em;opacity:0.6;font-weight:normal"></span></h2>
            </div>
            <p class="memory-desc doclib-desc" style="position:relative;top:-1px;">Relat\u00f3rios de pesquisa profunda conclu\u00eddos. Clique para visualizar.</p>
            <div class="memory-toolbar">
              <div class="memory-category-filters">
                <select class="memory-sort-select" id="doclib-research-sort">
                  <option value="recent">Recentes</option>
                  <option value="oldest">Mais antigos</option>
                  <option value="most-sources">Mais fontes</option>
                  <option value="alpha">A\u2013Z</option>
                </select>
                <button class="memory-toolbar-btn" id="doclib-research-select-btn">Selecionar</button>
                <button class="memory-toolbar-btn" id="doclib-research-tidy-btn" title="Organizar: excluir pesquisas sem fontes ou relat\u00f3rios vazios">Organizar</button>
              </div>
              <input type="text" id="doclib-research-search" placeholder="Buscar pesquisas\u2026" class="memory-search-input" />
            </div>
            <div id="doclib-research-bulk" class="memory-bulk-bar hidden" style="margin-bottom:5px;">
              ...
              <span id="doclib-research-selected-count">0 selecionado(s)</span>
          ...
          <div data-doclib-panel="documents" class="admin-card" style="flex:1;display:flex;flex-direction:column;overflow:hidden;">
            <div style="display:flex;align-items:baseline;gap:8px;margin-bottom:2px;">
              ...
              <button class="memory-toolbar-btn" id="doclib-import-file-btn" title="Importar arquivos do disco" style="margin-left:auto;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:2px;"><polyline points="7 10 12 5 17 10"/><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="21" x2="19" y2="21"/></svg> Importar</button>
              <button class="memory-toolbar-btn" id="doclib-create-btn" title="Criar novo documento em branco"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:3px;"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg> Criar</button>
            </div>
            <p class="memory-desc doclib-desc">Abra documentos em uma sess\u00e3o, clone para uma nova ou importe novos arquivos.</p>
            <div class="memory-toolbar">
              <div class="memory-category-filters">
                <select class="memory-sort-select" id="doclib-sort">
                  <option value="recent">Recentes</option>
                  <option value="oldest">Mais antigos</option>
                  <option value="edits">Mais editados</option>
                  <option value="alpha">A\u2013Z</option>
                </select>
                <button class="memory-toolbar-btn" id="doclib-select-btn" title="Selecionar documentos">Selecionar</button>
                <button class="memory-toolbar-btn" id="doclib-tidy-btn" title="Organizar: remover documentos vazios / in\u00fateis / duplicados">Organizar</button>
              </div>
              <input type="text" id="doclib-search" placeholder="Buscar t\u00edtulos e conte\u00fado\u2026" class="memory-search-input" />
            ...
            <div id="doclib-bulk-bar" class="memory-bulk-bar hidden" style="margin-bottom:5px;">
              ...
              <span id="doclib-selected-count">0 selecionado(s)</span>
              <button id="doclib-bulk-actions" class="memory-toolbar-btn" style="position:relative;top:-2px;margin-left:auto;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:3px;"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg>A\u00e7\u00f5es <span style="opacity:0.55;font-size:9px;">&#9660;</span></button>
              <button id="doclib-bulk-cancel" class="memory-toolbar-btn" title="Cancelar (Esc)" style="margin-left:4px;margin-right:4px;padding:3px 6px;position:relative;top:-2px;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
            </div>
            <div class="doclib-grid" id="doclib-grid"></div>
            <button class="doclib-load-more" id="doclib-load-more" style="display:none">Carregar mais</button>
          </div>
        </div>
      </div>
    `;
    ...
    async function _toggleChatPreview(card, session) {
      ...
      try {
        ...
        if (deleteBtn) deleteBtn.addEventListener('click', async (e) => {
          e.stopPropagation();
          if (!await window.styledConfirm('Excluir esta conversa?', { confirmText: 'Excluir', danger: true })) return;
    ...
    function _renderChatsGrid() {
      ...
      if (!filtered.length) {
        ...
        grid.innerHTML = '<div class="doclib-empty">Nenhuma conversa' + _sadIco + '</div>';
      ...
      for (const s of visible) {
        ...
        card.innerHTML =
          '<div class="doclib-chat-header" style="display:flex;align-items:center;width:100%;gap:6px;">' +
            ...
            '<div style="flex:1;min-width:0;">' +
              '<div class="memory-item-title">' + chatIconSvg + _esc(s.name || 'Sem título') + msgCountHtml + '</div>' +
            ...
            '</div>' +
            chevronSvg +
            '<div class="memory-item-actions"><button class="memory-item-btn _chat-menu" title="Ações"><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="5" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="12" cy="19" r="2"/></svg></button></div>' +
          '</div>' +
        ...
        card.querySelector('._chat-menu').addEventListener('click', (e) => { e.stopPropagation(); _showLibDropdown(e.currentTarget, [
          ...
          { label: 'Arquivar', action: async () => { await fetch(API_BASE + '/api/session/' + s.id + '/archive', { method: 'POST', headers: {'Content-Type':'application/json'} }); _renderLibChats(); } },
    ...
    function _updateChatsCount() { const el = document.getElementById('doclib-chats-selected-count'); if (el) el.textContent = _chatsSelected.size + ' Selecionado(s)'; }
    ...
    document.getElementById('doclib-chats-bulk-archive').addEventListener('click', async () => {
      ...
      if (failed.length && grid) {
        ...
        if (window.uiModule) window.uiModule.showError(`Falha ao arquivar ${failed.length} de ${ids.length} conversa${ids.length > 1 ? 's' : ''}`);
      }
    ...
    document.getElementById('doclib-chats-bulk-delete').addEventListener('click', async () => {
      const count = _chatsSelected.size;
      if (!count) return;
      if (!await window.styledConfirm(`Excluir ${count} conversa${count > 1 ? 's' : ''}? Isso não pode ser desfeito.`, { confirmText: 'Excluir', danger: true })) return;
      ...
      if (failed.length && grid) {
        ...
        if (window.uiModule) window.uiModule.showError(`Falha ao excluir ${failed.length} de ${ids.length} conversa${ids.length > 1 ? 's' : ''}`);
      }
      _chatsSelected.clear();
      _chatsSelectMode = false;
      document.getElementById('doclib-chats-bulk').classList.add('hidden');
    ...
    document.getElementById('doclib-chats-tidy-btn').addEventListener('click', async () => {
      ...
      try {
        ...
        if (data.status === 'ok') {
          if (window.uiModule) window.uiModule.showToast('Organizadas ' + data.updated + ' sessões em ' + data.folders.length + ' pastas');
          if (window.sessionModule) await window.sessionModule.loadSessions();
          _renderLibChats();
        } else {
          if (window.uiModule) window.uiModule.showToast(data.reason || 'Nada para organizar');
        }
      } catch (e) {
    ...
    async function _toggleArcDocPreview(card, d) {
      ...
      try {
        ...
        actions.querySelector('.doclib-chat-delete-btn').addEventListener('click', async (ev) => {
          ev.stopPropagation();
          if (!await window.styledConfirm('Excluir este documento?', { confirmText: 'Excluir', danger: true })) return;
    ...
    function _renderArcGrid() {
      ...
      if (stats) stats.textContent = (filtered.length + filtDocs.length + filtResearch.length) + ' arquivado(s)';
      ...
      if (!filtered.length && !filtDocs.length && !filtResearch.length) {
        ...
        grid.innerHTML = '<div class="doclib-empty">Nenhum item arquivado' + _neutralIco + '</div>';
      ...
      for (const s of visible) {
        ...
        card.innerHTML =
          '<div class="doclib-chat-header" style="display:flex;align-items:center;width:100%;gap:6px;">' +
            cbHtml +
            '<div style="flex:1;min-width:0;">' +
              '<div class="memory-item-title">' + arcIconSvg + _esc(s.name || 'Sem título') + '</div>' +
              '<div class="memory-item-meta" style="font-size:10px;opacity:0.4;margin-top:2px;">' + [model, _relTime(s.updated_at)].filter(Boolean).join(' \u00b7 ') + '</div>' +
            '</div>' +
            '<div class="memory-item-actions"><button class="memory-item-btn _arc-menu" title="Ações"><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="5" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="12" cy="19" r="2"/></svg></button></div>' +
          '</div>' +
          '<div class="doclib-chat-preview" style="display:none;"></div>';
        const cb = card.querySelector('.memory-select-cb');
        ...
        card.querySelector('._arc-menu').addEventListener('click', (e) => { e.stopPropagation(); _showLibDropdown(e.currentTarget, [
          ...
          { label: 'Excluir', action: async () => {
            if (!await window.styledConfirm('Excluir esta conversa permanentemente?', { confirmText: 'Excluir', danger: true })) return;
            await fetch(API_BASE + '/api/session/' + s.id, { method: 'DELETE' });
            _renderLibArchive();
          }, danger: true },
        ], { onSelect: () => {
      ...
      for (const d of filtDocs) {
        ...
        card.innerHTML =
          '<div class="doclib-chat-header" style="display:flex;align-items:center;width:100%;gap:6px;">' +
            _dcb +
            '<div style="flex:1;min-width:0;">' +
              '<div class="memory-item-title">' + _arcDocIco + _esc(d.title || 'Sem título') + '</div>' +
            ...
            '</div>' +
            '<div class="memory-item-actions"><button class="memory-item-btn _arc-doc-menu" title="Ações"><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="5" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="12" cy="19" r="2"/></svg></button></div>' +
          '</div>' +
          '<div class="doclib-chat-preview" style="display:none;"></div>';
        ...
        card.querySelector('._arc-doc-menu').addEventListener('click', (e) => { e.stopPropagation(); _showLibDropdown(e.currentTarget, [
          ...
          { label: 'Excluir', danger: true, action: async () => { if (!await window.styledConfirm('Excluir este documento?', { confirmText: 'Excluir', danger: true })) return; await fetch(API_BASE + '/api/document/' + d.id, { method: 'DELETE', credentials: 'same-origin' }); _renderLibArchive(); } },
        ], { onSelect: () => {
          _arcSelectMode = true;
      ...
      for (const r of filtResearch) {
        ...
        card.innerHTML =
          '<div class="doclib-chat-header" style="display:flex;align-items:center;width:100%;gap:6px;">' +
            _rcb +
            '<div style="flex:1;min-width:0;">' +
              '<div class="memory-item-title">' + _arcResIco + _esc(r.query || 'Pesquisa') + '</div>' +
              '<div class="memory-item-meta" style="font-size:10px;opacity:0.4;margin-top:2px;">' + ['Pesquisa', (r.source_count ? r.source_count + ' fontes' : ''), _relTime(r.completed_at ? new Date(r.completed_at * 1000).toISOString() : '')].filter(Boolean).join(' · ') + '</div>' +
            '</div>' +
            '<div class="memory-item-actions"><button class="memory-item-btn _arc-res-menu" title="Ações"><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="5" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="12" cy="19" r="2"/></svg></button></div>' +
          '</div>' +
          '<div class="doclib-chat-preview" style="display:none;"></div>';
        ...
        card.querySelector('._arc-res-menu').addEventListener('click', (e) => { e.stopPropagation(); _showLibDropdown(e.currentTarget, [
          ...
          { label: 'Excluir', danger: true, action: async () => { if (!await window.styledConfirm('Excluir esta pesquisa?', { confirmText: 'Excluir', danger: true })) return; await fetch('/api/research/' + r.id, { method: 'DELETE', credentials: 'same-origin' }); _renderLibArchive(); } },
        ], { onSelect: () => {
          _arcSelectMode = true;
    ...
    function _updateArcCount() { const el = document.getElementById('doclib-arc-selected-count'); if (el) el.textContent = _arcSelected.size + ' Selecionado(s)'; }
    ...
    document.getElementById('doclib-arc-bulk-delete').addEventListener('click', async () => {
      ...
      if (!count) return;
      if (!await window.styledConfirm(`Excluir ${count} item${count > 1 ? 's' : ''} arquivado${count > 1 ? 's' : ''} permanentemente?`, { confirmText: 'Excluir', danger: true })) return;
      const grid = document.getElementById('doclib-arc-grid');
      if (grid) {
    ...
    async function _toggleResearchPreview(card, item) {
      ...
      if (discussBtn) discussBtn.addEventListener('click', async (e) => {
        ...
        try {
          ...
          if (uiModule) uiModule.showError('Não foi possível iniciar a discussão: ' + (err.message || err));
        }
      });
      const openBtn = preview.querySelector('.doclib-chat-open-btn');
      if (openBtn) openBtn.addEventListener('click', (e) => {
      ...
      if (delBtn) delBtn.addEventListener('click', async (e) => {
        ...
        const ok = uiModule && uiModule.styledConfirm
          ? await uiModule.styledConfirm('Excluir este relatório de pesquisa?', { confirmText: 'Excluir', danger: true })
          : window.confirm('Excluir este relatório de pesquisa?');
        if (!ok) return;
        try {
          ...
          if (uiModule && uiModule.showError) uiModule.showError('Falha ao excluir: ' + err.message);
        }
      });
    ...
    function _renderResearchGrid() {
      ...
      if (stats) stats.textContent = items.length + ' pesquisa' + (items.length !== 1 ? 's' : '');
      if (!items.length) {
        grid.innerHTML =
          '<div class="hwfit-loading" style="display:flex;align-items:center;justify-content:center;gap:8px;flex-wrap:wrap;">' +
            '<span>Nenhuma pesquisa ainda</span>' +
            '<span style="opacity:0.7;font-size:11px;">' +
              'crie uma na aba <a href="#" data-doclib-open-research style="color:var(--accent,var(--red));text-decoration:underline;">Pesquisa Profunda</a>' +
            '</span>' +
          '</div>';
      ...
      for (const r of items) {
        ...
        html += `</div>`;
        if (!_researchSelectMode) html += `<div class="memory-item-actions"><button class="memory-item-btn doclib-research-delete" data-rid="${r.id}" title="Excluir"><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="5" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="12" cy="19" r="2"/></svg></button></div>`;
        html += `</div>`;
        ...
        btn.addEventListener('click', (e) => {
          ...
          _showLibDropdown(btn, [
            ...
            { label: 'Excluir', danger: true, action: async () => {
                if (!await window.styledConfirm('Excluir esta pesquisa?', { confirmText: 'Excluir', danger: true })) return;
                const card = btn.closest('.doclib-research-card');
                if (card) {
                  card.style.transition = 'opacity 0.25s, transform 0.25s';
                  card.style.opacity = '0';
    ...
    function _updateResearchCount() {
      ...
      if (el) el.textContent = _researchSelected.size + ' Selecionado(s)';
      const arc = document.getElementById('doclib-research-bulk-archive');
      if (arc) arc.textContent = _researchArchivedView ? 'Restaurar' : 'Arquivar';
    }
    ...
    document.getElementById('doclib-research-tidy-btn')?.addEventListener('click', async (e) => {
      ...
      try {
        ...
        if (candidates.length === 0) {
          if (uiModule) uiModule.showToast('Nada para organizar');
          return;
        }
        ...
        _renderResearchGrid();
        if (uiModule) uiModule.showToast('Excluído(s) ' + candidates.length);
      } finally {
        sp.stop();
        btn.disabled = false;
    ...
    document.getElementById('doclib-research-bulk-delete')?.addEventListener('click', async () => {
      ...
      if (!count) return;
      if (!await window.styledConfirm(`Excluir ${count} relatório${count > 1 ? 's' : ''} de pesquisa permanentemente?`, { confirmText: 'Excluir', danger: true })) return;
      const grid = document.getElementById('doclib-research-grid');
      if (grid) {
        grid.querySelectorAll('.doclib-research-card').forEach(card => {
    ...
    if (createBtn) {
      createBtn.addEventListener('click', async () => {
        ...
        try {
          ...
          if (uiModule) uiModule.showError('Falha ao criar documento');
        }
      });
    }
    ...
    if (tidyBtn) tidyBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (uiModule) uiModule.showToast('Falha ao organizar');
        tidyBtn.disabled = false;
        tidyBtn.classList.remove('spinning');
        tidyBtn.innerHTML = origHTML;
      }
    ...
    if (bulkActionsBtn) bulkActionsBtn.addEventListener('click', (e) => {
      ...
      if (_librarySelectedIds.size === 0) {
        if (uiModule) uiModule.showToast('Selecione documentos primeiro');
        return;
      }
  ...
  export function closeLibrary() {
    ...
    if (modal) {
      ...
      if (content) {
        ...
        // Track if modal was already removed to avoid double-removal
        let removed = false;
        const removeModal = () => {
          if (!removed && modal.parentElement) {
            removed = true;
            modal.remove();
          }
        };
        ...
        content.addEventListener('animationend', removeModal, { once: true });
        // Fallback timeout in case animationend doesn't fire
        // (animation duration is 0.18s, so 200ms provides margin)
