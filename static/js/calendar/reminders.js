  ...
  try {
    ...
    for (const note of notes) {
      ...
      fetch(`${API_BASE}/api/notes/fire-reminder`, {
        ...
        body: JSON.stringify({
          note_id: note.id,
          title: note.title || 'Lembrete do Calendário',
          body,
        }),
      }).catch(() => {});
      if ('Notification' in window && Notification.permission === 'granted') {
        new Notification(note.title || 'Lembrete do Calendário', {
          body,
          icon: '/static/favicon.png',
          tag: `cal-remind-${note.id}`,
        });
      }
      if (uiModule.showToast) uiModule.showToast((note.title || 'Lembrete do Calendário') + (body ? ' — ' + body : ''));
    }
