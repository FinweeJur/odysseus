...
const _CMP_START_LABEL = _CMP_PLAY_ICON + ' Iniciar';
...
async function showModelSelector() {
  return new Promise((resolve) => {
    ...
    blindBtn.title = 'Modo Cego — oculta os nomes dos modelos até você votar';
    blindBtn.innerHTML = EYE_CLOSED + _toggleLabel('Cego');
    blindBtn.addEventListener('click', () => {
      state._blindMode = !state._blindMode;
      blindBtn.classList.toggle('active', state._blindMode);
      blindBtn.innerHTML = (state._blindMode ? EYE_CLOSED : EYE_OPEN) + _toggleLabel('Cego');
      ...
      uiModule.showToast('Modo: Cego ' + (state._blindMode ? 'ativado' : 'desativado'));
      _updateModeLabel();
      _setModeHint(state._blindMode
        ? '<span style="color:var(--color-blind-orange)">Modo cego</span>: os nomes dos modelos ficam ocultos até você votar.'
        : '<span style="color:var(--color-blind-orange)">Modo cego desativado</span>: os nomes dos modelos são exibidos.');
    });
    ...
    parallelBtn.title = 'Paralelo — executa todos os modelos de uma vez em vez de um por vez';
    parallelBtn.innerHTML = ICON_PARALLEL + _toggleLabel('Paralelo');
    parallelBtn.addEventListener('click', () => {
      state._parallel = !state._parallel;
      parallelBtn.classList.toggle('active', state._parallel);
      parallelBtn.innerHTML = (state._parallel ? ICON_PARALLEL : ICON_SEQUENTIAL) + _toggleLabel(state._parallel ? 'Paralelo' : 'Sequencial');
      parallelBtn.title = state._parallel ? 'Mudar para um por vez' : 'Executar lado a lado';
      renderModelRows();
      uiModule.showToast('Modo: ' + (state._parallel ? 'Paralelo' : 'Sequencial'));
      _updateModeLabel();
      _setModeHint(state._parallel
        ? '<span style="color:#5b8def">Paralelo</span>: todos os modelos respondem ao mesmo tempo, lado a lado.'
        : '<span style="color:#e0a050">Sequencial</span>: os modelos respondem um por vez.');
    });
    ...
    diceBtn.title = 'Aleatório — escolhe modelos aleatoriamente para cada slot';
    diceBtn.innerHTML = ICON_DICE + _toggleLabel('Aleatório');
    diceBtn.addEventListener('click', () => {
      ...
      if (_shuffled) {
        ...
        diceBtn.classList.remove('active');
        renderModelRows();
        uiModule.showToast('Modo: Aleatório desativado');
        _updateModeLabel();
        _setModeHint('<span style="color:var(--red)">Aleatório desativado</span>: escolha os modelos você mesmo.');
        return;
      }
      ...
      renderModelRows();
      uiModule.showToast(state._blindMode ? 'Modo: Aleatório ativado · Cego ativado' : 'Modo: Aleatório ativado');
      _updateModeLabel();
      _setModeHint('<span style="color:var(--red)">Aleatório</span>: modelos aleatórios escolhidos para cada slot (ocultados automaticamente).');
    ...
    saveBtn.title = 'Salvar — mantém as sessões após encerrar a comparação';
    saveBtn.innerHTML = SAVE_ICON + _toggleLabel('Salvar');
    saveBtn.addEventListener('click', () => {
      state._saveOnClose = !state._saveOnClose;
      saveBtn.classList.toggle('active', state._saveOnClose);
      uiModule.showToast('Modo: Salvar ' + (state._saveOnClose ? 'ativado' : 'desativado'));
      _updateModeLabel();
      _setModeHint(state._saveOnClose
        ? '<span style="color:var(--color-save-green)">Salvar</span>: mantém essas sessões depois que você fechar a comparação.'
        : '<span style="color:var(--color-save-green)">Salvar desativado</span>: as sessões são descartadas ao fechar a comparação.');
    });
    ...
    resetBtn.title = 'Redefinir — restaura todos os padrões';
    resetBtn.innerHTML = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/></svg>' + _toggleLabel('Redefinir');
    ...
    desc.textContent = 'Selecione modelos para comparar lado a lado. Envia o mesmo prompt para todos.';
    ...
    modeLabel.innerHTML = 'Modo: <span class="compare-mode-current"></span>';
    ...
    function _updateModeLabel() {
      ...
      if (!cur) return;
      const parts = [];
      if (state._blindMode) parts.push('<span style="color:var(--color-blind-orange)">Cego</span>');
      parts.push(state._parallel
        ? '<span style="color:#5b8def">Paralelo</span>'
        : '<span style="color:#e0a050">Sequencial</span>');
      if (_shuffled) parts.push('<span style="color:var(--red)">Aleatório</span>');
      if (state._saveOnClose) parts.push('<span style="color:var(--color-save-green)">Salvar</span>');
    ...
    typeLabel.innerHTML = 'Tipo: <span class="compare-type-current"></span>';
    ...
    const _modes = [
      ...
      { id: 'agent', label: 'Agente', icon: _ICON_AGENT },
      { id: 'search', label: 'Busca', icon: _ICON_SEARCH },
      { id: 'research', label: 'Pesquisa', icon: _ICON_RESEARCH },
    ];
    _modes.forEach(m => {
    ...
    function setModeTab(mode) {
      ...
      if (mode === 'search' || mode === 'research') {
        ...
        parallelBtn.innerHTML = ICON_SEQUENTIAL + _toggleLabel('Sequencial');
      } else {
        state._parallel = true;
        parallelBtn.classList.add('active');
        parallelBtn.innerHTML = ICON_PARALLEL + _toggleLabel('Paralelo');
    ...
    if (spinnerModule) {
      const _loadSpinner = spinnerModule.create('Carregando modelos', 'right');
      _loadingDiv.appendChild(_loadSpinner.createElement());
      _loadSpinner.start();
    } else {
      _loadingDiv.textContent = 'Carregando modelos\u2026';
    }
    ...
    function renderModelRows() {
      ...
      if (state._compareMode === 'search') {
        ...
        if (!state._cachedProviders) {
          listContainer.innerHTML = '<div style="color:color-mix(in srgb, var(--fg) 40%, transparent);font-size:0.85em;padding:12px 0;text-align:left;">Carregando provedores de busca\u2026</div>';
          fetch(`${state.API_BASE}/api/search/providers`).then(r => r.json()).then(providers => {
            state._cachedProviders = providers;
            renderModelRows();
          }).catch(() => {
            listContainer.innerHTML = '<div style="color:var(--color-error);font-size:0.85em;padding:12px 0;">Falha ao carregar provedores de busca</div>';
          });
          return;
        }
      ...
      if (needsProviders && !state._cachedProviders) {
        listContainer.innerHTML = '<div style="color:color-mix(in srgb, var(--fg) 40%, transparent);font-size:0.85em;padding:12px 0;">Carregando provedores de busca\u2026</div>';
        fetch(`${state.API_BASE}/api/search/providers`).then(r => r.json()).then(providers => {
          state._cachedProviders = providers;
          renderModelRows();
        }).catch(() => {
      ...
      selections.forEach((sel, idx) => {
        ...
        if (needsProviders && researchProviders.length > 0 && !_shuffled) {
          const provSelect = document.createElement('select');
          provSelect.className = 'cmp-form-control cmp-prov-select';
          provSelect.title = 'Provedor de busca';
    ...
    addBtn.textContent = '+ Adicionar Modelo';
    ...
    timeoutLabel.textContent = 'Tempo limite:';
    ...
    timeoutSuffix.textContent = 'segundos';
    ...
    scoreBtn.innerHTML = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:4px;"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg>Placar';
    ...
    startBtn.addEventListener('click', async () => {
      ...
      probeCard.innerHTML = '<div class="compare-probe-title">Verificando modelos...</div>';
      ...
      async function _probeOne(m) {
        if (_isImageModel(m.model)) {
          return { status: 'ok', model: m.model, skipped: true, skipReason: 'Imagem' };
        }
        // Search mode — probe the LLM model normally (don't skip)
        if (state._compareMode === 'search' && !m.model) {
          return { status: 'ok', model: m.model, skipped: true, skipReason: 'Sem modelo' };
        }
      ...
      function _updateRow(idx, result) {
        ...
        if (result.status === 'ok') {
          ...
          status.textContent = result.skipped ? (result.skipReason || 'Ignorado') : (result.latency_ms ? `${result.latency_ms}ms` : 'OK');
          ...
          const errText = (result.error || 'Falhou');
          ...
          const retryLabel = result.error === 'Timeout' ? `Repetir ${Math.round(row._probeTimeout / 1000)}s` : 'Repetir';
      ...
      try {
          ...
          for (let i = 0; i < results.length; i++) {
            if (results[i].status !== 'ok') {
              ...
              if (row) {
                ...
                if (status) status.textContent = 'Trocando...';
              }
              ...
              if (!swapped) {
                ...
                if (row) {
                  ...
                  if (status) { status.textContent = 'Sem substituto'; }
                }
              }
            }
          }
        ...
        if (allOk && (state._compareMode === 'search' || state._compareMode === 'research')) {
          ...
          if (providers.length > 0) {
            ...
            titleEl.textContent = 'Verificando provedores de busca...';
            ...
            provResults.forEach((result, i) => {
              ...
              if (result.status === 'ok') {
                ...
                status.textContent = result.error || 'Falhou'; status.classList.add('fail');
                row.classList.add('fail');
                searchAllOk = false;
              }
            });
        ...
        if (allOk) {
          ...
          probeOverlay.querySelector('.compare-probe-title').textContent = 'Tudo pronto!';
          setTimeout(() => {
          ...
          titleEl.textContent = failedNames.length <= 2
            ? failedNames.join(' e ') + ' falharam'
            : `${failCount} modelos falharam`;
          const btnRow = document.createElement('div');
          btnRow.style.cssText = 'display:flex;gap:8px;justify-content:center;margin-top:12px;';
          const goBackBtn = document.createElement('button');
          goBackBtn.innerHTML = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:3px;"><polyline points="15 18 9 12 15 6"/></svg>Voltar';
          ...
          startAnywayBtn.textContent = 'Iniciar Mesmo Assim';
    ...
    fetchModels().then(fetched => {
      ...
      _modelsLoaded = true;
      if (models.length < 1) {
        listContainer.innerHTML = '<div style="color:var(--color-error);font-size:0.85em;padding:12px 0;text-align:center;">Nenhum modelo disponível</div>';
        return;
      }
      ...
      listContainer.innerHTML = '<div style="color:var(--color-error);font-size:0.85em;padding:12px 0;text-align:center;">Falha ao carregar modelos</div>';
    });
  });
}
