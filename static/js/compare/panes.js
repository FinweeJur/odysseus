...
function stopPane(paneIdx) {
  ...
  if (hist) {
    ...
    if (body && !body.textContent.trim()) {
      body.innerHTML = '<span style="opacity:0.4;font-style:italic;">Interrompido</span>';
    }
  }
}
...
async function rerollPane(paneIdx, overrideTimeout) {
  ...
  userMsg.innerHTML = '<div class="role">Você</div><div class="body">' + escapeHtml(firstUserText) + '</div>';
  ...
  if (state._compareMode === 'search') {
    ...
    try {
      ...
      aiBody.innerHTML = '<div style="color:var(--color-error);font-size:0.85em;">Erro: ' + escapeHtml(err.message) + '</div>';
    }
    state._abortControllers[paneIdx] = null;
    hist.scrollTop = hist.scrollHeight;
    return;
  ...
  const aiBody = aiMsg.querySelector('.body');
  if (spinnerModule) {
    const label = overrideTimeout ? 'Tentando novamente (' + overrideTimeout + 's)…' : 'Sorteando de novo…';
...
function _autoPreviewHtml(paneIdx, accumulated) {
  ...
  previewBtn.title = 'Executar prévia';
}
...
async function copyPaneResponse(paneIdx) {
  ...
  if (uiModule) uiModule.showToast(lastAi._imageData ? 'Prompt copiado!' : 'Copiado!');
}
...
async function _createAndAppendPane(m) {
  ...
  pane.innerHTML =
    '<div class="pane-header">' +
      ...
      '<div class="pane-actions">' +
        ...
        '<button class="pane-action-btn" data-action="reroll" data-pane="' + i + '" title="Sortear de novo">' + ICON_REROLL + '</button>' +
    ...
    '<div class="pane-vote-footer">' +
      '<button class="pane-vote-btn" data-pane="' + i + '" type="button" disabled style="opacity:0.4;">' +
        ...
        '<span class="pane-vote-label">Votar ' + escapeHtml(label) + '</span>' +
      '</button>' +
    '</div>';
  ...
  if (headerSpan) {
    const modeLabel = ({ search: ' provedores de busca', agent: ' agentes', research: ' modelos de pesquisa' }[state._compareMode] || ' modelos');
    headerSpan.textContent = 'Comparando' + modeLabel +
      (state._blindMode ? ' (an\u00f4nimo)' : '') + ' \u00b7 timeout de ' + state._timeout + 's';
  }
  ...
  if (state._blindMode && n > 2) {
    ...
    if (shuffleBtn) {
      ...
      bubble.textContent = 'Embaralhar modelos?';
...
function _removePane(paneIdx) {
  ...
  const n = state._selectedModels.length;
  for (let i = 0; i < n; i++) {
    const label = state._blindMode ? 'Modelo ' + _slotChar(i) : state._selectedModels[i].name;
    const pane = document.createElement('div');
    pane.className = 'compare-pane';
    pane.dataset.pane = String(i);
    pane.innerHTML =
      '<div class="pane-header">' +
        ...
        '<div class="pane-actions">' +
          ...
          '<button class="pane-action-btn pane-preview-btn" data-action="preview" data-pane="' + i + '" id="cmp-preview-' + i + '" title="Executar prévia" style="display:none;">' + ICON_PLAY + '</button>' +
          '<button class="pane-action-btn pane-needs-response" data-action="reroll" data-pane="' + i + '" title="Sortear de novo" style="display:none;">' + ICON_REROLL + '</button>' +
  ...
  filtered.forEach(m => {
    ...
    item.addEventListener('click', async (e) => {
      ...
      try {
        ...
        console.error('Falha ao criar sessão para o modelo trocado:', err);
      }
...
function shufflePanePositions() {
  ...
  setTimeout(() => {
    for (let i = 0; i < n; i++) {
      ...
      if (titleEl) {
        const lbl = state._blindMode ? 'Modelo ' + _slotChar(i) : state._selectedModels[i].name;
