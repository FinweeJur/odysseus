...
export function showScoreboard() {
  ...
  title.innerHTML = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px;margin-right:6px;"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg>Placar';
  ...
  const modeLabels = { chat: 'Chat', agent: 'Agente', search: 'Busca', research: 'Pesquisa' };
  ...
  function renderScoreTable() {
    ...
    if (sorted.length === 0) {
      ...
      empty.textContent = 'Nenhum voto de ' + activeMode + ' ainda. Execute uma comparação e vote!';
      wrap.appendChild(empty);
    } else {
      const table = document.createElement('table');
      table.className = 'scoreboard-table';
      const thead = document.createElement('thead');
      thead.innerHTML = '<tr><th>Modelo</th><th>Vit%</th><th>V</th><th>D</th><th>E</th><th>Jogos</th><th>$/1k</th></tr>';
      ...
      for (const [name, s] of sorted) {
        ...
        tr.innerHTML =
          ...
          '<td>' + s.games + '</td>' +
          '<td style="color:var(--color-success, #4caf50);" title="Custo médio estimado por 1.000 respostas">' + costStr + '</td>';
    ...
    total.textContent = filtered.length + ' voto' + (filtered.length !== 1 ? 's' : '') + ' registrado' + (filtered.length !== 1 ? 's' : '');
  ...
  clearBtn.addEventListener('click', () => {
    ...
    confirmLabel.textContent = 'Limpar todo o histórico de votos?';
    const yesBtn = document.createElement('button');
    yesBtn.textContent = 'Limpar';
