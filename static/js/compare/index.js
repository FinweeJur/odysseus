...
async function deactivate(teardown) {
  ...
  // Restaurar placeholder da textarea principal
...
async function _buildCompareUI() {
  if (state._selectedModels.length < 1) {
    if (uiModule) uiModule.showError('Selecione ao menos 1 modelo');
    return;
  }
  ...
  if (state._compareMode !== 'search') {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (!res.ok) throw new Error('Falha ao criar sessão para ' + modelShorts[i]);
  ...
  const _modeLabel = ({ search: ' provedores de busca', agent: ' agentes', research: ' modelos de pesquisa' }[state._compareMode] || ' modelos');
  headerLabel.textContent = 'Comparando' + _modeLabel + (state._blindMode ? ' (anônimo)' : '') + ' · timeout de ' + state._timeout + 's';
  ...
  checkBtn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M20 6L9 17l-5-5"/></svg><span style="font-size:11px;margin-left:3px;">Testar</span>';
  checkBtn.title = 'Testar modelos não verificados com uma requisição pequena';
  ...
  exportBtn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg><span style="font-size:11px;margin-left:3px;">Exportar</span>';
  exportBtn.title = 'Opções de exportação';
  ...
  shuffleBtn.innerHTML = ICON_DICE + '<span style="font-size:11px;margin-left:3px;">Embaralhar</span>';
  shuffleBtn.title = 'Embaralhar posições dos painéis';
  ...
  addBtn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg><span style="font-size:11px;margin-left:3px;">Adicionar</span>';
  addBtn.title = 'Adicionar painel de modelo';
  ...
  closeBtn.title = 'Fechar modo de comparação';
    ...
    pane.innerHTML =
      '<div class="pane-header">' +
        ...
        '<div class="pane-actions">' +
          '<button class="pane-action-btn pane-stop-btn" data-action="stop" data-pane="' + i + '" title="Parar" style="display:none;"><svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="6" width="12" height="12" rx="2"/></svg></button>' +
          '<button class="pane-action-btn pane-preview-btn" data-action="preview" data-pane="' + i + '" id="cmp-preview-' + i + '" title="Executar prévia" style="display:none;">' + ICON_PLAY + '</button>' +
          '<button class="pane-action-btn" data-action="reroll" data-pane="' + i + '" title="Gerar novamente">' + ICON_REROLL + '</button>' +
          '<button class="pane-action-btn" data-action="copy" data-pane="' + i + '" title="Copiar">' + ICON_COPY + '</button>' +
          '<button class="pane-action-btn" data-action="expand" data-pane="' + i + '" title="Expandir">' + ICON_EXPAND + '</button>' +
          '<button class="pane-action-btn pane-close-btn" data-action="close" data-pane="' + i + '" title="Remover painel">' + ICON_CLOSE + '</button>' +
        '</div>' +
      '</div>' +
      ...
      '<div class="pane-vote-footer">' +
        '<button class="pane-vote-btn" data-pane="' + i + '" type="button" disabled style="opacity:0.4;">' +
          ...
          '<span class="pane-vote-label">Votar em ' + escapeHtml(label) + '</span>' +
        '</button>' +
      '</div>';
  ...
  const msgTA = document.getElementById('message');
  if (msgTA) {
    msgTA.placeholder = 'Digite o prompt para todos os modelos…';
...
function _setSendBtn(mode) {
  ...
  if (mode === 'stop') {
    ...
    btn.title = 'Parar todos os modelos';
    ...
    btn.style.color = '';
    btn.title = 'Enviar para todos os modelos';
...
async function _executeCompare(message) {
  ...
  for (let i = 0; i < state._selectedModels.length; i++) {
    ...
    if (titleEl) {
      const label = state._blindMode
        ? 'Modelo ' + _slotChar(i)
        : ((state._selectedModels[i] && state._selectedModels[i].name) || 'Modelo ' + _slotChar(i));
  ...
  if (state._compareMode === 'search') {
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        userMsg.innerHTML = '<div class="role">Você</div><div class="body"></div>';
        userMsg.querySelector('.body').textContent = message;
        hist.appendChild(userMsg);

        const aiMsg = document.createElement('div');
        aiMsg.className = 'msg msg-ai';
        aiMsg.innerHTML = '<div class="role">Busca</div><div class="body"></div>';
        const aiBody = aiMsg.querySelector('.body');
        if (spinnerModule) {
          const spinner = spinnerModule.create('Buscando…', 'right');
      ...
      // Executar buscas — paralelo ou sequencial baseado em _parallel
      ...
      async function _searchOne(m, i) {
        ...
        try {
          ...
          return { idx: i, data: { results: [], error: err.name === 'AbortError' ? 'Interrompido' : err.message } };
        }
      }
      ...
      if (state._parallel) {
        ...
        // Sequencial — executar um por vez, deixar outros painéis escurecidos
        results = [];
        ...
        for (let i = 0; i < state._selectedModels.length; i++) {
          ...
          // Renderizar este resultado imediatamente
          ...
          if (hist) {
            ...
            if (aiMsg) {
              ...
              aiBody.innerHTML = '';
              if (data.error) {
                aiBody.innerHTML = '<div style="color:var(--color-error);font-size:0.85em;">Erro: ' + escapeHtml(data.error) + '</div>';
              } else if (!data.results || data.results.length === 0) {
                aiBody.innerHTML = '<div style="color:color-mix(in srgb, var(--fg) 50%, transparent);font-size:0.85em;font-style:italic;">Nenhum resultado encontrado</div>';
              } else {
              ...
              const parts = [];
              if (data.results) parts.push(data.results.length + ' resultados');
          ...
          // Sequencial: executar síntese para este painel antes de passar para o próximo
          ...
          if (!data.error && data.results && data.results.length > 0) {
            ...
            if (modelToUse) {
              ...
              if (seqHist) {
                ...
                synthMsg.innerHTML = '<div class="role">Análise</div><div class="body"></div>';
                const synthBody = synthMsg.querySelector('.body');
                let spinner = null;
                if (spinnerModule) { spinner = spinnerModule.create('Analisando...', 'right'); synthBody.appendChild(spinner.createElement()); spinner.start(); }
                seqHist.appendChild(synthMsg);
                seqHist.scrollTop = seqHist.scrollHeight;
                const resultsText = data.results.map((r, ri) => `[${ri + 1}] ${r.title}\n${r.snippet || ''}\nURL: ${r.url}`).join('\n\n');
                const synthPrompt = `Analise estes resultados de busca para a consulta "${message}". Resuma os principais achados, observe qualquer consenso ou informação conflitante e forneça uma síntese breve.\n\nResultados da busca:\n${resultsText}`;
      ...
      // ── Síntese: enviar resultados para LLM para análise (respeita _parallel) ──
      if (state._searchSynthModels) {
        // Construir lista de tarefas de síntese
        const synthTasks = [];
        for (let i = 0; i < results.length; i++) {
          const { idx, data } = results[i];
          // Pular painéis já sintetizados em modo sequencial
          ...
          // Adicionar mensagem de síntese com spinner
          const synthMsg = document.createElement('div');
          synthMsg.className = 'msg msg-ai';
          synthMsg.innerHTML = '<div class="role">Análise</div><div class="body"></div>';
          const synthBody = synthMsg.querySelector('.body');
          let spinner = null;
          if (spinnerModule) {
            spinner = spinnerModule.create('Analisando...', 'right');
            synthBody.appendChild(spinner.createElement());
            spinner.start();
          }
          hist.appendChild(synthMsg);
          // Auto-scroll para mostrar mensagem de Análise
          hist.scrollTop = hist.scrollHeight;
        ...
        // Executar streams de síntese (paralelo ou sequencial baseado em _parallel)
      ...
      if (uiModule) uiModule.showError('Falha na comparação de busca: ' + err.message);
    } finally {
      state._streaming = false;
      _setSendBtn('send');
    }
  ...
  try {
    ...
    if (state._parallel) {
      ...
      for (let i = 0; i < state._paneSessionIds.length; i++) {
        ...
        if (aiElements[i] && aiElements[i]._spinner) {
          aiElements[i]._spinner.updateLabel('Processando...');
        }
    ...
    if (uiModule) uiModule.showError('Falha na comparação: ' + err.message);
  } finally {
    state._streaming = false;
    _setSendBtn('send');
...
function _buildComparisonMarkdown() {
  ...
  const prompt = state._lastPrompt || '(nenhum prompt ainda — execute uma comparação primeiro)';
  const expected = state._expectedAnswer || '';
  const date = new Date().toISOString().slice(0, 19).replace('T', ' ');
  let md = '# Comparação\n\n';
  md += '**Quando:** ' + date + '\n';
  md += '**Tipo:** ' + (state._compareMode || 'chat') + (state._blindMode ? ' (anônimo)' : '') + '\n';
  md += '**Prompt:**\n\n```\n' + prompt + '\n```\n\n';
  if (expected) md += '**Resposta esperada:** `' + expected + '`\n\n';
...
function _toggleExportMenu(btn) {
  ...
  const opts = [
    ...
    { label: 'Baixar .md',           fn: () => _exportDownloadMarkdown() },
    { label: 'Imprimir / Salvar PDF', fn: () => _exportPrint() },
  ];
  for (const o of opts) {
...
async function _exportCopyMarkdown(_btn) {
  ...
  try {
    ...
    try { window.uiModule?.showToast?.('Comparação copiada para a área de transferência'); } catch {}
  } catch (e) {
    try { window.uiModule?.showToast?.('Falha ao copiar'); } catch {}
  }
}
...
function _exportPrint() {
  ...
  const html = '<!doctype html><meta charset="utf-8"><title>Exportar comparação</title>' +
...
async function _exportComparison(btn) {
  ...
  try {
    ...
    if (btn) {
      btn.innerHTML = '<span style="font-size:11px;color:var(--color-error);">Falhou</span>';
...
function _setupEvalPicker() {
  ...
  btn.title = 'Inserir um prompt de avaliação';
  btn.innerHTML =
    '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>'
    + '<span class="cmp-eval-label">Prompts de avaliação</span>'
  ...
  function _renderItems() {
    ...
    if (!list.length) {
      menu.innerHTML = '<div class="cmp-eval-empty">Sem prompts para este tipo</div>';
      return;
    }
    ...
    for (const sub of order) {
      ...
      for (const p of groups[sub]) {
        ...
        const checkMark = p.answer ? '<span class="cmp-eval-item-tick" title="Tem resposta esperada">✓</span>' : '';
  ...
  hintChip.innerHTML =
    '<span class="cmp-eval-expected-label">Esperado:</span>'
    + ' <strong class="cmp-eval-expected-value"></strong>'
    + ' <button type="button" class="cmp-eval-expected-close" title="Descartar">×</button>';
  ...
  // Ocultar picker quando textarea tem texto (só útil no início)
  // Reaparece quando limpo. O chip de resposta esperada permanece nos envios
  // — limpá-lo a cada tick de textarea vazia apagava state._expectedAnswer
  // antes da classificação ler, então badges ✓/✗ nunca apareciam. O chip
  // só é limpo por seu botão descartar (ou quando usuário escolhe novo eval).
  const ta = document.getElementById('message');
  ...
  // Guardar limpeza para cleanupResults() desanexar listener doc e
  // restaurar model-picker quando compare desativa.
  wrap._cleanup = () => {
...
/** Remover elementos de UI de comparação e restaurar vista original. */
function cleanupResults() {
  // Remover todos elementos de comparação
  ...
  // Remover overlays de compare/probe remanescentes
  ...
  // Recarregar página para restaurar limpo todo estado de UI
...
/** Editor de pool de embaralhamento — deixa usuários excluir modelos quebrados. */
async function showShufflePoolEditor() {
  let models;
  try { models = await fetchModels(); } catch (e) {
    if (uiModule) uiModule.showError('Falha ao carregar modelos');
    return;
  }
  ...
  header.innerHTML = '<h4>Pool de Embaralhamento</h4>';
  const closeBtn = document.createElement('button');
  ...
  desc.textContent = 'Desmarque modelos para excluí-los do embaralhamento aleatório. Eles ainda podem ser escolhidos manualmente.';
  ...
  Object.entries(groups).forEach(([type, items]) => {
    ...
    heading.textContent = type === 'chat' ? 'Modelos de Chat' : 'Modelos de Imagem';
