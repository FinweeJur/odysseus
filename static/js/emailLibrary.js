...
function _recipientChipHtml(full, label, extraClass = '') {
  ...
  return `<span class="${cls}" data-full="${_esc(fullText || labelText)}" data-email="${_esc(addr)}" title="Clique para ver detalhes"><span class="recipient-chip-label">${_esc(labelText)}</span><button type="button" class="recipient-chip-copy" title="Copiar e-mail" aria-label="Copiar e-mail" hidden>${_COPY_EMAIL_ICON}</button></span>`;
}
...
function _syncUnreadTabBadge(count) {
  const label = count > 999 ? '999+ não lidos' : `${count} não lidos`;
  document.querySelectorAll('.minimized-dock-chip[data-modal-id="email-lib-modal"]').forEach(chip => {
    if (count > 0) {
      ...
      chip.title = `Abrir ${label}`;
    } else {
      delete chip.dataset.emailUnreadLabel;
      chip.title = 'Restaurar E-mail';
    }
  });
}
...
function _renderAccountsLoading() {
  ...
  try {
    ...
    label.textContent = 'Contas';
    strip.appendChild(wp.element);
    strip.appendChild(label);
  } catch (_) {
    strip.textContent = 'Contas…';
  }
}
...
async function _deleteEmailAndAdvance(em, card, opts = {}) {
  ...
  if (opts.confirm !== false) {
    const subject = em.subject || '(sem assunto)';
    const ok = await styledConfirm(`Excluir "${subject}"?`, { confirmText: 'Excluir', cancelText: 'Cancelar', danger: true });
    if (!ok) return;
  }
  ...
  try {
    ...
    showToast('Falha ao excluir e-mail');
    return;
  }
...
function _emailSetupHintHtml() {
  return '<div style="margin-top:6px;opacity:0.72;font-size:11px;">' +
    'Configurar em: <a href="#" data-open-settings="integrations" style="color:var(--accent,var(--red));text-decoration:underline;">Configurações &rsaquo; Integrações</a>' +
    '</div>';
}
...
export function openEmailLibrary(opts = {}) {
    ...
    <div class="modal-content doclib-modal-content" style="width:min(720px, 92vw);max-height:85vh;background:var(--bg);">
      <div class="modal-header">
        <h4>
          ...
          Email
          <span id="email-lib-unread-badge" class="email-lib-unread-badge" role="button" tabindex="0" title="Mostrar e-mails não lidos" style="display:none"></span>
      ...
      <div class="modal-body" style="display:flex;flex-direction:column;gap:10px;overflow:hidden;">
        <div class="admin-card" style="flex:1;flex-direction:column;display:flex;overflow:hidden;">
          ...
          <div class="memory-toolbar">
            <div class="memory-category-filters">
              ...
              <select class="memory-sort-select" id="email-lib-filter" style="flex:1;min-width:0;">
                ...
                <option value="pending_30d">Pendentes · 30d</option>
                <option value="stale_30d">Antigos · &gt;30d</option>
                <optgroup label="Tags">
                  <option value="tag:urgent">Urgente</option>
                  <option value="tag:reply-soon">Responder em breve</option>
              ...
              <button class="memory-toolbar-btn email-filter-select-btn" id="email-lib-select-btn">Selecionar</button>
              <button class="memory-toolbar-btn email-filter-refresh-btn" id="email-lib-refresh-btn" title="Atualizar">
              ...
              <button class="memory-toolbar-btn email-reminders-clear-btn hidden" id="email-reminders-clear-btn" title="Excluir permanentemente e-mails de lembrete do Cutia">
            ...
            <div class="email-search-wrap" style="position:relative;flex:1;min-width:140px;">
              <input type="text" id="email-lib-search" placeholder="Buscar e-mails\u2026" class="memory-search-input" style="width:100%;padding-right:96px;" />
              <button class="memory-toolbar-btn email-undone-toggle email-undone-toggle-inline" id="email-undone-btn" title="Mostrar apenas e-mails n\u00e3o marcados como conclu\u00eddos">
              ...
              <button class="memory-toolbar-btn email-reminder-toggle-inline hidden" id="email-reminder-btn" title="Mostrar e-mails de lembrete do Cutia">
              ...
              <button class="memory-toolbar-btn email-attach-toggle email-attach-toggle-inline" id="email-attach-btn" title="Mostrar apenas e-mails com anexos">
          ...
          <div id="email-lib-bulk" class="memory-bulk-bar hidden" style="margin-bottom:5px;">
            ...
            <span id="email-lib-selected-count" style="position:relative;top:1px;">0 selecionados</span>
          ...
          <button class="email-lib-fab" id="email-lib-fab" type="button" aria-label="Novo e-mail">
            ...
            <span class="email-lib-fab-label">Novo</span>
          </button>
        </div>
      </div>
    </div>
  ...
  document.getElementById('email-reminders-clear-btn')?.addEventListener('click', async () => {
    const ok = await styledConfirm('Excluir permanentemente todos os e-mails de lembrete do Cutia?', {
      confirmText: 'Excluir',
      cancelText: 'Cancelar',
      danger: true,
    });
    ...
    try {
      ...
      showToast(`${data.deleted || 0} e-mail${(data.deleted || 0) === 1 ? '' : 's'} de lembrete excluído${(data.deleted || 0) === 1 ? '' : 's'}`);
      if ((data.deleted || 0) > 0) {
      ...
      console.error(err);
      showToast('Falha ao limpar e-mails de lembrete');
    }
  });
  ...
  document.getElementById('email-lib-bulk-actions').addEventListener('click', (e) => {
    ...
    if (state._selectedUids.size === 0) {
      showToast('Selecione e-mails primeiro');
      return;
    }
...
async function _doSearch() {
  ...
  try {
    ...
    sp.destroy();
    grid.innerHTML = '<div class="email-loading">Busca falhou</div>';
  }
}
...
function _renderEmailLoading(grid) {
  ...
  label.textContent = 'Carregando e-mails';
  ...
  try {
    ...
    if (state._libFilter === 'unread') {
      ...
      try {
        ...
        badge.textContent = `${t} tudo`;
        badge.title = 'Mostrar todos os e-mails';
        badge.style.display = '';
      } catch (_) {
        badge.textContent = 'Mostrar tudo';
        badge.title = 'Mostrar todos os e-mails';
        badge.style.display = '';
      }
    } else if (n > 0) {
      badge.textContent = n > 999 ? '999+ não lidos' : `${n} não lidos`;
      badge.title = 'Mostrar e-mails não lidos';
      badge.style.display = '';
    } else {
...
async function _loadEmails({ force = false, useCache = true } = {}) {
  ...
  if (cached) {
    ...
    _renderGrid();
    const stats = document.getElementById('email-lib-stats');
    if (stats) stats.textContent = `${state._libTotal} e-mails`;
  } else {
  ...
  try {
    ...
    if (!cached) {
      const msg = e && e.message ? `Falha ao carregar: ${e.message}` : 'Falha ao carregar';
  ...
  for (const it of items) {
    ...
    content.innerHTML = `
      ...
      <div style="font-size:10px;opacity:0.7;margin-top:2px;">
        Para: ${_esc(toDisplay)} · Envia ${_esc(dateStr)}
      </div>
    ...
    cancelBtn.title = 'Cancelar envio agendado';
    ...
    cancelBtn.addEventListener('click', async (e) => {
      ...
      const ok = await styledConfirm(`Cancelar e-mail agendado "${subject}"?`, { confirmText: 'Cancelar envio', cancelText: 'Manter', danger: true });
      if (!ok) return;
      try {
...
function _createCard(em) {
  ...
  navArrows.innerHTML = `
    <button type="button" class="email-card-nav-btn" data-nav-dir="-1" title="E-mail anterior"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg></button>
    <button type="button" class="email-card-nav-btn" data-nav-dir="1" title="Próximo e-mail"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg></button>
  `;
...
async function _toggleCardPreview(card, em) {
  ...
  try {
      ...
      <div class="email-reader-header">
        ...
        <div class="email-reader-actions">
          <div class="email-reader-actions-row email-reader-actions-row-primary">
            ...
            ${_hasMultipleRecipients(data) ? `<button class="memory-toolbar-btn reader-icon-btn" data-act="reply-all" title="Responder a todos"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="7 17 2 12 7 7"/><polyline points="12 17 7 12 12 7"/><path d="M22 18v-2a4 4 0 0 0-4-4H7"/></svg><span class="reader-btn-label">Resp. a todos</span></button>` : ''}
            <button class="memory-toolbar-btn reader-icon-btn" data-act="forward" title="Encaminhar"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 17 20 12 15 7"/><path d="M4 18v-2a4 4 0 0 1 4-4h12"/></svg><span class="reader-btn-label">Encaminhar</span></button>
          </div>
          <div class="email-reader-actions-row email-reader-actions-row-secondary">
            <button class="memory-toolbar-btn reader-icon-btn" data-act="ai-reply" title="${data.cached_ai_reply ? 'Resposta com IA (rascunho em cache pronto)' : 'Resposta com IA (sugerir um rascunho)'}">${_aiReplyIcon(data)}<span class="reader-btn-label">Resposta IA</span></button>
            <button class="memory-toolbar-btn reader-icon-btn" data-act="summarize" title="Resumir">${_summaryIcon(data)}<span class="reader-btn-label">Resumo</span></button>
            <button class="memory-toolbar-btn reader-icon-btn" data-act="from-sender" title="Buscar texto nesta conversa"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="7"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg><span class="reader-btn-label">Buscar</span></button>
            <div class="email-reader-more-wrap" style="position:relative">
              <button class="memory-toolbar-btn reader-icon-btn" data-act="more" title="Mais ações"><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="5" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="12" cy="19" r="2"/></svg><span class="reader-btn-label">Mais</span></button>
            </div>
          </div>
        </div>
      </div>
    ...
    reader.innerHTML = `<div style="padding:20px;color:var(--red,#e55)">Falha ao carregar e-mail</div>`;
  }
}
    ...
    openBtn.addEventListener('click', async (ev) => {
      ...
      try {
        ...
        if (!res.ok || !json.doc_id) {
          ...
          try { const { showError } = await import('./ui.js'); showError(`Não foi possível abrir ${name}: ${msg}`); } catch (_) { alert(`Não foi possível abrir ${name}: ${msg}`); }
          return;
        }
        try {
    ...
    reader.innerHTML = `
      <div class="email-reader-header">
        ...
        <div class="email-reader-actions">
          ...
          <div class="email-reader-actions-row email-reader-actions-row-secondary">
            <button class="memory-toolbar-btn reader-icon-btn" data-act="ai-reply" title="${data.cached_ai_reply ? 'Resposta com IA (rascunho em cache pronto)' : 'Resposta com IA'}">${_aiReplyIcon(data)}<span class="reader-btn-label">Resposta IA</span></button>
            <button class="memory-toolbar-btn reader-icon-btn" data-act="summarize" title="Resumir">${_summaryIcon(data)}<span class="reader-btn-label">Resumo</span></button>
            <button class="memory-toolbar-btn reader-icon-btn" data-act="from-sender" title="Buscar texto nesta conversa"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="7"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg><span class="reader-btn-label">Buscar</span></button>
            <div class="email-reader-more-wrap" style="position:relative">
              <button class="memory-toolbar-btn reader-icon-btn" data-act="more" title="Mais ações"><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="5" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="12" cy="19" r="2"/></svg><span class="reader-btn-label">Mais</span></button>
    ...
    },
    {
      ...
      action: async () => {
        ...
        const ok = await styledConfirm(
          `Excluir permanentemente "${subject}"? Isto não pode ser desfeito.`,
...
function _showCardMenu(em, anchor) {
  ...
  actions.push(
    { label: 'Excluir', icon: _delIcon, danger: true, action: async () => {
      const subject = em.subject || '(sem assunto)';
      const ok = await styledConfirm(`Excluir "${subject}"?`, { confirmText: 'Excluir', cancelText: 'Cancelar', danger: true });
      if (!ok) return;
      await fetch(`${API_BASE}/api/email/delete/${em.uid}?folder=${encodeURIComponent(state._libFolder)}${_acct()}`, { method: 'DELETE' });
...
async function _bulkAction(action) {
  ...
  if (action === 'delete') {
    const ok = await styledConfirm(
      `Excluir ${uids.length} e-mail${uids.length === 1 ? '' : 's'} selecionado${uids.length === 1 ? '' : 's'}?`,
      { confirmText: 'Excluir', cancelText: 'Cancelar', danger: true },
    );
    if (!ok) return;
  }
