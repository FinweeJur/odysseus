...
function _preloadVisibleMarkdown() {
  document.querySelectorAll('#skills-list .skill-card[data-skill-name]').forEach(card => {
    ...
    const apply = (md) => { if (pre) pre.textContent = md || '(vazio)'; card._mdLoaded = true; card._md = md || ''; };
...
function _statusPill(sk) {
  ...
  if (s === 'published') return '<span class="memory-cat-badge skill-status-pill" data-status="published" style="background:color-mix(in srgb, var(--accent, #4ade80) 30%, transparent)">publicada</span>';
  if (s === 'draft')     return '<span class="memory-cat-badge skill-status-pill" data-status="draft" style="background:color-mix(in srgb, var(--fg) 14%, transparent)">rascunho</span>';
...
function _sourcePill(sk) {
  ...
  return `<span class="memory-cat-badge" title="Criado por escalonamento do professor: ${esc(teacher)}" style="background:color-mix(in srgb, var(--color-warning, #f0ad4e) 22%, transparent);">criado pelo professor</span>`;
}
...
function _auditModelPills(sk) {
  ...
  let html = '';
  if (worker) {
    html += `<span class="memory-cat-badge skill-model-pill skill-model-student" title="Última auditoria pelo modelo de auditoria padrão: ${esc(worker)}">auditoria</span>`;
  }
  if (sk.audit_by_teacher || teacher) {
    const title = teacher
      ? `O professor reescreveu esta skill; o modelo de auditoria aprovou após a reescrita. Professor: ${teacher}`
      : 'O professor reescreveu esta skill; o modelo de auditoria aprovou após a reescrita.';
    html += `<span class="memory-cat-badge skill-model-pill skill-model-teacher" title="${esc(title)}">corrigido pelo professor</span>`;
  }
  return html;
}
...
function _necessityPill(sk) {
  ...
  const label = kind === 'duplicate' ? (sk._duplicateGroup ? `duplicada #${sk._duplicateGroup}` : 'duplicada')
    : kind === 'trivial' ? 'genérica'
    : 'possivelmente irrelevante';
  const group = sk._duplicateNames || [];
  const why = sk._duplicateGroup
    ? `Grupo de duplicadas #${sk._duplicateGroup}. Recomendado manter: ${sk._duplicateKeepName}. Grupo: ${group.join(', ')}`
    : (nec.reason || 'Pode não valer a pena manter') + (dup.length ? ' | sobreposições: ' + dup.join(', ') : '');
...
function _duplicatePriorityPill(sk) {
  ...
  if (sk._duplicateKeep) {
    return `<span class="memory-cat-badge skill-duplicate-keep" title="Melhor candidata duplicada por status de publicação, usos, confiança e especificidade">recomendada</span>`;
  }
  return `<span class="memory-cat-badge skill-duplicate-lower" title="Duplicada de prioridade menor. Manter sugerida: ${esc(sk._duplicateKeepName || '')}">prioridade menor</span>`;
}
...
function _auditMarks(sk) {
  ...
  if (sk.audit_verdict === 'pass') {
    html += `<span class="skill-verified" title="Passou em um teste automatizado"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></span>`;
  }
  if (sk.audit_by_teacher) {
    const teacher = sk.audit_teacher_model ? `: ${sk.audit_teacher_model}` : '';
    html += `<span class="skill-teachermark" title="O professor reescreveu esta skill; o modelo de auditoria aprovou após a reescrita${esc(teacher)}"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 10L12 5 2 10l10 5 10-5z"/><path d="M6 12v5c0 1 3 2 6 2s6-1 6-2v-5"/></svg></span>`;
  }
  return html;
}
...
function _openSkillMenu(btn, card, sk, name, isPublished) {
  ...
  if (isPublished) mk(_ICON.unpublish, 'Despublicar', {}, () => _setSkillStatus(name, 'draft'));
  else mk(_ICON.approve, 'Publicar', {}, () => _setSkillStatus(name, 'published'));
  mk(_ICON.edit, 'Editar', {}, async () => {
    if (!card.classList.contains('doclib-card-expanded')) await _expandSkillCard(card, name);
    _toggleSkillEdit(card, name);
  });
  mk(_ICON.test, 'Testar', {}, () => _testSkill(card, name));
  // Audit kicks off the bulk audit-all loop (test → judge → fix → retry → demote).
  // Starts at the top of the list and walks down.
  mk(_ICON.test, 'Auditar', {}, () => _auditAllSkills());
  mk(_ICON.del, 'Excluir', { danger: true }, () => _deleteSkill(name, card));
  ...
  selItem.innerHTML = '<span style="display:inline-flex;width:14px;height:14px;align-items:center;justify-content:center;"><span style="font-size:16px;line-height:1;">●</span></span><span>Selecionar</span>';
  ...
  cancelItem.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg><span>Cancelar</span>';
...
function _buildBuiltinCards() {
  return builtinSkills.map(b => {
    ...
    header.innerHTML = `
      ...
      <div style="flex:1;min-width:0;overflow:hidden;">
        <div class="doclib-card-title" style="display:flex;align-items:center;gap:6px;min-width:0;">
          ...
          <span class="memory-cat-badge" style="background:color-mix(in srgb, var(--fg) 14%, transparent)">nativo</span>
          ${b.is_overridden ? '<span class="memory-cat-badge" title="Você editou esta capacidade nativa" style="background:color-mix(in srgb, var(--color-warning, #f0ad4e) 30%, transparent);">editado</span>' : ''}
        </div>
    ...
    warn.innerHTML = '⚠ Esta é uma capacidade nativa. Editar altera como o assistente é instruído a usar essa ferramenta nativa — pode quebrar ou alterar o comportamento principal. Use Reverter para restaurar o padrão original.';
    ...
    revertBtn.innerHTML = '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:3px;"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/></svg>Reverter';
    revertBtn.title = 'Restaurar as instruções originais';
...
async function _expandBuiltinCard(card, name) {
  ...
  if (pre && !card._loaded) {
    ...
    try {
      ...
      pre.textContent = 'Falha ao carregar.';
    }
  }
}
...
async function _saveBuiltinEdit(card, name) {
  ...
  try {
    ...
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    uiModule.showToast('Capacidade nativa atualizada');
    builtinSkills = [];  // force reload of built-in list (refreshes "edited" badge)
    await loadSkills();
  } catch (e) { uiModule.showError('Falha ao salvar: ' + e.message); }
}

async function _revertBuiltin(name) {
  if (!(await uiModule.styledConfirm(`Reverter "${name}" para as instruções nativas originais?`, { confirmText: 'Reverter', danger: true }))) return;
  try {
    const res = await fetch(`${API}/api/skills/builtin/${encodeURIComponent(name)}`, { method: 'DELETE' });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    uiModule.showToast('Restaurado ao padrão');
    builtinSkills = [];
    await loadSkills();
  } catch (e) { uiModule.showError('Falha ao reverter: ' + e.message); }
}
...
function renderSkillsList() {
  ...
  if (!sorted.length && !showBuiltin) {
    container.innerHTML = `<div style="text-align:center;opacity:0.4;padding:24px 0;font-size:11px;">${loaded ? 'Nenhuma skill ainda, use o agente para extraí-las automaticamente.' : 'Carregando…'}</div>`;
    return;
  }
  ...
  for (const sk of sorted) {
    ...
    header.innerHTML = `
      ...
      <div class="skill-card-right">
        ...
        <span class="skill-chevron-up" title="Recolher"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="18 15 12 9 6 15"/></svg></span>
        <button class="skill-kebab-btn" title="Ações" aria-label="Ações"><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="5" r="1.6"/><circle cx="12" cy="12" r="1.6"/><circle cx="12" cy="19" r="1.6"/></svg></button>
      </div>
    `;
    card.appendChild(header);
    ...
    delBtn.innerHTML = '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:3px;"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg>Excluir';
    ...
    if (isPublished) {
      pubBtn.innerHTML = '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M5 12l5 5L20 7"/></svg>Despublicar';
      pubBtn.title = 'Voltar para rascunho';
      pubBtn.addEventListener('click', (e) => { e.stopPropagation(); _setSkillStatus(name, 'draft'); });
    } else {
      pubBtn.innerHTML = '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><polyline points="20 6 9 17 4 12"/></svg>Publicar';
      pubBtn.title = 'Publicar — aparece no índice de skills';
    ...
    testBtn.innerHTML = _svg(_ICON.test, { size: 11 }) + 'Testar';
    testBtn.title = 'Testar esta skill — executa + julgamento por IA';
    testBtn.addEventListener('click', (e) => {
      ...
      testBtn.innerHTML = _svg(_ICON.test, { size: 11 }) + 'Iniciando…';
  ...
  if (cards.length) {
    if (showBuiltin) container.appendChild(_mkSectionHeader('user', 'Suas skills', cards.length));
  ...
  if (showBuiltin) {
    ...
    container.appendChild(_mkSectionHeader('builtin', 'Capacidades nativas', builtinCards.length));
  ...
  if (pre && !card._mdLoaded) {
    ...
    if (_mdCache.has(name)) {
      ...
      try {
        ...
        pre.textContent = 'Falha ao carregar SKILL.md';
      }
    }
  }
}
