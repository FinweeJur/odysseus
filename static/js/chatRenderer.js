    ...
    if (isImage) {
      ...
      if (att.id || att.previewUrl) {
        ...
        if (att.id) {
          ...
          ocrBtn.innerHTML = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4 12.5-12.5z"/></svg><span class="attach-ocr-label">Legenda</span>';
      ...
      if (att.vision_model) {
        ...
        visionLabel.textContent = 'Visão: ' + String(att.vision_model).split('/').pop();
...
function _openImageLightbox(att) {
  ...
  full.addEventListener('error', () => {
    ...
    err.textContent = 'Falha ao carregar imagem em resolução completa.';
...
function _openVisionEditor(att, userMsgEl) {
  ...
  title.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="opacity:0.7;flex-shrink:0"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg><span>Texto de visão</span>';
  panel.appendChild(title);
  const desc = document.createElement('div');
  desc.className = 'vision-editor-desc';
  desc.textContent = 'Edite o texto e salve, novas conversas terão o novo contexto. Regenere ou continue a partir daí.';
  ...
  closeBtn.innerHTML = '<span class="vision-btn-label">Fechar</span>';
  ...
  fetch(`/api/upload/${att.id}/vision`, { credentials: 'same-origin' })
    ...
    .catch(() => {
      ta.value = '';
      ta.placeholder = 'Não foi possível carregar o texto OCR — digite sua correção e salve.';
      ta.disabled = false;
      saveBtn.disabled = false;
      regenBtn.disabled = !userMsgEl;
    });
...
export function buildImageBubble(imageUrl, prompt, model, size, quality, imageId) {
  ...
  delBtn.addEventListener('click', async (e) => {
    e.stopPropagation();
    const ok = await uiModule.styledConfirm('Excluir esta imagem?', {
      confirmText: 'Excluir',
      cancelText: 'Cancelar',
      danger: true,
    });
    ...
    if (imageId) {
      try {
        ...
        if (!res.ok && res.status !== 404) {
          uiModule.showToast?.('Falha ao excluir', 4000);
          return;
        }
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Cutia' : shortModel(resolvedModel);
