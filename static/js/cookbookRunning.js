...
function _statusLabel(status, type) {
  if (status === 'running' && type === 'download') return 'baixando';
  if (status === 'done' && type === 'download') return 'finalizado';
  if (status === 'error') return 'parado';
  return status || '';
}
...
function _taskBadge(task) {
  if (task._unreachable && task.status === 'running') return { text: 'inacessível', cls: 'cookbook-task-error' };
...
function _terminalServeDiagnosis(task, outputText) {
  ...
  if (_serveTaskLooksAwqOnLocalBackend(task, out)) {
    return {
      message: 'AWQ/GPTQ/FP8 não pode ser servido via modo unified-memory do llama.cpp/Ollama.',
      suggestion: 'Ação sugerida: use vLLM/SGLang em um servidor GPU CUDA/ROCm compatível, ou baixe uma versão GGUF para servir via llama.cpp/Ollama/unified-memory.',
      fixes: [
        { label: 'Buscar download GGUF', action: () => _openDownloadForGgufTask(task) },
        { label: 'Editar serviço', action: (panel) => _openServeEditForTask(task) },
      ],
    };
  }
  if (_serveTaskLooksAwqWithoutUsableAccelerator(task, out)) {
    return {
      message: 'AWQ/GPTQ/FP8 precisa de um caminho de acelerador vLLM/SGLang funcional; este servidor não expôs nenhum.',
      suggestion: 'Ação sugerida: escolha um servidor CUDA/ROCm onde o vLLM/SGLang consiga ver a GPU, ou baixe uma versão GGUF e a sirva com llama.cpp/Ollama.',
      fixes: [
        { label: 'Buscar download GGUF', action: () => _openDownloadForGgufTask(task) },
        { label: 'Editar serviço', action: (panel) => _openServeEditForTask(task) },
      ],
  ...
  return _diagnose(out) || {
    message: /Native llama-server not found|building llama-server|llama\.cpp/i.test(out)
      ? 'A compilação do llama.cpp parou antes do servidor ficar acessível.'
      : 'O serviço parou antes do modelo ficar acessível.',
    suggestion: /Native llama-server not found|building llama-server|llama\.cpp/i.test(out)
      ? 'Ação sugerida: copie o pacote de diagnóstico e depois edite as configurações de serviço. Para o caminho local/CPU mais rápido, use o Ollama ou um llama-server pré-compilado; builds a partir do código-fonte podem levar vários minutos e falhar se as dependências de build estiverem incompletas.'
      : 'Ação sugerida: copie o pacote de diagnóstico e depois edite as configurações de serviço ou reinicie com um fallback de CPU/backend.',
    fixes: [{ label: 'Editar serviço', action: (panel) => _openServeEditForTask(task) }],
  };
}
...
function _lastLines(text, count = 160) {
  ...
  if (!clean) return '(sem saída capturada)';
  return clean.split('\n').slice(-count).join('\n');
}
...
function _buildCrashReport(task, outputText) {
  ...
  const report = [
    '## Relatório de falha do Cookbook do Cutia',
    '',
    'Revise este relatório procurando segredos antes de publicá-lo.',
    '',
    '### Task',
    `- ID: \`${task?.sessionId || task?.id || 'desconhecido'}\``,
    `- Tipo: \`${task?.type || 'desconhecido'}\``,
    `- Status: \`${task?._unreachable ? 'inacessível' : (task?.status || 'desconhecido')}\``,
    `- Modelo/repo: \`${task?.payload?.repo_id || task?.name || 'desconhecido'}\``,
    `- Host: \`${_taskHostLabel(task)}\``,
  ];
  if (task?.platform) report.push(`- Plataforma: \`${task.platform}\``);
  if (started) report.push(`- Iniciado em: \`${started}\``);
  const port = _taskPort(task);
  if (port) report.push(`- Porta: \`${port}\``);
  if (diag?.message) report.push(`- Diagnóstico: ${diag.message}`);
  if (cmd) {
    report.push('', '### Comando', '```bash', _codeFence(cmd), '```');
  }
  report.push('', '### Última saída capturada', '```text', _codeFence(_lastLines(capturedOutput)), '```');
  return report.join('\n');
}
...
export function _parseServePhase(snapshot) {
  ...
  if (flat.includes('Application startup complete')) {
    return { phase: 'pronto', status: 'ready' };
  }
  ...
  if (llamaBuildMatches.length) {
    ...
    return { phase: `compilando llama.cpp ${pct}%`, status: 'running', pct };
  }
  if (/Native llama-server not found|building from source/i.test(flat)) {
    if (/Cloning into ['"]?llama\.cpp/i.test(flat) && !/Receiving objects:\s*100%/i.test(flat)) {
      return { phase: 'clonando llama.cpp', status: 'running' };
    }
    ...
    if (/CMAKE_BUILD_TYPE|Detecting CXX|Found Threads|Including CPU backend|CUDA nvcc found|building llama-server/i.test(flat)) {
      return { phase: 'configurando llama.cpp', status: 'running' };
    }
    return { phase: 'compilando llama.cpp', status: 'running' };
  }
  // HTTP access logs (e.g. GET /v1/models 200 OK) mean the server is up
  if (/(?:GET|POST)\s+\/[^\s]*\s+HTTP\/[\d.]+"\s*\d{3}/.test(flat)) {
    return { phase: 'ocioso', status: 'ready' };
  }
  if (flat.includes('Loading weights took')) {
    return { phase: 'inicializando', status: 'running' };
  }
  ...
  if (flat.includes('GPU KV cache') && !flat.includes('GPU KV cache usage')) {
    return { phase: 'aquecendo', status: 'running' };
  }
  if (loadMatches.length) {
    const pct = parseInt(loadMatches[loadMatches.length - 1][1]);
    return { phase: `carregando ${pct}%`, status: 'running', pct };
  }
  if (dlMatches.length) {
    const pct = parseInt(dlMatches[dlMatches.length - 1][1]);
    return { phase: `baixando ${pct}%`, status: 'running', pct };
  }
  return {};
}
...
function _refreshModelsAfterEndpointChange() {
  ...
  if (pickerLabel) {
    ...
    pickerLabel.innerHTML = '<span style="opacity:0.4;">atualizando…</span>';
  }
...
async function _startQueuedDownload(task) {
  if (!task.payload) {
    _updateTask(task.sessionId, { status: 'error', output: 'Sem payload' });
    _renderRunningTab();
    return;
  }
  ...
  try {
    ...
    if (!data.ok) {
      _updateTask(task.sessionId, { status: 'error', output: data.error || 'Erro desconhecido' });
      _renderRunningTab();
      return;
    }
    ...
    _updateTask(task.sessionId, { status: 'error', output: e.message || 'Erro de rede' });
    _renderRunningTab();
  }
}
...
function _autoSaveWorkingConfig(task) {
  ...
  _savePresets(presets);
  task._autoSaved = true;
  uiModule.showToast('Configuração funcional salva');
}
...
async function _retryTask(el, task) {
  ...
  if (badge) { badge.textContent = 'reiniciando…'; badge.className = 'cookbook-task-status'; }
  ...
  if (task.payload) {
    if (task.type === 'serve' && task.payload._cmd) {
      ...
      uiModule.showToast('Tentando o download novamente — o progresso pode parecer reiniciado enquanto o HuggingFace verifica os arquivos em cache, depois deve retomar.', 7000);
...
async function _retryDownload(name, payload, replaceSessionId = '') {
  try {
    ...
    if (!res.ok) {
      uiModule.showToast('Falha no download: HTTP ' + res.status);
      if (replaceSessionId) _updateTask(replaceSessionId, { status: 'crashed', _retrying: false });
      return;
    }
    const data = await res.json();
    if (!data.ok) {
      uiModule.showToast('Falha no download: ' + (data.error || ''));
      if (replaceSessionId) _updateTask(replaceSessionId, { status: 'crashed', _retrying: false });
      return;
    }
    if (replaceSessionId) {
    ...
    uiModule.showToast(`Baixando ${name}...`);
  } catch (e) {
    uiModule.showToast('Falha no download: ' + e.message);
    if (replaceSessionId) _updateTask(replaceSessionId, { status: 'crashed', _retrying: false });
  }
}
...
export async function _serveAutoFix(panel, envVar) {
  ...
  try {
    uiModule.showToast(`Tentando novamente com ${envVar}...`);
...
async function _openServeEditForTask(task, cmdOverride, fieldOverrides = null) {
  ...
  if (!repo) { uiModule.showToast('Nenhuma informação de modelo nesta tarefa'); return; }
  ...
  try {
    ...
    uiModule.showToast('Não foi possível abrir o painel de serviço');
  }
}
...
export async function _serveAutoRetryReplace(panel, flag, value) {
  ...
  if (task.remoteHost) _envState.remoteHost = task.remoteHost;
  try {
    uiModule.showToast(`Tentando novamente com ${flag} ${value}...`);
    await _launchServeTask(task.name, task.payload.repo_id, newCmd);
  } finally {
    _envState.remoteHost = origHost;
  }
...
export async function _serveAutoRetryRemove(panel, flag) {
  ...
  if (task.remoteHost) _envState.remoteHost = task.remoteHost;
  try {
    uiModule.showToast(`Tentando novamente sem ${flag}...`);
    await _launchServeTask(task.name, task.payload.repo_id, newCmd);
  } finally {
    _envState.remoteHost = origHost;
  }
...
export async function _serveAutoRetry(panel, flag) {
  ...
  if (task.remoteHost) _envState.remoteHost = task.remoteHost;
  try {
    uiModule.showToast(`Tentando novamente com ${flag}...`);
    await _launchServeTask(task.name, task.payload.repo_id, newCmd);
  } finally {
    _envState.remoteHost = origHost;
  }
  ...
  return new Promise((resolve) => {
    ...
    overlay.innerHTML = `
      <div class="cookbook-edit-modal">
        <div class="cookbook-edit-title">Editar comando de serviço</div>
        ...
        <div class="cookbook-edit-actions">
          ...
          <button class="cookbook-edit-save memory-toolbar-btn">Salvar e reiniciar</button>
        </div>
      </div>`;
...
export async function _launchServeTask(shortName, repo, cmd, fields, hostOverride) {
  ...
  try {
    ...
    if (!data.ok) {
      ...
      uiModule.showToast('Falha ao iniciar: ' + String(err).slice(0, 200), 9000);
      return;
    }
    ...
    uiModule.showToast(`Servindo ${shortName}...`);
  } catch (e) {
    uiModule.showToast('Falha: ' + e.message);
  }
}
...
export function _renderRunningTab() {
  ...
  if (hasContent && !runTab) {
    ...
    runTab.innerHTML = `Em execução${activeCountHtml}${_errCount ? `<span class="cookbook-tab-error-dot"></span>` : ''}`;
    ...
    runTab.innerHTML = tasks.length ? `Em execução${activeCountHtml}${_errCount2 ? '<span class="cookbook-tab-error-dot"></span>' : ''}` : 'Em execução';
    if (!hasContent) {
  ...
  if (hasContent && !group) {
    ...
    group.innerHTML = '<div class="admin-card" style="flex:1;display:flex;flex-direction:column;overflow:hidden;">' +
      ...
      '<h2 style="margin:0;padding:0;line-height:1;">Em execução <span id="running-count" class="memory-count" style="font-size:0.6em;opacity:0.6;font-weight:normal">' + activeCount + '</span></h2>' +
      '</div>' +
      '<p class="memory-desc doclib-desc" style="margin-top:6px;">Downloads ativos e processos em execução.</p>' +
      '</div>';
  ...
  group.querySelectorAll('[data-clear-server]').forEach(btn => {
    ...
    btn.addEventListener('click', async (e) => {
      ...
      if (!await window.styledConfirm(`Limpar tarefas concluídas em ${_serverName(host)}?`, { confirmText: 'Limpar' })) return;
  ...
  group.querySelectorAll('[data-stop-server]').forEach(btn => {
    ...
    btn.addEventListener('click', async (e) => {
      ...
      if (!running.length) { uiModule.showToast(`Nada em execução em ${_serverName(host)}`); return; }
      if (!await window.styledConfirm(`Parar ${running.length} tarefa${running.length > 1 ? 's' : ''} em execução em ${_serverName(host)}?`, { confirmText: 'Parar todos' })) return;
      ...
      uiModule.showToast(`${running.length} tarefa${running.length > 1 ? 's' : ''} parada${running.length > 1 ? 's' : ''} em ${_serverName(host)}`);
    });
  });
  ...
  group.querySelectorAll('.cookbook-task').forEach(el => {
    ...
    if (task) {
      ...
      if (typeChip) {
        ...
        typeChip.textContent = isDoneDl ? 'finalizado' : task.type;
  ...
  for (const task of tasks) {
    ...
    const _bdg = _taskBadge(task);
    const _bdgTitle = (task._unreachable && task.status === 'running') ? ' title="Servidor não responde — pode ter travado"' : '';
    el.innerHTML = `
      <div class="cookbook-task-header">
        ...
        <span class="cookbook-task-indicator"><span class="cookbook-task-wave" style="display:${task.status === 'running' ? '' : 'none'}"></span><span class="cookbook-task-check" title="Limpar" style="display:${_canClearTask(task) ? '' : 'none'}"><svg class="cookbook-task-check-ico" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#50fa7b" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg><svg class="cookbook-task-clear-ico" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg><span class="cookbook-task-done-label">${esc(_clearPillLabel(task))}</span><span class="cookbook-task-clear-label">limpar</span></span></span>
        <button type="button" class="cookbook-task-start-now" title="Iniciar este download em fila agora" style="display:${(task.type === 'download' && task.status === 'queued') ? '' : 'none'}"><svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><polygon points="8 5 19 12 8 19 8 5"/></svg><span>iniciar agora</span></button>
        <span class="cookbook-task-status ${_bdg.cls}"${_bdgTitle}>${esc(_bdg.text)}</span>
        <button class="cookbook-task-menu-btn" title="Ações">&#8942;</button>
      </div>
      <div class="cookbook-task-sub"><span class="cookbook-task-session">${esc(task.sessionId)}</span><span class="cookbook-task-uptime" style="display:${((task.type === 'serve' || task.type === 'download') && task.status === 'running') ? '' : 'none'}"></span>${(task.type === 'download') ? `<span class="cookbook-task-dldir" title="Destino do download" style="font-size:9px;color:var(--fg-muted);font-family:'Fira Code',monospace;opacity:0.4;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:40ch;">Pasta: ${esc(task.payload?.local_dir || '~/.cache/huggingface/hub')}</span>` : ''}</div>
    ...
    if (task.type === 'download') {
      ...
      if (_serveBtn) {
        _serveBtn.addEventListener('click', async (e) => {
          ...
          } catch (err) { uiModule.showToast('Não foi possível abrir o Servir: ' + err.message); }
        });
      }
    }
    ...
    if (menuBtn) {
      ...
      menuBtn.addEventListener('click', (e) => {
        ...
        if (task.type === 'download' && task.status === 'queued') {
          items.push({ label: 'Iniciar agora', action: 'start-now', custom: () => {
            _startQueuedDownload(task);
            _renderRunningTab();
          }});
        }
        ...
        if (task.status === 'running') {
          items.push({ label: 'Parar', action: 'stop', danger: true });
        }
        ...
        if (task.type === 'serve' && task.payload?.repo_id) {
          items.push({ label: 'Editar serviço', action: 'edit-panel', custom: () => _openEdit() });
        }
        // Save serve — save current launch config as a preset.
        if (task.type === 'serve' && task.payload?._cmd) {
          items.push({ label: 'Salvar serviço', action: 'save', custom: () => {
            if (!_saveTaskAsPreset(task)) { uiModule.showToast('Já está salvo'); return; }
            uiModule.showToast('Salvo nas configurações');
            _renderRunningTab();
          }});
        }
        ...
        if (task.type === 'serve' && task.payload?._cmd) {
          items.push({ label: 'Registrar endpoint', action: 'register-endpoint', custom: async () => {
            ...
            try {
              ...
              if (existing) {
                uiModule.showToast(`Já registrado como "${existing.name}"`);
              ...
              if (res.ok) {
                ...
                uiModule.showError(`Falha ao registrar: ${res.status} ${body.slice(0, 140)}`);
              }
            } catch (e) {
              uiModule.showError(`Falha ao registrar: ${e.message || e}`);
            }
          }});
        }
        if (_isWindows(task)) {
          ...
          items.push({ label: 'Copiar comando de log', action: 'copy-tmux', custom: () => {
            _copyText(logCmd);
          }});
        } else {
        ...
        if (_shouldOfferCrashReport(task)) {
          items.push({ label: 'Copiar relatório de falha', action: 'copy-crash-report', custom: () => {
            const out = (el.querySelector('.cookbook-output-pre')?.textContent || task.output || '');
            _copyText(_buildCrashReport(task, out));
            uiModule.showToast('Relatório de falha copiado');
          }});
        }
        // Copy the last 50 lines of the task's output/log.
        items.push({ label: 'Copiar últimas 50 linhas', action: 'copy-log', custom: () => {
          const out = (el.querySelector('.cookbook-output-pre')?.textContent || task.output || '');
          const last = out.split('\n').slice(-50).join('\n');
          _copyText(last);
          uiModule.showToast('Últimas 50 linhas copiadas');
        }});
        items.push({ label: 'Remover', action: 'kill', danger: true });
    ...
    el.querySelector('.cookbook-task-action-stop').addEventListener('click', async () => {
      ...
      if (el._abort) el._abort.abort();
      const badge = el.querySelector('.cookbook-task-status');
      if (badge) { badge.textContent = 'parando...'; badge.className = 'cookbook-task-status cookbook-task-stopping'; }
    ...
    try {
            ...
            if (task.type === 'serve') {
              const diag = _diagnose(lastOutput) || {
                message: _serveTaskLooksAwqOnLocalBackend(task, lastOutput)
                  ? 'AWQ/GPTQ/FP8 não pode ser servido via modo unified-memory do llama.cpp/Ollama.'
                ...
                suggestion: _serveTaskLooksAwqOnLocalBackend(task, lastOutput)
                  ? 'Ação sugerida: use vLLM/SGLang em um servidor GPU CUDA/ROCm compatível, ou baixe uma versão GGUF para servir via llama.cpp/Ollama/unified-memory.'
                  : /Native llama-server not found|building llama-server|llama\.cpp/i.test(lastOutput)
                  ? 'Ação sugerida: copie o pacote de diagnóstico e depois edite as configurações de serviço. Para o caminho local/CPU mais rápido, use o Ollama ou um llama-server pré-compilado; builds a partir do código-fonte podem levar vários minutos e falhar se as dependências de build estiverem incompletas.'
                  : 'Ação sugerida: copie o pacote de diagnóstico e depois edite as configurações de serviço ou reinicie com um fallback de CPU/backend.',
                fixes: [{ label: 'Editar serviço', action: (panel) => _openServeEditForTask(task) }],
              ...
              const diag = {
                message: isDisk
                  ? 'O download parou porque este servidor ficou sem espaço em disco.'
                  : isNetwork
                  ? 'O download parou após a conexão com o HuggingFace ser interrompida.'
                  : nearDone
                  ? 'O download parou perto do fim, antes de capturar a marca final de conclusão.'
                  : 'O download parou antes do HuggingFace informar a conclusão.',
                suggestion: isDisk
                  ? 'Ação sugerida: libere espaço em disco e tente o download novamente. O HuggingFace retoma arquivos incompletos quando possível.'
                  : nearDone
                  ? 'Ação sugerida: clique em Reconectar primeiro — o download pode ter terminado após o buffer de saída rolar. Tente novamente apenas se reconectar não recuperar.'
                  : 'Ação sugerida: clique em Reconectar para reanexar à sessão tmux. Se falhar, tente novamente — o HuggingFace retoma arquivos incompletos quando possível.',
                fixes: isDisk
                  ? [
                      { label: 'Tentar download novamente', action: () => _retryTask(el, task) },
                      { label: 'Copiar últimas 50 linhas', action: () => {
                        const last = String(lastOutput || '').split('\n').slice(-50).join('\n');
                        _copyText(last || 'Nenhum log de download disponível.');
                      } },
                    ]
                  : [
                      _reconnectFix,
      ...
      if (snapshot) {
        ...
        if (task.type === 'download') {
          ...
          if (badge) {
            ...
            if (curProgress !== el._lastProgress) {
              ...
              badge.textContent = `travado há ${mins}m ↻`;
              badge.className = 'cookbook-task-status cookbook-task-error';
              badge.title = 'Clique para tentar novamente — retoma de onde parou';
              ...
              badge.textContent = _startupStalled ? 'travado em 0% — tentando novamente' : 'desatualizado — reiniciando';
              badge.className = 'cookbook-task-status cookbook-task-error';
              _showCookbookNotif(true);
              try {
                await fetch('/api/shell/exec', {
              ...
              try {
                ...
                if (data.ok && data.session_id) {
                  ...
                  el._lastProgressTime = Date.now();
                  badge.textContent = 'reiniciado';
                  badge.className = 'cookbook-task-status cookbook-task-running';
                  continue;
                }
              } catch {}
              badge.textContent = 'desatualizado — falha ao reiniciar';
              badge.className = 'cookbook-task-status cookbook-task-error';
              _showCookbookNotif(true);
              break;
            }
            ...
            if (snapshot.includes('DOWNLOAD_FAILED')) {
              ...
              if (!controller.signal.aborted && !_accessDenied && task.type === 'download' && task.payload && _dlN < _DL_MAX_AUTO_RETRY) {
                ...
                badge.textContent = `tentando novamente (${_dlN + 1}/${_DL_MAX_AUTO_RETRY})…`;
                badge.className = 'cookbook-task-status cookbook-task-running';
                uiModule.showToast(`Download interrompido — tentando novamente (${_dlN + 1}/${_DL_MAX_AUTO_RETRY}), retoma de onde parou…`, 6000);
            ...
            if (snapshot.includes('DOWNLOAD_OK') || (snapshot.includes('/snapshots/') && completed >= totalFiles && totalFiles > 0)) {
              ...
              if (_typeChip) { _typeChip.textContent = 'finalizado'; _typeChip.classList.add('cookbook-task-type-done'); }
          ...
          fetch('/api/model-endpoints', { credentials: 'same-origin' })
            ...
            .then(async (res) => {
              if (res && res.ok) {
                ...
                _updateTask(task.sessionId, { _endpointAdded: true });
                _autoSaveWorkingConfig(task);   // endpoint live → remember these settings
                uiModule.showToast(`Endpoint de modelo adicionado: ${host}:${port}`);
                  ...
                  for (const item of items) {
                    ...
                    if (url.includes(host) || url.includes(port)) {
                      ...
                      if (mid && window.sessionModule?.createDirectChat) {
                        ...
                        if (window.sessionModule?.updateModelPicker) window.sessionModule.updateModelPicker();
                        uiModule.showToast(`Alterado para ${mid.split('/').pop()}`);
                        return;
                      }
                    }
                  }
                ...
                uiModule.showError(`Falha ao registrar endpoint automaticamente (${res.status}). Use ⋮ → Registrar endpoint para tentar novamente.`);
              }
            })
            .catch((e) => {
              console.warn('Endpoint auto-add error', e);
              uiModule.showError(`Erro ao registrar endpoint automaticamente: ${e.message || e}. Use ⋮ → Registrar endpoint para tentar novamente.`);
            })
            .finally(() => { task._endpointAddInFlight = false; });
          _updateTask(task.sessionId, { status: 'running' });
          const badge = el.querySelector('.cookbook-task-status');
          if (badge) { badge.textContent = 'em execução'; badge.className = 'cookbook-task-status cookbook-task-running'; }
          _showCookbookNotif();
        }
        // Detect process exit
        if (snapshot.includes('=== Process exited with code')) {
          ...
          const badge = el.querySelector('.cookbook-task-status');
          if (badge) { badge.textContent = _statusLabel(status, task.type); badge.className = `cookbook-task-status cookbook-task-${status}`; }
          _renderRunningTab();
        }
