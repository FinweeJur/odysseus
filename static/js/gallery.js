...
function _updateTagCount() {
  ...
  if (el) el.textContent = _total ? `${_totalTagged}/${_total} marcadas` : '';
}
let _search = '';
...
async function _bulkUpload(filesOrItems, fallbackAlbumId) {
  ...
  async function worker() {
    while (true) {
      ...
      if (status) status.textContent = `${done}/${total}${dupes ? ` (${dupes} duplicadas)` : ''}`;
    }
  }
  await Promise.all(Array.from({ length: Math.min(CONCURRENCY, total) }, worker));

  const msg = `${done - dupes - errors} importadas` +
    (dupes ? `, ${dupes} duplicadas ignoradas` : '') +
    (errors ? `, ${errors} erros` : '');
...
async function _handleGalleryDrop(e) {
  ...
  if (looksLikeFolderUri) {
    uiModule.showError('Navegadores não conseguem ler pastas arrastadas de gerenciadores de arquivos nativos (Thunar/Nautilus). Use o quadro "Upload album" na aba Álbuns.');
  } else if (entries.length || dtItems.length) {
    uiModule.showToast('Nenhuma imagem encontrada nesse arraste');
  }
}
...
function _renderAlbums() {
  ...
  if (filterC) {
    ...
    if (!_activeAlbum) {
      fhtml += `<button class="gallery-chip gallery-chip-fav${_favoritesOnly ? ' active' : ''}" data-fav="true" title="Favoritas">&#9829;</button>`;
    }
    _activeTags.forEach(t => {
      fhtml += `<span class="gallery-chip gallery-chip-active-album" title="Filtrado pela tag — clique no × para remover"><span>#${_esc(t)}</span><button class="gallery-chip-clear" data-clear-tag="${_esc(t)}" aria-label="Remover filtro de tag">&times;</button></span>`;
    });
    if (_activeAlbum) {
      const a = _albums.find(x => x.id === _activeAlbum);
      if (a) {
        fhtml += `<span class="gallery-chip gallery-chip-active-album" title="Mostrando este álbum agora — clique no X para limpar"><span>${_esc(a.name)}</span><button class="gallery-chip-clear" data-clear="album" aria-label="Limpar filtro de álbum">&times;</button></span>`;
      }
    }
...
function _ensureAlbumsToolbar(container) {
  ...
  container.innerHTML = `
    <div class="gallery-toolbar" id="gallery-albums-toolbar">
      <div class="gallery-search-wrap">
        <input type="text" class="gallery-search" id="gallery-albums-search" placeholder="Buscar álbuns..." />
      </div>
      <button class="gallery-select-btn gallery-toolbar-action" id="gallery-albums-select-btn" title="Selecionar para ações em massa" style="position:relative;top:2px;"><span style="position:relative;top:1px;">Selecionar</span></button>
    </div>
    <div class="memory-bulk-bar hidden" id="gallery-albums-bulk-bar">
      ...
      <span id="gallery-albums-bulk-count" style="position:relative;top:-1px;">0 selecionados</span>
      <button class="memory-toolbar-btn" id="gallery-albums-bulk-delete" title="Excluir selecionados" style="margin-left:auto;color:var(--color-error, #f44);position:relative;top:-3px;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:3px;"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/></svg>Excluir</button>
  ...
  container.querySelector('#gallery-albums-bulk-delete').addEventListener('click', (e) => {
    e.stopPropagation();
    if (!_albumSelected.size) { uiModule.showToast('Selecione álbuns primeiro'); return; }
...
function _setAlbumSelectMode(on) {
  ...
  const container = document.getElementById('gallery-albums-container');
  container.querySelector('#gallery-albums-select-btn span').textContent = on ? 'Cancelar' : 'Selecionar';
...
function _updateAlbumBulkCount() {
  ...
  if (cnt) cnt.textContent = sel + ' selecionados';
  ...
  if (!_albums.length) {
    wrap.innerHTML = `
      <div class="gallery-albums-empty">
        ...
        <button class="gallery-select-btn" id="gallery-albums-new">+ Novo álbum</button>
      </div>`;
    _wireAlbumsEvents(wrap);
    return;
  }
  if (!albums.length) {
    wrap.innerHTML = `<div class="gallery-albums-empty"><p>Nenhum álbum corresponde a "${_esc(_albumSearch)}".</p></div>`;
    return;
  }
    ...
    html += `
      <div class="gallery-album-card gallery-album-card-add" id="gallery-albums-new">
        ...
        <div class="gallery-album-info">
          <div class="gallery-album-name">Novo álbum</div>
        </div>
      </div>
      <div class="gallery-album-card gallery-album-card-add" id="gallery-albums-upload">
        ...
        <div class="gallery-album-info">
          <div class="gallery-album-name">Upload de álbum</div>
          <div class="gallery-album-count">Escolha uma pasta</div>
        </div>
      </div>`;
  }
  albums.forEach(a => {
    ...
    html += `
      <div class="${cls}" data-album="${_esc(a.id)}">
        ${dot}
        <button class="gallery-album-menu-btn" data-album="${_esc(a.id)}" title="Opções" aria-label="Opções do álbum"${_albumSelectMode ? ' style="display:none"' : ''}>
        ...
        <div class="gallery-album-info">
          ...
          <div class="gallery-album-count">${a.count} foto${a.count === 1 ? '' : 's'}</div>
        </div>
      </div>`;
  });
  html += '</div>';
...
function _wireAlbumsEvents(scope) {
  ...
  container.querySelectorAll('.gallery-album-menu-pop').forEach(pop => {
    ...
    pop.querySelector('[data-action="rename"]')?.addEventListener('click', async (e) => {
      ...
      const newName = prompt('Renomear álbum:', album?.name || '');
    ...
    pop.querySelector('[data-action="delete"]')?.addEventListener('click', async (e) => {
      ...
      const ok = await uiModule.styledConfirm(
        `Excluir álbum "${album?.name || ''}"? As fotos dentro dele permanecerão na sua biblioteca.`,
  ...
  document.getElementById('gallery-albums-new')?.addEventListener('click', async () => {
    const name = (uiModule.styledPrompt
      ? await uiModule.styledPrompt('Dê um nome ao seu novo álbum.', { title: 'Novo álbum', placeholder: 'ex.: Férias 2026', confirmText: 'Criar' })
  ...
  document.getElementById('gallery-albums-upload')?.addEventListener('click', () => {
    ...
    picker.addEventListener('change', async () => {
      ...
      picker.remove();
      if (!images.length) {
        if (uiModule) uiModule.showToast('Nenhuma imagem ou vídeo nessa pasta');
        return;
      }
      ...
      if (!folderName) {
        folderName = prompt('Nome do álbum para essas fotos:') || '';
      ...
      if (!album) {
        if (uiModule) uiModule.showError('Não foi possível criar o álbum');
        return;
      }
...
async function _bulkDeleteAlbums(ids) {
  ...
  const ok = await uiModule.styledConfirm(
    `Excluir ${ids.length} álbum${ids.length > 1 ? 's' : ''}? As fotos dentro deles permanecerão na sua biblioteca.`,
    { confirmText: 'Excluir', danger: true },
  );
  if (!ok) return;
  let failed = 0;
  ...
  if (failed) uiModule.showError(`Falha ao excluir ${failed} de ${ids.length} álbuns`);
  else if (uiModule) uiModule.showToast(`${ids.length} álbum${ids.length > 1 ? 's' : ''} excluído${ids.length > 1 ? 's' : ''}`);
  _setAlbumSelectMode(false);
  await _fetchAlbums();
  _renderAlbumsTab();
  _renderAlbums();
...
function _draftsPaint() {
  ...
  grid.innerHTML = filtered.map(d => {
    ...
    const checkbox = _draftsSelectMode
      ...
      : '';
    return `
      <div class="gallery-editor-draft-card${checked ? ' selected' : ''}${_draftsSelectMode ? ' select-mode' : ''}" data-draft-id="${_esc(d.id)}" tabindex="0" title="Retomar ${_esc(d.name || 'projeto')}">
        ${checkbox}
        ${thumb}
        <div class="gallery-editor-draft-info">
          <div class="gallery-editor-draft-name">${_esc(d.name || 'Sem título')}</div>
          <div class="gallery-editor-draft-meta">${_esc([dims, updated].filter(Boolean).join(' · '))}</div>
        </div>
        <button class="gallery-editor-draft-delete" data-draft-id="${_esc(d.id)}" title="Excluir projeto" aria-label="Excluir projeto">×</button>
      </div>`;
  }).join('');
  ...
  grid.querySelectorAll('.gallery-editor-draft-delete').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      if (!id) return;
      const ok = await uiModule.styledConfirm('Excluir este projeto?', {
...
function _draftsSyncBulkBar() {
  ...
  if (countEl) countEl.textContent = `${_draftsSelected.size} selecionados`;
  if (selectBtn) {
    selectBtn.textContent = _draftsSelectMode ? 'Cancelar' : 'Selecionar';
...
function _draftsWireOnce() {
  ...
  document.getElementById('gallery-editor-drafts-bulk-delete')?.addEventListener('click', async () => {
    ...
    const ok = await uiModule.styledConfirm(`Excluir ${n} projeto${n === 1 ? '' : 's'}?`, {
      confirmText: 'Excluir', cancelText: 'Cancelar', danger: true,
    });
    if (!ok) return;
...
function _renderEditorLanding() {
  ...
  const presets = [
    { w: 1024, h: 1024, label: 'Quadrado HD — 1024 × 1024' },
    { w: 1920, h: 1080, label: 'Widescreen — 1920 × 1080' },
    { w: 1080, h: 1920, label: 'Retrato — 1080 × 1920' },
    { w: 1080, h: 1080, label: 'Instagram — 1080 × 1080' },
    { w: 1500, h: 1050, label: 'Cartão postal — 1500 × 1050' },
    { w: 2480, h: 3508, label: 'A4 (300dpi) — 2480 × 3508' },
    { w: 2550, h: 3300, label: 'Carta (300dpi) — 2550 × 3300' },
  ...
  container.innerHTML = `
    <div class="gallery-editor-landing">
      ...
      <h3>Editor de Imagem <span class="ge-alpha-tag">Alpha</span></h3>
      <p>Comece com uma tela em branco, ou abra uma foto da sua galeria para editá-la.</p>
      <div class="gallery-editor-landing-actions">
        <button class="gallery-select-btn" id="gallery-editor-new">Nova tela...</button>
        <button class="gallery-select-btn" id="gallery-editor-pick">Procurar fotos</button>
      </div>
      ...
      <div class="gallery-editor-drafts" id="gallery-editor-drafts" hidden>
        <div class="gallery-editor-drafts-header">
          <h4 class="gallery-editor-drafts-title">Projetos salvos</h4>
          <input type="search" class="gallery-editor-drafts-search" id="gallery-editor-drafts-search" placeholder="Buscar projetos…" autocomplete="off" />
          <button class="gallery-select-btn" id="gallery-editor-drafts-select" title="Alternar multi-seleção">Selecionar</button>
        </div>
        <div class="gallery-bulk-bar hidden" id="gallery-editor-drafts-bulk">
          <label class="memory-bulk-check-all"><input type="checkbox" id="gallery-editor-drafts-select-all"> Todos</label>
          <span class="gallery-bulk-count" id="gallery-editor-drafts-bulk-count">0 selecionados</span>
          <button class="gallery-bulk-delete" id="gallery-editor-drafts-bulk-delete"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:3px;"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/></svg>Excluir selecionados</button>
          <button class="memory-toolbar-btn" id="gallery-editor-drafts-bulk-cancel" title="Cancelar (Esc)" style="margin-left:4px;padding:3px 6px;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
        </div>
  ...
  document.getElementById('gallery-editor-new')?.addEventListener('click', async () => {
    ...
    await openEditor(null, null, null, 'Nova tela');
...
function _renderGrid() {
  ...
  // both grids.
  const uploadTile = `
    <div class="gallery-card gallery-card-upload" id="gallery-upload-tile" title="Enviar fotos ou vídeos">
      <div class="gallery-card-upload-inner">
        <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
        <div class="gallery-card-upload-label">Enviar</div>
      </div>
    </div>`;

  if (_items.length === 0) {
    grid.innerHTML = uploadTile + '<div class="gallery-empty">Nenhuma foto ainda. Clique em Enviar ou arraste e solte para começar!</div>';
    _wireUploadTile();
  ...
  _items.forEach(img => {
    ...
    html += `
      <div class="gallery-card" data-id="${_esc(img.id)}">
        ...
        <button class="gallery-fav-btn${favCls}" data-id="${_esc(img.id)}" title="Favorita">&#9829;</button>
        <button class="gallery-dl-btn" data-id="${_esc(img.id)}" data-url="${_esc(img.url)}" data-filename="${_esc(img.filename || '')}" title="Baixar">
...
function _openDetail(img) {
  ...
  const _relAgo = (d) => {
    ...
    if (secs < 3600) { const m = Math.floor(secs / 60); return `há ${m} minuto${m !== 1 ? 's' : ''}`; }
    if (secs < 86400) { const h = Math.floor(secs / 3600); return `há ${h} hora${h !== 1 ? 's' : ''}`; }
    if (secs < 86400 * 7) { const d2 = Math.floor(secs / 86400); return `há ${d2} dia${d2 !== 1 ? 's' : ''}`; }
    if (secs < 86400 * 30) { const w = Math.floor(secs / (86400 * 7)); return `há ${w} semana${w !== 1 ? 's' : ''}`; }
    if (secs < 86400 * 365) { const mo = Math.floor(secs / (86400 * 30)); return `há ${mo} mês${mo !== 1 ? 'es' : ''}`; }
  ...
  if (img.updated_at && img.created_at) {
    ...
    if (!isNaN(u) && !isNaN(c) && (u.getTime() - c.getTime() > 10000)) {
      editedHtml = `<div class="gallery-detail-section"><label>Editada</label><div>${u.toLocaleString()}<span class="gallery-date-rel"> (${_relAgo(u)})</span></div></div>`;
    }
  }

  detail.innerHTML = `
    <div class="gallery-detail-header">
      <button class="gallery-detail-back" id="gallery-detail-back">&larr; Voltar</button>
      <div style="flex:1"></div>
      <button class="gallery-detail-back" id="gallery-edit-direct-btn" title="Editar (E)" aria-label="Editar foto" style="display:inline-flex;align-items:center;gap:4px;">
      ...
      <button class="gallery-detail-back gallery-detail-fav-header${img.favorite ? ' active' : ''}" id="gallery-detail-fav-header" title="${img.favorite ? 'Remover de favoritas' : 'Adicionar a favoritas'}" aria-label="Favorita" aria-pressed="${img.favorite ? 'true' : 'false'}" style="display:inline-flex;align-items:center;justify-content:center;padding:4px 8px;">
      ...
      <div class="gallery-detail-menu-wrap">
        <button class="gallery-detail-action gallery-detail-menu-btn" id="gallery-detail-menu-btn" title="Ações" aria-label="Ações da foto">
        ...
        <div class="gallery-detail-menu dropdown" id="gallery-detail-menu" hidden>
          <button class="dropdown-item-compact" id="gallery-fav-detail">
            ...
            ${img.favorite ? 'Adicionada a favoritas' : 'Adicionar a favoritas'}
          </button>
          <button class="dropdown-item-compact" id="gallery-ai-tag-btn" data-mode="${aiTags ? 'clear' : 'tag'}">
            ...
            ${aiTags ? 'Limpar tags IA' : 'Marcar com IA'}
          </button>
    ...
    <div class="gallery-detail-body">
      <div class="gallery-detail-image" id="gallery-detail-image-wrap" style="position:relative">
        <button class="gallery-detail-rotate gallery-detail-rotate-ccw" id="gallery-rotate-ccw-btn" title="Girar 90° para esquerda" aria-label="Girar para esquerda">
        ...
        <button class="gallery-detail-rotate gallery-detail-rotate-cw" id="gallery-rotate-btn" title="Girar 90° para direita" aria-label="Girar para direita">
        ...
        <button class="gallery-detail-nav gallery-detail-nav-prev" id="gallery-detail-prev" title="Anterior (←)" aria-label="Anterior">
        ...
        <button class="gallery-detail-nav gallery-detail-nav-next" id="gallery-detail-next" title="Próximo (→)" aria-label="Próximo">
      ...
      <div class="gallery-detail-sidebar">
        <div class="gallery-detail-section">
          ...
          <div class="gallery-name-wrap">
            <input type="text" class="gallery-detail-name-input" id="gallery-detail-name-input"
              value="${_esc(img.prompt || '')}" placeholder="Foto sem título (pressione Enter para salvar)" />
        ...
        ${img.camera ? `<div class=”gallery-detail-section”><label>Câmera</label><div>${_esc(img.camera)}</div></div>` : ''}
        ${img.gps ? `<div class=”gallery-detail-section”><label>Localização</label><div>${img.gps.lat}, ${img.gps.lng}</div></div>` : ''}
        ${img.model ? `<div class=”gallery-detail-section”><label>Fonte</label><div>${_esc(img.model)}</div></div>` : ''}
        ${img.session_name ? `<div class=”gallery-detail-section”><label>Sessão</label><div>${_esc(img.session_name)}</div></div>` : ''}
        ${aiTags ? `<div class=”gallery-detail-section”><label>Tags IA</label><div class=”gallery-ai-tags”>${aiTags.split(',').map(t => t.trim()).filter(Boolean).map(t => `<button class=”gallery-ai-chip gallery-aitag-chip” data-tag-filter=”${_esc(t)}” title=”Tag gerada por IA — clique para filtrar fotos com a tag “${_esc(t)}””><span class=”gallery-aitag-mark” aria-hidden=”true”>✦</span>${_esc(t)}</button>`).join('')}</div></div>` : ''}
        <div class=”gallery-detail-section”>
          <label>Tags</label>
          <div class=”gallery-ai-tags” id=”gallery-user-tag-chips”>${userTags.split(',').map(t => t.trim()).filter(Boolean).map(t => `<button class=”gallery-ai-chip gallery-user-chip” data-tag-filter=”${_esc(t)}” title=”Filtrar fotos com a tag “${_esc(t)}””>${_esc(t)}<span class=”gallery-tag-x” title=”Remover tag” aria-label=”Remover tag”>×</span></button>`).join('')}</div>
          <div class=”gallery-tag-input-wrap”>
            <input type=”text” class=”gallery-tag-input” id=”gallery-tag-input”
              value=”” placeholder=”Adicionar tag” title=”Digite uma tag e pressione Enter para adicioná-la” />
        ...
        <div class="gallery-detail-section" id="gallery-detail-people-section" style="display:none">
          <label>Pessoas nesta foto</label>
  ...
  const _toggleDetailFavorite = async () => {
    ...
    if (menuItem) menuItem.innerHTML = data.favorite ? '&#9829; Adicionada a favoritas' : '&#9825; Adicionar a favoritas';
    ...
    if (headerBtn) {
      ...
      headerBtn.setAttribute('title', data.favorite ? 'Remover de favoritas' : 'Adicionar a favoritas');
  ...
  document.getElementById('gallery-ai-tag-btn').addEventListener('click', async (e) => {
    ...
    if (stage) {
      ...
      try {
        ...
        label.textContent = clearMode ? 'Limpando…' : 'Marcando com IA…';
        label.style.cssText = 'font-size:11px;opacity:0.7;';
        overlay.appendChild(label);
      } catch (_) { overlay.textContent = clearMode ? 'Limpando…' : 'Marcando com IA…'; }
    ...
    try {
      ...
      if (data.ok) {
        ...
        uiModule.showToast(clearMode ? 'Tags IA limpas' : 'Tags IA adicionadas');
        _openDetail(img); // re-render detail
      } else {
        uiModule.showError(data.error || (clearMode ? 'Falha ao limpar' : 'Falha ao marcar com IA'));
      }
    } catch (e2) {
      cleanup();
      uiModule.showError(clearMode ? 'Falha ao limpar' : 'Falha ao marcar com IA');
    }
  });
  ...
  const _rotate = async (angle) => {
    ...
    if (stage) {
      ...
      try {
        ...
        spinner.element.style.cssText = 'width:36px;height:36px;margin:0;';
        overlay.appendChild(spinner.element);
      } catch (_) { overlay.textContent = 'Girando…'; }
      if (getComputedStyle(stage).position === 'static') stage.style.position = 'relative';
      stage.appendChild(overlay);
    }
    const cleanup = () => {
    ...
    try {
      ...
      if (!r.ok) { cleanup(); uiModule.showError('Falha ao girar'); return; }
      ...
      cleanup();
      uiModule.showToast('Girada');
      _fetchLibrary(false);
    } catch (e) {
      cleanup();
      uiModule.showError('Falha ao girar');
    }
  };
  ...
  document.getElementById('gallery-set-cover-btn')?.addEventListener('click', async () => {
    ...
    try {
      ...
      if (r.ok) {
        uiModule.showToast('Capa do álbum atualizada');
        await _fetchAlbums();
      } else {
        uiModule.showError('Falha ao definir capa');
      }
    } catch (e) {
      uiModule.showError('Falha ao definir capa');
    }
  ...
  document.getElementById('gallery-delete-btn').addEventListener('click', async () => {
    if (!await uiModule.styledConfirm('Excluir esta foto? Isto não pode ser desfeito.', { confirmText: 'Excluir', danger: true })) return;
    const ok = await _deleteImage(img.id);
    if (!ok) {
      uiModule.showError('Falha ao excluir foto');
      return;
    }
    detail.style.display = 'none';
    ...
    _renderGrid();
    _renderStats();
    if (uiModule) uiModule.showToast('Foto excluída');
  });
  ...
  if (_nameInput) {
    const _saveName = async () => {
      ...
      try {
        ...
        img.prompt = newName;
        if (uiModule) uiModule.showToast('Renomeada');
        window.dispatchEvent(new CustomEvent('gallery-refresh'));
      } catch {
        if (uiModule) uiModule.showError('Falha ao renomear');
      }
    };
  ...
  if (_tagInput) {
    ...
    const _addTags = async () => {
      ...
      const ok = await _patchImage(img.id, { tags: cleaned });
      if (!ok) { if (uiModule) uiModule.showError('Falha ao salvar tags'); return; }
      img.tags = cleaned;
      img.user_tags = cleaned;
      ...
      if (chips) {
        added.forEach(t => {
          ...
          b.title = `Filtrar fotos marcadas com “${t}”`;
          b.textContent = t;
          const x = document.createElement('span');
          x.className = 'gallery-tag-x';
          x.title = 'Remover tag';
          x.setAttribute('aria-label', 'Remover tag');
          x.textContent = '×';
          b.appendChild(x);
          chips.appendChild(b);
          _wireTagChip(b);
  ...
  document.getElementById('gallery-detail-album').addEventListener('change', async (e) => {
    ...
    if (!ok) { uiModule.showError('Falha ao atualizar álbum'); return; }
    img.album_id = albumId || null;
    uiModule.showToast(albumId ? 'Adicionado ao álbum' : 'Removido do álbum');
  });
}
...
export function openGallery() {
    ...
    <div class="modal-content gallery-modal-content">
      ...
      <div class="gallery-tabs">
        <button class="gallery-tab active" data-tab="images">
          ...
          <span class="gallery-tab-label">Fotos</span>
        </button>
        <button class="gallery-tab" data-tab="albums">
          ...
          <span class="gallery-tab-label">Álbuns</span>
        </button>
        <button class="gallery-tab" data-tab="editor" id="gallery-editor-tab">
          ...
          <span class="gallery-tab-label">Editar</span>
          <span class="gallery-tab-close" id="gallery-editor-tab-close" title="Fechar edição" aria-label="Fechar edição">×</span>
        </button>
        <button class="gallery-tab" data-tab="settings">
          ...
          <span class="gallery-tab-label">Configurações</span>
        </button>
      </div>
      <div class="modal-body">
        ...
        <div class="gallery-toolbar">
          <div class="gallery-search-wrap">
            <input type="text" class="gallery-search" id="gallery-search" placeholder="Buscar fotos, tags..." />
            <span class="gallery-search-enter-hint" aria-hidden="true"><svg class="gallery-enter-key" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 10 4 15 9 20"/><path d="M20 4v7a4 4 0 0 1-4 4H4"/></svg>para marcar</span>
          </div>
          ...
          <select class="gallery-sort" id="gallery-sort">
            <option value="shuffle">Aleatória</option>
            <option value="recent">Recentes</option>
            <option value="oldest">Antigas</option>
          </select>
          <button class="gallery-select-btn gallery-toolbar-action" id="gallery-select-btn" title="Selecionar para ações em massa"><span style="position:relative;top:1px;">Selecionar</span></button>
        </div>
        ...
        <div class="memory-bulk-bar hidden" id="gallery-bulk-bar" style="margin-bottom:4px;">
          <label class="memory-bulk-check-all" style="position:relative;top:-1px;"><input type="checkbox" id="gallery-bulk-select-all"> Todos</label>
          <span id="gallery-bulk-count" style="position:relative;top:-1px;">0 selecionados</span>
          ...
          <button class="memory-toolbar-btn" id="gallery-bulk-cancel" title="Cancelar (Esc)" style="margin-left:4px;padding:3px 6px;position:relative;top:-3px;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
        </div>
        <div class="gallery-tag-chips" id="gallery-tag-chips"></div>
        <div class="gallery-grid" id="gallery-grid"></div>
        <button class="gallery-load-more" id="gallery-load-more" style="display:none">Carregar mais</button>
        ...
        <div class="gallery-settings-container" id="gallery-settings-container" style="display:none;">
          <div class="admin-card">
            <h2>Marcação com IA <span id="gallery-tag-count" class="memory-count" style="font-size:0.6em;opacity:0.6;font-weight:normal;"></span></h2>
            <p class="memory-desc doclib-desc">Marque fotos automaticamente por conteúdo com seu <a href="#" id="gallery-vision-link" class="ge-vision-link">modelo de visão</a>. Suas tags pessoais são mantidas.</p>
            <div id="gallery-tag-bar" style="display:none;padding:8px 0 0;">
              ...
              <div style="display:flex;justify-content:space-between;align-items:center;margin-top:4px;">
                ...
                <button id="gallery-tag-cancel" class="gallery-select-btn" style="font-size:10px;padding:1px 6px;">Cancelar</button>
              </div>
            </div>
            <div class="memory-toolbar" style="display:flex;flex-direction:row;gap:6px;align-items:center;justify-content:space-between;flex-wrap:wrap;margin-top:32px;">
              <button class="memory-toolbar-btn" id="gallery-clear-ai-tags-btn" title="Remover todas as tags geradas por IA de cada foto">Limpar tags IA</button>
              <button class="memory-toolbar-btn" id="gallery-tag-all-btn" title="Marcar todas as fotos não marcadas com IA (no álbum atual, se houver)">
  ...
  document.getElementById('gallery-close').addEventListener('click', async () => {
    if (isEditorOpen()) {
      const ok = await uiModule.styledConfirm(
        'Fechar Galeria e a edição ativa?',
        { confirmText: 'Fechar', danger: true },
      );
      if (!ok) return;
  ...
  if (editorTabClose) {
    editorTabClose.addEventListener('click', async (e) => {
      ...
      if (isEditorOpen()) {
        const ok = await uiModule.styledConfirm(
          'Fechar a edição? Todas as alterações não salvas serão perdidas.',
          { confirmText: 'Fechar', danger: true },
        );
        if (!ok) return;
      }
  ...
  if (editorTab) {
    editorTab.addEventListener('dblclick', (e) => {
      ...
      const input = document.createElement('input');
      input.type = 'text';
      input.value = current === 'Editar' ? '' : current;
      input.placeholder = 'Nome da edição';
      ...
      const finish = (commit) => {
        if (commit && input.value.trim()) {
          labelEl.textContent = `Editar: ${input.value.trim().slice(0, 24)}`;
        } else {
