...
export async function openAssistantChat() {
  try {
    ...
    if (!info?.session_id) {
      uiModule.showToast('Sessão do assistente indisponível');
      return;
    }
    ...
    uiModule.showToast('Não foi possível abrir o assistente');
  }
}
...
async function _runCheckInNow(taskId) {
  try {
    ...
    uiModule.showToast('Check-in em execução…');
  } catch (e) {
    console.error(e);
    uiModule.showToast('Não foi possível executar o check-in');
  }
}
  ...
  modal.innerHTML = `
    <div class="modal-content" style="max-width:640px;width:96%;">
      ...
      <div class="modal-body" id="assistant-settings-body">
        <div class="hwfit-loading">Carregando…</div>
      </div>
    </div>`;
...
const TOOL_GROUPS = {
  'E-mail': ['list_emails', 'read_email', 'send_email', 'reply_to_email', 'archive_email', 'delete_email', 'mark_email_read'],
  'Agenda e notas': ['manage_calendar', 'manage_notes', 'manage_tasks'],
  'Conhecimento': ['web_search', 'read_file', 'manage_memory', 'manage_rag', 'search_chats'],
  'Código': ['bash', 'python', 'write_file'],
  'Documentos': ['create_document', 'edit_document', 'update_document', 'suggest_document'],
  'IA e modelos': ['chat_with_model', 'second_opinion', 'ask_teacher', 'pipeline', 'list_models', 'generate_image'],
  'Sistema': ['manage_session', 'manage_endpoints', 'manage_mcp', 'manage_settings', 'manage_skills', 'manage_webhooks', 'manage_tokens', 'manage_documents', 'create_session', 'list_sessions', 'send_to_session', 'ui_control'],
};
...
function _renderSettingsBody(body, data, tzList) {
  ...
  const checkInsHTML = checkIns.map((c) => `
    <div class="assistant-checkin-row" data-task-id="${_esc(c.id)}">
      <div class="assistant-checkin-head">
        <input type="checkbox" class="assistant-checkin-enabled" ${c.enabled ? 'checked' : ''} title="Ativar este check-in" />
        ...
        <button type="button" class="assistant-checkin-run" title="Executar agora">Executar agora</button>
      </div>
      ...
      <div class="assistant-checkin-meta">
        ${c.next_run ? `próxima execução: ${_esc(c.next_run)}` : ''}
        ${c.last_run ? ` · última execução: ${_esc(c.last_run)}` : ''}
        ${typeof c.run_count === 'number' ? ` · ${c.run_count} execuções` : ''}
      </div>
    </div>`).join('');
  ...
  body.innerHTML = `
    <div class="assistant-settings-form">
      ...
      <div class="assistant-field">
        <span style="display:flex;align-items:center;gap:8px;">Personalidade
        ...
        <textarea id="assistant-personality" rows="6" placeholder="Descreva a personalidade, o tom e o comportamento do assistente...">${_esc(crew.personality || '')}</textarea>
      </div>
      <div class="assistant-field-row">
        <label class="assistant-field">
          <span>Fuso horário</span>
          <select id="assistant-timezone">
            <option value=""${!crew.timezone ? ' selected' : ''}>(padrão -- UTC)</option>
            ${tzOptions}
          </select>
        </label>
      </div>
      <div class="assistant-field-row">
        ...
        <label class="assistant-field" style="flex:1;">
          ...
          <select id="assistant-model" style="width:100%;">
            <option value="${_esc(crew.model || '')}">${_esc(crew.model || '(padrão)')}</option>
          </select>
        </label>
      </div>
      <div class="assistant-field">
        <span style="display:flex;align-items:center;gap:8px;">Ferramentas
          ...
          <button type="button" id="assistant-tools-none" class="assistant-tools-toggle" style="font-size:10px;opacity:0.5;cursor:pointer;background:none;border:1px solid var(--border);border-radius:3px;padding:1px 6px;">nenhuma</button>
        </span>
      ...
      <div class="assistant-checkins">
        <h5>Check-ins diários</h5>
        ${checkInsHTML || '<div style="opacity:0.6;">Nenhum check-in configurado.</div>'}
      </div>
      <div class="assistant-settings-actions">
        <button type="button" class="cal-btn" id="assistant-settings-cancel">Cancelar</button>
        <button type="button" class="cal-btn cal-btn-primary" id="assistant-settings-save">Salvar</button>
      </div>
    </div>
  `;
  ...
  _fetchEndpoints().then(endpoints => {
    let epHTML = '<option value="">(usar padrão da sessão)</option>';
    ...
    epSelect.addEventListener('change', async () => {
      ...
      if (!url) { modelSelect.innerHTML = '<option value="">(padrão)</option>'; return; }
      ...
      try {
        ...
        modelSelect.innerHTML = mHTML || '<option value="">(nenhum modelo)</option>';
      } catch { modelSelect.innerHTML = '<option value="">(falhou)</option>'; }
    });
  ...
  if (charPick && personalityEl) {
    (async () => {
      try {
        ...
        let opts = '<option value="">-- escolher persona --</option>';
        if (allPresets.length) {
          opts += '<optgroup label="Predefinições">';
          for (const p of allPresets) {
            if (!p.system_prompt) continue;
            const name = p.character_name || p.name || p._key || 'Sem nome';
        ...
        if (allTemplates.length) {
          ...
          for (const t of allTemplates) {
            ...
            const name = t.character_name || t.name || 'Sem nome';
  ...
  body.querySelector('#assistant-settings-save').addEventListener('click', async () => {
    ...
    try {
      ...
      uiModule.showToast('Configurações do assistente salvas');
      _closeModal();
    } catch (e) {
      console.error(e);
      uiModule.showToast('Falha ao salvar');
    }
  });
...
export async function openAssistantSettings() {
  ...
  body.innerHTML = '<div class="hwfit-loading">Carregando…</div>';
  try {
    const [data, tzList] = await Promise.all([_getSettings(true), _listTimezones()]);
    _renderSettingsBody(body, data, tzList);
  } catch (e) {
    console.error(e);
    body.innerHTML = '<div style="padding:12px;opacity:0.6;">Não foi possível carregar as configurações do assistente.</div>';
  }
}
...
async function _ensureHeaderAffordances(sessionId) {
  ...
  gear.type = 'button';
  gear.title = 'Configurações do assistente';
