...
export async function refreshModels(force = false) {
  ...
  try {
    ...
    if (!_cachedItems || _cachedItems.length === 0) {
      ...
      if (welcomeSub) welcomeSub.innerHTML = 'Digite <span class="setup-trigger-link" style="color:var(--accent,var(--red));font-weight:600;cursor:pointer;text-decoration:underline;" title="Clique para iniciar a configuração">/setup</span> para começar.';
      ...
      const welcomeSub = document.getElementById('welcome-sub');
      if (welcomeSub) welcomeSub.textContent = 'Pronto para a jornada.';
      const welcomeTip = document.getElementById('welcome-tip');
      if (welcomeTip) {
        const tips = window.innerWidth <= 768
          ? [
              'Dica: Toque e segure numa sessão para renomear, excluir ou ver opções de memória.',
              'Dica: Toque no ícone de olho para o modo Ninguém - sem histórico salvo.',
              'Dica: Mude para o modo Agente quando precisar de ferramentas.',
              'Dica: Anexe imagens ou arquivos usando o botão + ao lado do campo de mensagem.',
            ]
          : [
              'Dica: Pressione Ctrl+K para pesquisar em todas as suas conversas.',
              'Dica: Pressione Ctrl+B para alternar a barra lateral rapidamente.',
              'Dica: Shift+clique no botão da barra lateral para trocá-la de lado.',
              'Dica: Arraste e solte arquivos no chat para anexá-los.',
              'Dica: Clique com o botão direito numa sessão para renomear, excluir ou ver opções de memória.',
            ];
