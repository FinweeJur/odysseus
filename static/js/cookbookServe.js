...
function _serveBackendWarning(model, repo, backend, fields = {}) {
  ...
  if (awqLike && (backend === 'llamacpp' || backend === 'ollama')) {
    return {
      title: 'AWQ precisa de vLLM ou SGLang',
      body: 'Este modelo parece ser AWQ/GPTQ/FP8 safetensors. llama.cpp e Ollama precisam de arquivos GGUF, então este backend não consegue servi-lo. Escolha vLLM/SGLang em um servidor GPU CUDA/ROCm, ou baixe uma versão GGUF para llama.cpp/Ollama.',
    };
  }
  if (awqLike && _isMetal() && (backend === 'vllm' || backend === 'sglang')) {
    return {
      title: 'AWQ não é um caminho de memória unificada',
      body: 'Este modelo parece ser AWQ/GPTQ/FP8 safetensors. AWQ é para vLLM/SGLang em servidores GPU estilo CUDA/ROCm, não para serving local de memória unificada via llama.cpp/Ollama. Para memória unificada, baixe um modelo GGUF e use llama.cpp/Ollama.',
    };
  }
  if (awqLike && fields.unified_mem) {
    return {
      title: 'AWQ não é um caminho de memória unificada',
      body: 'Este modelo parece ser AWQ/GPTQ/FP8 safetensors, mas o serving local de memória unificada espera GGUF. Use vLLM/SGLang em um servidor GPU compatível, ou baixe uma versão GGUF para llama.cpp/Ollama.',
    };
  }
  if (ggufLike && (backend === 'vllm' || backend === 'sglang')) {
    return {
      title: 'GGUF precisa de llama.cpp ou Ollama',
      body: 'Este modelo parece ser GGUF. vLLM/SGLang esperam repositórios estilo HuggingFace safetensors. Escolha llama.cpp/Ollama para GGUF, ou baixe um modelo safetensors para vLLM/SGLang.',
    };
  }
  return null;
}
...
function _runtimeNoteText(backend, pkg, target) {
  ...
  if (!pkg) return `Disponibilidade de ${label} indisponível para ${target.label}.`;
  const note = pkg.status_note || pkg.update_note || '';
  if (pkg.installed) {
    return note ? `${label} pronto em ${target.label}: ${note}` : `${label} pronto em ${target.label}.`;
  }
  return note ? `${label} ausente em ${target.label}: ${note}` : `${label} ausente em ${target.label}.`;
}
...
function _rerenderCachedModels() {
  ...
  for (const m of allModels) {
    ...
    if (m.status === 'downloading') {
      ...
      metaParts.push(`<span class="cookbook-dl-status" style="color:var(--accent,var(--red));">${_active ? 'baixando' : 'download travado'}</span>`);
    }
    ...
    const _runningPill = _isActivelyServing(m.repo_id)
      ? ' <span class="cookbook-serve-running-pill" title="Este modelo está sendo servido agora">em execução</span>'
      : '';
    ...
    html += `<div class="memory-item-actions"><button type="button" class="memory-item-btn hwfit-cached-menu-btn" title="Ações" aria-label="Ações do modelo"><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="5" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="12" cy="19" r="2"/></svg></button></div>`;
    html += `</div>`;
  }
  if (!visibleCount) html += '<div class="hwfit-loading">Nenhum modelo encontrado</div>';
  list.innerHTML = html;
  ...
  list.querySelectorAll('.hwfit-cached-menu-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      if (m && m.status === 'downloading') items.push({ label: 'Tentar novamente', icon: _retryIco, action: 'retry' });
      items.push({ label: 'Selecionar', icon: _selectIco, action: 'select' });
      items.push({ label: 'Excluir', icon: _deleteIco, action: 'delete', danger: true });
  ...
  list.querySelectorAll('.memory-item[data-repo]').forEach(item => {
    item.addEventListener('click', (e) => {
      ...
      const _arrowTitle = _modelPresets.length > 0
        ? `${_modelPresets.length} configuração${_modelPresets.length === 1 ? '' : 'ões'} de lançamento salva${_modelPresets.length === 1 ? '' : 's'} para ${_repoShort} — clique em ▾ para carregar ou excluir`
        : `Nenhuma configuração de lançamento salva para ${_repoShort} ainda — clique em Salvar para adicionar uma`;
      let _slotsHtml = `<div class="cookbook-serve-slots cookbook-saved-split">`
        + `<button type="button" class="cookbook-slot-btn cookbook-saved-save" title="Salvar configuração atual"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg>Salvar</button>`
      ...
      if (m && (m.status === 'downloading' || m.status === 'stalled' || m.has_incomplete)) {
        const _warnText = m.status === 'stalled'
          ? `Este modelo parece ser uma casca de download travado (${esc(m.size || '0 KB')}). Os pesos não estão no disco — o serviço vai falhar ao carregar. Baixe de novo primeiro, ou escolha outro modelo.`
          : `O download deste modelo ainda não terminou (${esc(m.size || 'partial')}). O serviço vai iniciar mas é provável que trave por falta de algum shard. Espere o download terminar, ou relance depois que ele acabar.`;
      ...
      // reflects the stored presets. Visual padrão .dropdown do Cutia, posicionado
