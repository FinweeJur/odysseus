...
export function notifyStreamComplete(sessionId, query) {
  ...
  var body = query ? 'A resposta para "' + query.substring(0, 60) + '" está pronta' : 'Sua resposta do chat foi concluída';
  var notification = new Notification('Resposta Concluída', {
    body: body,
...
export function insertStreamDoneToast(sessionId, query) {
  ...
  var name = sess ? sess.name : 'outra sessão';
  ...
  div.innerHTML = '<div class="body">'
    ...
    + '<span>Resposta pronta em <strong>' + (name || 'sessão').replace(/</g, '&lt;') + '</strong>'
...
export function notifyResearchComplete(sessionId, query) {
  ...
  if (!isHidden && !isOtherSession) return;
  if (!('Notification' in window) || Notification.permission !== 'granted') return;
  var body = query ? 'A pesquisa sobre "' + query.substring(0, 60) + '" está pronta' : 'Sua pesquisa profunda foi concluída';
  var notification = new Notification('Pesquisa Concluída', {
    body: body,
