...
export function _renderGpuToggles(system) {
  ...
  let html = '';
  if (heterogeneous) {
    html += `<select class="hwfit-gpu-group" id="hwfit-gpu-group" title="De qual pool de GPU servir — o vLLM só faz tensor-parallel entre GPUs idênticas">`;
    groups.forEach((g, i) => {
  ...
  html += '<button class="hwfit-gpu-btn" data-count="0" title="Apenas CPU / RAM">RAM</button>';
...
function _hwfitShowError(list, host, detail) {
  if (!list) return;
  const where = host ? esc(host) : 'esta máquina';
  const div = document.createElement('div');
  div.className = 'hwfit-loading';
  div.style.cssText = 'flex-direction:column;gap:8px;text-align:center;';
  div.innerHTML =
    `<div style="color:var(--red);font-weight:600;">Não foi possível escanear ${where}</div>`
    + (detail ? `<div style="opacity:0.6;font-size:11px;max-width:340px;line-height:1.4;">${esc(detail)}</div>` : '')
    + `<button type="button" class="hwfit-gpu-btn" id="hwfit-retry" style="margin-top:2px;height:26px;">↻ Tentar novamente</button>`;
  list.innerHTML = '';
...
export async function _hwfitFetch(fresh = false) {
  ...
  if (_cached) {
    ...
    loadingLbl.textContent = 'Carregando…';
    loadingLbl.style.cssText = 'text-align:center;opacity:0.5;font-size:11px;';
    loadingDiv.appendChild(loadingLbl);
    setTimeout(() => { if (loadingLbl.isConnected) loadingLbl.textContent = 'Escaneando hardware…'; }, 2000);
    list.innerHTML = '';
        ...
        list.querySelectorAll('.hwfit-row[data-model]').forEach(row => {
          ...
          if (_cachedModelIds.has(name) || [..._cachedModelIds].some(id => id.endsWith('/' + name?.split('/').pop()))) {
            ...
            if (nameEl && !nameEl.querySelector('.hwfit-dl-dot')) {
              nameEl.insertAdjacentHTML('beforeend', '<span class="hwfit-dl-dot" title="Baixado">\u25CF</span>');
            }
          }
        });
      }).catch(() => {});
...
export function _hwfitRenderHw(el, sys) {
  ...
  const chip = (key, label, title = 'Clique para desativar (X para esconder)') => {
    ...
    return (
      ...
      + `<button type="button" class="hwfit-hw-chip-x" data-hw-chip="${esc(key)}" title="Remover este chip" aria-label="Remover">×</button>`
      + `</span>`
    );
  };
  let gpuChip;
  if (sys.gpu_name) {
    ...
    const tip = gpus.length
      ...
      : 'Clique para desativar (X para esconder)';
    ...
    gpuChip = _removedHwChips.has('gpu')
      ...
      : (() => {
          ...
          return (
            ...
            + `<button type="button" class="hwfit-hw-chip-toggle" data-hw-chip="gpu" title="${esc(sys.gpu_error)}">Erro de driver da GPU</button>`
            + `<button type="button" class="hwfit-hw-chip-x" data-hw-chip="gpu" title="Remover este chip" aria-label="Remover">×</button>`
            + `</span>`
          );
        })();
  } else {
    gpuChip = chip('gpu', 'Sem GPU');
  }
  ...
  const manualChip = (sys.manual_hardware || manual)
    ? `<span class="hwfit-hw-chip hwfit-hw-chip-row hwfit-hw-chip-manual" data-hw-chip="manual">`
      + `<button type="button" class="hwfit-hw-chip-toggle" data-hw-chip="manual" title="Usando hardware manual">${esc(_manualHwLabel(manual) || 'Hardware manual')}</button>`
      + `<button type="button" class="hwfit-hw-chip-x" data-hw-chip="manual" title="Limpar hardware manual" aria-label="Limpar">×</button>`
      + `</span>`
    : '';
...
export function _hwfitRenderList(el, models) {
  ...
  for (const m of models) {
    ...
    const dlDot = (_cachedModelIds && (_cachedModelIds.has(m.name) || [..._cachedModelIds].some(id => id === m.name?.split('/').pop()))) ? '<span class="hwfit-dl-dot" title="Baixado">\u25CF</span>' : '';
...
export function _expandModelRow(row, modelData) {
  ...
  if (quickRunBtn) {
    quickRunBtn.addEventListener('click', async () => {
      ...
      quickRunBtn.textContent = 'Iniciando...';
      ...
      quickRunBtn.textContent = 'Executar';
    });
  }
...
export function _hwfitInit() {
  ...
  async function _populateServerKeyPanel(entry, generate = false) {
    ...
    if (genBtn) {
      ...
      genBtn.textContent = generate ? 'Gerando...' : 'Carregando...';
    }
    try {
      ...
      if (genBtn) genBtn.textContent = 'Chave pronta';
    } catch (e) {
      cmdBox.value = e.message || String(e);
      if (copyBtn) copyBtn.disabled = true;
      if (genBtn) genBtn.textContent = 'Gerar chave';
    } finally {
  ...
  function _wireServerEntry(entry) {
    ...
    if (_defBtn && !_defBtn.dataset.bound) {
      ...
      _defBtn.addEventListener('click', (e) => {
        ...
        document.querySelectorAll('.cookbook-srv-default').forEach(b => {
          ...
          b.innerHTML = (on ? _MODELDIR_CHECK_ON : _MODELDIR_CHECK_OFF) + '<span class="cookbook-srv-default-label">padrão</span>';
          b.title = on ? 'Servidor padrão — Cutia abre aqui' : 'Tornar este o servidor padrão';
        });
    ...
    if (saveBtn && !saveBtn.dataset.bound) {
      ...
      saveBtn.addEventListener('click', () => {
        ...
        saveBtn.innerHTML = '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#50fa7b" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round" style="margin-right:4px;flex-shrink:0;"><polyline points="20 6 9 17 4 12"/></svg>Salvo';
      });
    }
    ...
    if (setupBtn) {
      setupBtn.addEventListener('click', async () => {
        ...
        setupBtn.textContent = 'Instalando...';
        try {
