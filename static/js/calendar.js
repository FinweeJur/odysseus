  ...
  return new Promise(resolve => {
    ...
    input.addEventListener('change', async () => {
      ...
      try {
        ...
        if (!fileId) throw new Error('Falha no envio');
...
async function _fetchEvents(start, end, force) {
  ...
  const fetchPromise = fetch(`${API_BASE}/api/calendar/events?start=${start}&end=${end}`, { credentials: 'same-origin' })
    ...
    .catch(e => { console.error('Calendário: falha ao buscar eventos', e); });
...
async function _fetchCalendars() {
  ...
  } catch (e) { _calendars = []; _calendarsError = e.message || 'Falha na conexão'; }
...
async function _syncCaldav(interactive) {
  try {
    ...
    if (interactive) return { errors: [e.message || 'Falha na sincronização'] };
  }
}
...
async function _createEvent(data) {
  ...
  fetch(`${API_BASE}/api/calendar/events`, {
    ...
    delete _allEvents[tempUid];
    if (_open) _render();
    if (window.uiModule) window.uiModule.showError('Falha ao criar evento: ' + (e?.message || 'unknown'));
  });
  return { uid: tempUid };
}
...
async function _updateEvent(uid, data) {
  ...
  fetch(`${API_BASE}/api/calendar/events/${encodeURIComponent(uid)}`, {
    ...
    if (_open) _render();
    if (window.uiModule) window.uiModule.showError('Falha ao atualizar evento: ' + (e?.message || 'unknown'));
  });
  return { ok: true };
}
...
async function _deleteEvent(uid) {
  ...
  fetch(`${API_BASE}/api/calendar/events/${encodeURIComponent(uid)}`, {
    ...
    if (window.uiModule) window.uiModule.showError('Falha ao excluir evento: ' + (e?.message || 'unknown'));
    if (_open) _render();
  });
  return { ok: true };
}
...
function _showEventMoreMenu(ev, anchor) {
  ...
  dropdown.appendChild(_item(_trashIcon, 'Excluir', async () => {
    closeMenu();
    const name = ev.summary ? `"${ev.summary}"` : 'este evento';
    const ok = await uiModule.styledConfirm(`Excluir ${name}?`, { confirmText: 'Excluir', danger: true });
    if (!ok) return;
...
async function _createEventReminder(ev, dueDate) {
  ...
  try {
    ...
    if (uiModule.showToast) uiModule.showToast(`Lembrete definido para ${fmt}`);
    try { window.notesModule?.refreshDueBadge?.({ force: true }); } catch {}
    if ('Notification' in window && Notification.permission === 'default') {
      try { Notification.requestPermission(); } catch {}
    }
  } catch (e) {
    if (uiModule.showError) uiModule.showError('Falha ao criar lembrete');
  }
}
...
function _updateBadge() {
  ...
  if (count > 0 && !_isBadgeSeenToday()) {
    ...
    badge.title = `${count} evento${count > 1 ? 's' : ''} hoje`;
...
function _renderEmpty() {
  ...
  body.innerHTML = `
    <div class="cal-empty-state">
      ...
      <div class="cal-empty-title">${hasError ? 'Calendário indisponível' : 'Nenhum calendário ainda'}</div>
      <div class="cal-empty-msg">${hasError ? _e(_calendarsError) : 'Crie um calendário local, importe um arquivo .ics ou sincronize via CalDAV.'}</div>
      ${hasError ? `
        <button class="cal-btn cal-btn-primary" id="cal-goto-settings">Abrir Configurações</button>
      ` : `
        <div style="display:flex;gap:8px;justify-content:center;flex-wrap:wrap;margin-top:4px;">
          <button class="cal-btn cal-btn-primary" id="cal-empty-new">Novo calendário</button>
          <button class="cal-btn" id="cal-empty-import">Importar .ics</button>
        </div>
        <div style="margin-top:10px;font-size:11px;opacity:0.55;">Ou <a href="#" id="cal-empty-caldav" style="color:var(--accent, var(--red));text-decoration:none;font-weight:600;">configure a sincronização CalDAV</a>.</div>
      `}
    </div>`;
...
function _headerHTML() {
  ...
  <div class="cal-quickadd-row" id="cal-quickadd-row">
    <input
      ...
      autocomplete="off"
    />
    <span class="cal-quickadd-hint" id="cal-quickadd-hint" aria-hidden="true"><span class="qa-hint-accent">Adição rápida</span> — voltar para casa em Ítaca 13h amanhã <svg class="qa-hint-enter" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="9 10 4 15 9 20"/><path d="M20 4v7a4 4 0 0 1-4 4H4"/></svg></span>
...
function _filtersData() {
  ...
  if (hasUntagged) {
    ...
    typeFilters += `<label class="cal-filter-item${off ? ' cal-filter-off' : ''}" data-type="__untagged__">
      <span class="cal-filter-dot" style="background:${_TYPE_PALETTE.untagged}"></span>sem tag</label>`;
  }
...
function _filtersToggleHTML() {
  ...
  return `<button class="cal-filter-toggle" id="cal-filter-toggle" title="${_filtersCollapsed ? 'Mostrar filtros' : 'Ocultar filtros'}">${_filtersCollapsed ? '+ tags' : '− tags'}</button>`;
}
    ...
    for (let col = 0; col < 7; col++) {
      ...
      if (singles.length) {
        ...
        for (const ev of showInline) {
          ...
          const _impMark = ev.importance === 'critical' ? '<span style="color:var(--red);margin-right:2px" title="crítico">!!</span>'
                         : ev.importance === 'high' ? '<span style="color:var(--orange,#e5a33a);margin-right:2px" title="alta">!</span>' : '';
        ...
        if (singles.length > maxInline) h += `<div class="cal-event-more">+${singles.length - maxInline} mais</div>`;
      }
      h += '</div>';
    }
...
function _wkFormatHourLabel(h) {
  ...
  const ampm = h < 12 ? 'AM' : 'PM'; // mantido — formato de hora segue preferência do SO
  const hh = ((h + 11) % 12) + 1;
  return `${hh} ${ampm}`;
}
...
async function _renderWeek() {
  ...
  let railHtml = `<div class="cal-wk-rail">
    <div class="cal-wk-rail-spacer">
      <button class="cal-wk-zoom" id="cal-wk-zoom-out" title="Diminuir zoom (–)" aria-label="Diminuir zoom">−</button>
      <button class="cal-wk-zoom" id="cal-wk-zoom-in" title="Aumentar zoom (+)" aria-label="Aumentar zoom">+</button>
    </div>`;
  ...
  for (const { d, ds, idx } of days) {
    ...
    for (const ev of timedEvents) {
      ...
      colsHtml += `<div class="cal-wk-block-resize" title="Arraste para redimensionar"></div>`;
      colsHtml += `</div>`;
    }
        ...
        try {
          ...
          _showCalUndoToast('Evento movido', async () => {
            try {
              ...
              _render();
            } catch (err) { console.error('Falha ao desfazer:', err); }
          });
        } catch {
          _render();
        }
...
async function _renderAgenda() {
  ...
  if (!dates.length) {
    ...
    h += '<div class="cal-empty" style="display:flex;align-items:center;justify-content:center;gap:10px;flex-wrap:wrap;">' +
      '<span>Nenhum evento futuro</span>' +
      '<span style="opacity:0.7;font-size:11px;">' +
        '<a href="#" data-cal-open-settings="integrations" style="color:var(--accent,var(--red));text-decoration:underline;">Configurações &rsaquo; Integrações</a>' +
        ' &middot; ' +
        '<a href="#" data-cal-create-event="1" style="color:var(--accent,var(--red));text-decoration:underline;">Criar evento</a>' +
      '</span>' +
    '</div>';
  } else {
    for (const date of dates) {
      const evs = byDate.get(date);
      const todayBadge = (date === today) ? ' <span class="cal-agenda-today-badge">Hoje</span>' : '';
      h += `<div class="cal-agenda-day${date === today ? ' is-today' : ''}"><div class="cal-agenda-date">${_fmtDate(date)}${todayBadge}</div>`;
      if (!evs.length) {
        h += '<div class="cal-agenda-empty">Sem eventos</div>';
      }
      for (const ev of evs) {
        const t = ev.all_day ? 'Dia inteiro' : _fmtTime(ev.dtstart) + ' – ' + _fmtTime(ev.dtend);
        const _typeTag = ev.event_type
          ...
          : '';
        const _impMark = ev.importance === 'critical' ? '<span style="color:var(--red);margin-right:4px" title="crítico">!!</span>'
                       : ev.importance === 'high' ? '<span style="color:var(--orange,#e5a33a);margin-right:4px" title="alta">!</span>' : '';
        h += `<div class="cal-agenda-event" data-uid="${_e(ev.uid)}">
          ...
          <button class="cal-event-more" data-uid="${_e(ev.uid)}" title="Mais">${_moreIcon}</button>
        </div>`;
      }
      h += '</div>';
    }
...
async function _renderSearch() {
  ...
  h += `<div class="cal-search-count">${results.length} resultado${results.length !== 1 ? 's' : ''} para "${_e(_searchQuery)}"</div>`;
  if (!results.length) {
    h += '<div class="cal-empty">Nenhum evento corresponde à sua busca</div>';
  } else {
...
async function _renderYear() {
  ...
  for (let m = 0; m < 12; m++) {
    ...
    for (const wd of ['S', 'T', 'Q', 'Q', 'S', 'S', 'D']) h += `<div class="cal-year-wd">${wd}</div>`;
    const first = new Date(y, m, 1);
    const dow = (first.getDay() + 6) % 7;
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      h += `<div class="${cls}" data-date="${ds}" title="${evs.length ? evs.length + (evs.length > 1 ? ' eventos' : ' evento') : ''}">${d}</div>`;
    }
    h += '</div></div>';
  }
  h += '</div>';
...
function _dayDetailHTML(dateStr) {
  ...
  const searchInput = `<div class="cal-search-wrap">
    ...
    <input type="search" class="cal-search-input cal-day-search" id="cal-search" placeholder="Pesquisar todos os eventos…" value="${_e(_searchQuery)}" />
  </div>`;
  let h = `<div class="cal-splitter" role="separator" aria-orientation="horizontal" tabindex="0" title="Arraste para redimensionar"><div class="cal-splitter-grip"></div></div>
    <div class="cal-day-detail">
    ${searchInput}
    <div class="cal-detail-header">
      <span>${_fmtDate(dateStr)}${isToday ? ' <span style="color:var(--accent, var(--red));font-weight:600;">(Hoje)</span>' : ''}</span>
      <button class="cal-add-btn cal-add-btn-text cal-add-btn-sm" id="cal-add-day" title="Novo evento"><span class="cal-add-plus">+</span><span class="cal-add-label">Novo</span></button>
    </div>`;
  if (_searchQuery) {
    const q = _searchQuery.toLowerCase();
    ...
    if (!results.length) {
      h += '<div class="cal-empty">Nenhum evento corresponde</div>';
    } else {
      results.forEach(ev => {
...
function _wireAll(body) {
    ...
    const _submitQA = async () => {
      ...
      if (_qaStatus) {
        ...
        try {
          ...
          _qaSpinTimer = setTimeout(() => { if (_qaStatus) _qaStatus.textContent = 'analisando…'; }, 250);
        }
      }
      try {
        ...
        if (!res.ok || !data.ok) {
          ...
          uiModule.showError('Adição rápida: ' + (data.error || data.detail || `HTTP ${res.status}`));
          return;
        }
        ...
        uiModule.showError('Falha na adição rápida: ' + e.message);
      } finally {
        _qaSubmitting = false;
  ...
  document.getElementById('cal-sync')?.addEventListener('click', async () => {
    ...
    try {
      ...
      if (uiModule?.showToast) uiModule.showToast('Calendário atualizado');
    }
  });
  ...
  body.querySelectorAll('[data-date]').forEach(cell => {
    ...
    cell.addEventListener('drop', async (e) => {
      ...
      _render();
      uiModule.showToast?.('Movido', { duration: 4000, action: 'Desfazer', actionHint: 'Ctrl+Z', onAction: _popAndRunCalUndo });
    });
  });
}
...
async function _showCalSettings() {
  ...
  overlay.innerHTML = `
    <div class="modal-content" style="width:420px;max-width:92vw;">
      <div class="modal-header">
        <h4>Configurações do calendário</h4>
      ...
      <div class="modal-body" style="padding:16px;display:flex;flex-direction:column;gap:16px;">
        <div>
          <div style="font-size:11px;opacity:0.5;margin-bottom:6px;">Seus calendários</div>
        ...
        <div style="border-top:1px solid var(--border);padding-top:12px;">
          <div style="font-size:11px;opacity:0.5;margin-bottom:6px;">Importar calendário</div>
          ...
          <div style="font-size:10px;opacity:0.4;margin-top:4px;">Envie um arquivo .ics para importar eventos. Google Calendar, Apple Calendar e Outlook exportam arquivos .ics.</div>
        </div>
        <div style="border-top:1px solid var(--border);padding-top:12px;">
          <div style="font-size:11px;opacity:0.5;margin-bottom:6px;">Exportar calendário</div>
          <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;">
            ${cals.map(c => `
              <button class="memory-toolbar-btn cal-s-export-chip" data-id="${_e(c.href)}" title="Baixar ${_e(c.name)}.ics" style="cursor:pointer;">
          ...
          <div style="font-size:10px;opacity:0.4;margin-top:4px;">Baixe um calendário como .ics para backup ou para importar em outro app.</div>
        </div>
        <div style="border-top:1px solid var(--border);padding-top:12px;">
          ...
          <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
            <button class="memory-toolbar-btn" id="cal-settings-sync-now" style="cursor:pointer;">
              ...
              <span style="position:relative;top:1px;">Sincronizar agora</span>
            </button>
            <span id="cal-settings-sync-status" style="font-size:11px;opacity:0.6;"></span>
          </div>
          <div style="font-size:10px;opacity:0.4;margin-top:4px;">Busca eventos do seu servidor CalDAV. Para conectar ou alterar as credenciais do CalDAV, abra <a href="#" id="cal-settings-open-caldav" style="color:var(--accent, var(--red));text-decoration:none;font-weight:600;">Configurações → Integrações</a>.</div>
        </div>
      </div>
    </div>
  `;
  ...
  overlay.querySelector('#cal-settings-add')?.addEventListener('click', async (e) => {
    ...
    try {
      const r = await fetch(`${API_BASE}/api/calendar/calendars?name=${encodeURIComponent('Novo calendário')}&color=${encodeURIComponent(color)}`, { method: 'POST', credentials: 'same-origin' });
      const d = await r.json().catch(() => ({}));
      if (!r.ok || !d.ok) throw new Error(d.error || 'Falha ao criar calendário');
      ...
      btn.disabled = false;
      if (window.showError) window.showError(err.message || 'Falha ao criar calendário');
  ...
  overlay.querySelectorAll('.cal-settings-row').forEach(row => {
    ...
    const save = () => {
      ...
      saveTimer = setTimeout(async () => {
        ...
        if (uiModule?.showToast) uiModule.showToast(`Salvo “${nameInput.value || 'calendário'}”`);
    ...
    delBtn.addEventListener('click', async () => {
      ...
      if (!await window.styledConfirm(`Excluir o calendário "${name}" e todos os seus eventos?`, { confirmText: 'Excluir', danger: true })) return;
  ...
  overlay.querySelector('#cal-import-file').addEventListener('change', async (e) => {
    ...
    try {
      ...
      if (res.ok && data && data.ok) {
        status.textContent = `${data.imported} eventos importados para "${data.calendar}"` + (data.skipped ? ` (${data.skipped} ignorados)` : '');
        _allEvents = {}; _fetchedRanges = []; localStorage.removeItem(LS_KEY);
        await _fetchCalendars();
        _render();
      } else {
        ...
        status.textContent = `Falha na importação: ${reason}`;
        console.error('Falha na importação do calendário', res.status, data || raw);
      }
    } catch (err) {
      status.textContent = `Falha na importação: ${err.message || err}`;
      console.error('Erro na importação do calendário', err);
    }
    e.target.value = '';
  });
  ...
  overlay.querySelector('#cal-settings-sync-now')?.addEventListener('click', async (e) => {
    ...
    if (data.errors && data.errors.length) {
      status.textContent = `Falha na sincronização: ${data.errors[0]}`;
    } else {
      const parts = [];
      if (data.events) parts.push(`${data.events} eventos`);
      if (data.deleted) parts.push(`${data.deleted} removidos`);
      status.textContent = parts.length ? `Sincronizado — ${parts.join(', ')}` : 'Sincronizado — sem alterações';
      _allEvents = {}; _fetchedRanges = [];
      try { localStorage.removeItem(LS_KEY); } catch (_) {}
      await _fetchCalendars();
      _render();
...
function _showEventForm(existing, defaultDate, defaultEndDate) {
  ...
  body.innerHTML = `<div class="cal-form cal-form-bespoke${_expandedAtStart ? ' is-expanded' : ''}">
    <button type="button" class="cal-form-mobile-cancel" id="cal-form-mobile-cancel" title="Cancelar" aria-label="Cancelar evento">
    ...
    <div class="cal-form-today" id="cal-form-today">Hoje é <span id="cal-form-today-text">${_clockDate(_today())} · ${_nowClock()}</span></div>
    <div class="cal-hero">
      <button type="button" class="cal-hero-time" id="cal-hero-time" title="Alterar horário">
      ...
      <button type="button" class="cal-hero-date" id="cal-hero-date" title="Alterar data">${_clockDate(ds)}</button>
    </div>

    <div class="cal-title-wrap">
      <input type="text" id="cal-f-sum" placeholder=" " value="${_e(existing?.summary || '')}" class="cal-input cal-hero-title" autocomplete="off" />
      <span class="cal-title-hint" aria-hidden="true">${isEdit ? 'Título do evento' : 'O que está acontecendo?'}<svg class="cal-title-enter-ico" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 10 4 15 9 20"/><path d="M20 4v7a4 4 0 0 1-4 4H4"/></svg></span>
    </div>
    ...
    <div class="cal-form-details" id="cal-form-details" aria-hidden="${_expandedAtStart ? 'false' : 'true'}">
      <div class="cal-form-row">
        ...
        <span style="opacity:0.3">até</span>
      ...
      <div class="cal-loc-row">
        <input type="text" id="cal-f-loc" placeholder="Local" value="${_e(existing?.location || '')}" class="cal-input" />
        <a id="cal-f-loc-map" class="cal-loc-map" href="#" target="_blank" rel="noopener noreferrer" title="Abrir no Maps" aria-label="Abrir no Apple Maps" tabindex="-1">
      ...
      <select id="cal-f-rrule" class="cal-input">
        <option value="" ${!existing?.rrule ? 'selected' : ''}>Não repete</option>
        <option value="FREQ=DAILY" ${existing?.rrule === 'FREQ=DAILY' ? 'selected' : ''}>Diariamente</option>
        <option value="FREQ=WEEKLY" ${existing?.rrule === 'FREQ=WEEKLY' ? 'selected' : ''}>Semanalmente</option>
        <option value="FREQ=WEEKLY;BYDAY=MO,TU,WE,TH,FR" ${existing?.rrule === 'FREQ=WEEKLY;BYDAY=MO,TU,WE,TH,FR' ? 'selected' : ''}>Dias de semana</option>
        <option value="FREQ=MONTHLY" ${existing?.rrule === 'FREQ=MONTHLY' ? 'selected' : ''}>Mensalmente</option>
        <option value="FREQ=YEARLY" ${existing?.rrule === 'FREQ=YEARLY' ? 'selected' : ''}>Anualmente</option>
      </select>
      <textarea id="cal-f-desc" placeholder="Descrição" class="cal-input" rows="2">${_e(existing?.description || '')}</textarea>
      <div class="cal-form-row" style="align-items:center;gap:8px;">
        <label style="font-size:11px;display:flex;align-items:center;gap:4px;"><svg class="cal-remind-bell" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="var(--accent, var(--red))" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg><span style="opacity:0.5;">Lembrete</span></label>
        <select id="cal-f-remind" class="cal-input" style="flex:1;">
          <option value="" ${isEdit ? 'selected' : ''}>Sem lembrete</option>
          <option value="0">Na hora do evento</option>
          <option value="5">5 minutos antes</option>
          <option value="10">10 minutos antes</option>
          <option value="15" ${!isEdit ? 'selected' : ''}>15 minutos antes</option>
          <option value="30">30 minutos antes</option>
          <option value="60">1 hora antes</option>
          <option value="120">2 horas antes</option>
          <option value="1440">1 dia antes</option>
          <option value="custom">Hora exata...</option>
        </select>
        <input type="datetime-local" id="cal-f-remind-custom" class="cal-input" style="flex:1;display:none;" />
      </div>
      <div class="cal-form-row" style="align-items:center;gap:8px;">
        <label style="font-size:11px;opacity:0.5;">Cor</label>
    ...
    <div class="cal-form-actions">
      ...
      <button id="cal-f-save" class="cal-btn cal-btn-primary" style="display:inline-flex;align-items:center;gap:5px;">${isEdit
        ? '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="20 6 9 17 4 12"/></svg>Salvar'
        : '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Criar'}</button>
    </div>
  </div>`;
  ...
  document.getElementById('cal-f-save')?.addEventListener('click', async () => {
    ...
    if (!summary) { uiModule.showToast('Título obrigatório'); return; }
    ...
    } catch (e) { uiModule.showToast('Falha ao salvar'); }
  });
  document.getElementById('cal-f-del')?.addEventListener('click', async () => {
    const name = existing && existing.summary ? `"${existing.summary}"` : 'este evento';
    const ok = await uiModule.styledConfirm(`Excluir ${name}?`, { confirmText: 'Excluir', danger: true });
    if (!ok) return;
    try { await _deleteEvent(existing.uid); _render(); }
    catch (e) { uiModule.showToast('Falha ao excluir'); }
  });
  ...
  const _syncHero = () => {
    ...
    if (clockEl) clockEl.innerHTML = allday ? '<span class="cal-hero-clock-allday">Dia inteiro</span>' : _clockFace(startVal);
...
function _clockAmpm(hhmm) {
  ...
  return h < 12 ? 'AM' : 'PM'; // mantido — formato de hora segue preferência do SO
}
...
function _locHTML(loc) {
  ...
  return `<a href="${mapUrl}" target="_blank" rel="noopener" onclick="event.stopPropagation();" title="Abrir no OpenStreetMap">${_e(loc)}</a>`;
}
