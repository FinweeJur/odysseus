...
export function formatPhase(progress, maxRounds) {
  if (!progress || !progress.phase) return 'Iniciando...';
  const p = progress;
  const rn = p.round ? (maxRounds ? `Rodada ${p.round}/${maxRounds}: ` : `Rodada ${p.round}: `) : '';
  switch (p.phase) {
    case 'probing': return 'Testando modelo...';
    case 'planning': return 'Planejando estratégia de pesquisa...';
    case 'searching': return `${rn}Buscando (${p.queries || 0} consultas)`;
    case 'reading': return `${rn}Lendo ${p.total_sources || 0} fontes`;
    case 'analyzing': return `${rn}Analisando ${p.total_findings || 0} achados`;
    case 'writing': return `Escrevendo relatório -- ${p.total_sources || 0} fontes`;
...
function _finishJob(job, status) {
  ...
  if (status === 'done') {
    if ('Notification' in window && Notification.permission === 'granted') {
      try { new Notification('Pesquisa concluída', { body: job.query.slice(0, 80) }); } catch {}
    }
