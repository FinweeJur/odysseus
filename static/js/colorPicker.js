...
function computeSuggestions() {
  ...
  return [
    { hex: hsvToHex(_h + 180, _s, _v),                                   label: 'Complementar' },
    { hex: hsvToHex(_h + 30, _s, _v),                                    label: 'Análoga +30°' },
    { hex: hsvToHex(_h - 30, _s, _v),                                    label: 'Análoga -30°' },
    { hex: hsvToHex(_h + 150, _s, _v),                                   label: 'Complementar dividida' },
    { hex: hsvToHex(_h, _s, clamp(_v > 50 ? _v - 30 : _v + 30, 10, 95)), label: 'Variação de tom' },
  ];
}
...
function buildPopover() {
  ...
  p.innerHTML = `
    ...
    <div class="cp-row">
      ...
      <button class="cp-eyedropper" title="Conta-gotas" type="button">
    ...
    <div class="cp-section-label">Sugestões</div>
    <div class="cp-swatches cp-suggestions"></div>
    <div class="cp-section-label">Recentes</div>
...
function syncUI() {
  ...
  rContainer.innerHTML = recs.length
    ...
    : '<div class="cp-recent-empty">(nenhuma ainda)</div>';
}
...
function wireHandlers(p) {
  ...
  if (window.EyeDropper) {
    ...
    eye.title = 'Conta-gotas não suportado neste navegador';
  }
}
