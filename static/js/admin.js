...
/* ═══════════════════════════════════════════
   ...
   ═══════════════════════════════════════════ */
const PRIV_LABELS = {
  can_use_agent: 'Modo agente',
  can_use_browser: 'Automação de navegador',
  can_use_bash: 'Shell / Python / Arquivos',
  can_use_documents: 'Editor de documentos',
  can_use_research: 'Pesquisa profunda',
  can_generate_images: 'Geração de imagens',
  can_manage_memory: 'Memória e habilidades',
};
...
async function loadUsers() {
  ...
  try {
    ...
    if (res.status === 401 || res.status === 403) { list.innerHTML = '<div class="admin-empty">Acesso negado</div>'; return; }
    const data = await res.json();
    if (!data.users || data.users.length === 0) { list.innerHTML = '<div class="admin-empty">Nenhum usuário encontrado</div>'; return; }
    list.innerHTML = '';
    data.users.forEach(u => {
      ...
      header.innerHTML = `
        <div class="admin-user-info">
          ...
          <div>
            ...
            ${u.is_admin ? '<span class="admin-badge" style="margin-left:6px;">ADMIN</span>' : '<span style="font-size:10px;opacity:0.4;display:block;">Clique para gerenciar privilégios</span>'}
          </div>
        </div>
        <div style="display:flex;gap:8px;align-items:center;">
          <button class="admin-btn-sm" data-adm-rename-user="${esc(u.username)}" style="font-size:11px;">Renomear</button>
          ${u.is_admin ? '' : `<button class="admin-btn-delete" data-adm-del-user="${esc(u.username)}" style="font-size:11px;">Remover</button>`}
      ...
      if (!u.is_admin) {
        ...
        // Boolean toggles
        let html = '<div style="font-size:10px;text-transform:uppercase;letter-spacing:0.5px;opacity:0.35;font-weight:600;margin-bottom:4px;">Recursos</div>';
        ...
        // Rate limit
        html += '<div style="font-size:10px;text-transform:uppercase;letter-spacing:0.5px;opacity:0.35;font-weight:600;margin:10px 0 4px;">Limites</div>';
        const maxMsg = (u.privileges && u.privileges.max_messages_per_day) || 0;
        html += `<div style="display:flex;align-items:center;justify-content:space-between;padding:4px 0;">
          <div>
            <span style="font-size:12px;">Limite diário de mensagens</span>
            <div style="font-size:10px;opacity:0.4;">0 = sem limite</div>
          </div>
        ...
        html += `<div style="padding:4px 0;">
          <div style="display:flex;align-items:center;justify-content:space-between;">
            <span style="font-size:12px;">Modelos permitidos</span>
            <div style="display:flex;gap:8px;">
              <a href="#" class="priv-models-all" data-user="${esc(u.username)}" style="font-size:10px;opacity:0.5;">Todos</a>
              <a href="#" class="priv-models-none" data-user="${esc(u.username)}" style="font-size:10px;opacity:0.5;">Nenhum</a>
            </div>
          </div>
          <div style="font-size:10px;opacity:0.4;margin-bottom:4px;">${allEmpty ? 'Todos os modelos permitidos (sem restrições)' : allowedSet.size + ' modelo(s) permitido(s)'}</div>
          <div class="priv-models-list" data-user="${esc(u.username)}">
            <span style="opacity:0.4;font-size:11px;">Carregando modelos…</span>
          </div>
        </div>`;
        ...
        privPanel.querySelectorAll('[data-priv]').forEach(input => {
          const handler = async () => {
            ...
            } catch (e) { uiModule.showError('Falha ao atualizar privilégio'); }
          };
      ...
      if (renameBtn) {
        renameBtn.addEventListener('click', async (e) => {
          ...
          const next = await uiModule.styledPrompt(`Renomear "${oldUsername}"`, {
          ...
          try {
            ...
            if (!res.ok) {
              uiModule.showError(data.detail || 'Falha ao renomear usuário');
              return;
            }
            ...
            uiModule.showError('Falha ao renomear usuário');
          }
        });
      }
      ...
      if (delBtn) {
        delBtn.addEventListener('click', async (e) => {
          ...
          if (!await uiModule.styledConfirm(`Remover usuário "${username}"?`, { confirmText: 'Remover', danger: true })) return;
          ...
          if (res.ok) loadUsers();
          else uiModule.showError('Falha ao excluir usuário');
        });
      }

      list.appendChild(row);
    });
  } catch (e) { list.innerHTML = '<div class="admin-error">Falha ao carregar usuários</div>'; }
}
...
async function _loadModelsForUser(username, allowedSet, privPanel) {
  ...
  try {
    ...
    if (!allModels.length) {
      listEl.innerHTML = '<span style="opacity:0.4;font-size:11px;">Nenhum modelo disponível</span>';
      return;
    }
    const allEmpty = allowedSet.size === 0;
    ...
    function _saveModels() {
      ...
      if (hint) hint.textContent = value.length === 0 ? 'Todos os modelos permitidos (sem restrições)' : value.length + ' modelo(s) permitido(s)';
    ...
    listEl.innerHTML = '<span style="opacity:0.4;font-size:11px;">Falha ao carregar modelos</span>';
  }
}
...
function initAddUser() {
  el('adm-addBtn').addEventListener('click', async () => {
    ...
    if (!username) { msg.textContent = 'Nome de usuário obrigatório'; msg.className = 'admin-error'; return; }
    if (password.length < 8) { msg.textContent = 'A senha deve ter no mínimo 8 caracteres'; msg.className = 'admin-error'; return; }
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) { msg.textContent = 'Usuário criado'; msg.className = 'admin-success'; el('adm-newUsername').value = ''; el('adm-newPassword').value = ''; el('adm-newIsAdmin').checked = false; loadUsers(); }
      else { msg.textContent = data.detail || 'Falha'; msg.className = 'admin-error'; }
    } catch (e) { msg.textContent = 'Falha na requisição'; msg.className = 'admin-error'; }
...
async function loadEndpoints() {
  ...
  try {
    ...
    if (!Array.isArray(data) || data.length === 0) {
      const empty = '<div class="admin-empty">Nenhum</div>';
      if (listLocal) listLocal.innerHTML = empty;
      if (listApi) listApi.innerHTML = '<div class="admin-empty">Nenhum</div>';
    ...
    const rowHtml = data.map(ep => {
      ...
      const statusBadge = ep.status === 'empty'
        ? '<span class="admin-badge">sem modelos</span>'
        : ep.online
          ? `<span class="admin-badge">${visibleCount}/${totalCount} modelos ativados</span>`
        ...
        <div class="admin-user-row${ep.is_enabled ? '' : ' admin-ep-disabled'}${justAddedClass}" data-adm-ep-id="${ep.id}">
          <div style="display:flex;align-items:center;justify-content:space-between;${hasModels ? 'cursor:pointer;' : ''}padding:4px 0;" data-adm-ep-header="${ep.id}">
            <div class="admin-user-info" style="flex:1;flex-wrap:wrap;gap:0.3rem;">
              ...
              ${ep.model_type === 'image' ? '<span class="admin-badge" style="background:color-mix(in srgb, var(--accent) 20%, transparent);color:var(--accent);">Imagem</span>' : ''}
              ${kindLabel ? `<span class="admin-badge">${esc(kindLabel)}</span>` : ''}
              ${statusBadge}
              ${ep.is_enabled ? '' : '<span class="admin-badge admin-badge-off">desativado</span>'}
              ${hasModels ? '<span style="font-size:10px;opacity:0.4;">Clique para gerenciar modelos</span>' : ''}
            </div>
            <div style="display:flex;gap:4px;align-items:center;">
              <button class="admin-btn-sm" data-adm-toggle-ep="${ep.id}">${ep.is_enabled ? 'Desativar' : 'Ativar'}</button>
              <button class="admin-btn-delete" data-adm-del-ep="${ep.id}" data-adm-ep-online="${ep.online ? '1' : '0'}">Excluir</button>
              ${hasModels ? '<svg class="admin-user-chevron" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="opacity:0.3;transition:transform 0.2s,opacity 0.2s;"><polyline points="6 9 12 15 18 9"/></svg>' : ''}
            </div>
          </div>
          <div class="admin-ep-detail">${esc(ep.base_url)}${category === 'local' ? `<button type="button" class="admin-ep-copy-btn" data-adm-copy-url="${esc(ep.base_url)}" title="Copiar URL" aria-label="Copiar URL" style="background:none;border:none;padding:0 2px;margin-left:6px;cursor:pointer;color:inherit;opacity:0.45;vertical-align:-2px;line-height:1;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg></button>` : ''}${ep.has_key ? ' (chave configurada)' : ''}</div>
    ...
    queryAll('[data-adm-del-ep]').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        if (!isOffline) {
          ...
          var msg = 'Excluir este endpoint?';
          if (deps.length) {
            msg += '\n\nAs seguintes configurações usam este endpoint e serão redefinidas:\n— ' + deps.join('\n— ');
          }
          if (!await uiModule.styledConfirm(msg, { confirmText: 'Excluir', danger: true })) return;
        }
    ...
    queryAll('[data-adm-ep-id]').forEach(row => {
      ...
      row.addEventListener('click', async (e) => {
        ...
        if (!_modelsLoaded && isOpen) {
          ...
          _ld.appendChild(document.createTextNode('Carregando modelos…'));
          try {
          ...
          const renderModels = (models, warning = '') => {
            ...
            const attachRefresh = () => {
              panel.querySelector(`[data-ep-refresh-models="${epId}"]`)?.addEventListener('click', async (e) => {
                e.preventDefault();
                panel.innerHTML = _loadingHtml('Atualizando modelos…');
                try {
                  ...
                  renderModels(sortedModels, 'Falha ao atualizar modelos; modelos em cache mantidos.');
                }
              });
            };
            if (!sortedModels.length) {
              panel.innerHTML = `<div class="mcp-tools-header">
                ...
                <span style="display:flex;gap:8px;align-items:center;">
                  <span class="mcp-tools-count">0/0 ativados</span>
                  <a href="#" data-ep-refresh-models="${epId}">Atualizar</a>
                </span>
              </div>${warningHtml}<span style="opacity:0.5;font-size:11px;">Nenhum modelo</span>`;
              attachRefresh();
              return;
            }
            ...
            panel.innerHTML = `<div class="mcp-tools-header">
              <span>Modelos</span>
              <span style="display:flex;gap:8px;align-items:center;">
                <span class="mcp-tools-count">${sortedModels.length - hiddenSet.size}/${sortedModels.length} ativados</span>
                <a href="#" data-ep-refresh-models="${epId}">Atualizar</a>
                <a href="#" data-ep-select-all="${epId}">Todos</a>
                <a href="#" data-ep-select-none="${epId}">Nenhum</a>
              </span>
            </div>${warningHtml}${showSearch ? `<input type="search" class="mcp-tools-search" placeholder="Buscar ${sortedModels.length} modelos…" data-ep-search="${epId}">` : ''}<div class="mcp-tools-list">` + sortedModels.map(m =>
          ...
          try {
            ...
            _stopSpin();
            renderModels(models);
          } catch (e) { _stopSpin(); panel.innerHTML = '<span class="admin-error" style="font-size:11px;">Falha ao carregar modelos</span>'; }
        }
      });
    });
  } catch (e) {
    const err = '<div class="admin-error">Falha ao carregar</div>';
...
function initEndpointForm() {
  ...
  function _renderEndpointTestResult(msg, res, d) {
    if (res.ok && d.status === 'empty') {
      msg.textContent = 'Online — nenhum modelo encontrado';
    ...
    if (res.ok && d.online) {
      ...
      msg.innerHTML = `Online — ${models.length} modelo${models.length !== 1 ? 's' : ''} encontrado${models.length !== 1 ? 's' : ''}${preview ? `: ${preview}${models.length > 3 ? ', …' : ''}` : ''}`;
      msg.className = 'admin-success';
      return;
    }
  ...
  if (apiTestBtn) {
    apiTestBtn.addEventListener('click', async () => {
      ...
      if (!rawUrl) { msg.textContent = 'Selecione um provedor ou informe uma URL base'; msg.className = 'admin-error'; return; }
      if (provider.value && !apiKey) { msg.textContent = 'Chave de API é obrigatória para provedores em nuvem'; msg.className = 'admin-error'; return; }
      const url = provider.value && rawUrl === provider.value ? rawUrl : _normalizeBaseUrl(rawUrl);
      apiTestController = new AbortController();
      apiTestBtn.disabled = true;
      apiTestBtn.textContent = 'Testando...';
      ...
      try {
        ...
        if (e && e.name === 'AbortError') {
          msg.textContent = 'Teste cancelado';
          msg.className = '';
        } else {
          msg.textContent = 'Falha no teste: ' + (e && e.message ? e.message : 'requisição falhou');
          msg.className = 'admin-error';
        }
      }
      apiTestController = null;
      apiTestBtn.disabled = false;
      apiTestBtn.textContent = 'Testar';
  ...
  el('adm-epAddBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (!d.online) {
          msg.textContent = 'Adicionado (endpoint offline — tentará novamente na próxima carga)';
          msg.className = 'admin-error';
        } else if (d.status === 'empty') {
          msg.textContent = 'Adicionado — endpoint acessível, nenhum modelo encontrado';
          msg.className = 'admin-success';
        } else {
          msg.textContent = `Adicionado — ${count} modelo${count !== 1 ? 's' : ''} encontrado${count !== 1 ? 's' : ''}`;
          msg.className = 'admin-success';
        }
      } else { msg.textContent = d.detail || 'Falhou'; msg.className = 'admin-error'; }
    } catch (e) { msg.textContent = 'Falha na requisição'; msg.className = 'admin-error'; }
  ...
  if (localTestBtn) {
    localTestBtn.addEventListener('click', async () => {
      ...
      if (!raw) { msg.textContent = 'Informe uma URL base para testar'; msg.className = 'admin-error'; return; }
      const url = _normalizeBaseUrl(raw);
      const keyEl = el('adm-epLocalApiKey');
      const apiKey = keyEl ? keyEl.value.trim() : '';
      localTestBtn.disabled = true;
      localTestBtn.textContent = 'Testando...';
      try {
        const fd = new FormData();
        fd.append('base_url', url);
        if (apiKey) fd.append('api_key', apiKey);
      ...
      localTestBtn.textContent = 'Testar';
    });
  }
  if (localAddBtn) {
    localAddBtn.addEventListener('click', async () => {
      ...
      msg.textContent = ''; msg.className = '';
      const raw = (el('adm-epLocalUrl').value || '').trim();
      if (!raw) { msg.textContent = 'Informe uma URL base (ex.: http://localhost:8002/v1)'; msg.className = 'admin-error'; return; }
      const url = _normalizeBaseUrl(raw);
      const keyEl = el('adm-epLocalApiKey');
      const apiKey = keyEl ? keyEl.value.trim() : '';
      localAddBtn.disabled = true; localAddBtn.textContent = 'Adicionando...';
      try {
        const fd = new FormData();
        fd.append('base_url', url);
        if (apiKey) fd.append('api_key', apiKey);
        ...
        if (res.ok) {
          ...
          msg.textContent = d.status === 'empty'
            ? 'Adicionado — Ollama está rodando, nenhum modelo baixado ainda'
            : d.online
            ? `Adicionado — ${count} modelo${count !== 1 ? 's' : ''} encontrado${count !== 1 ? 's' : ''}`
            : 'Adicionado (offline — tentará novamente na próxima carga)';
          msg.className = d.online ? 'admin-success' : 'admin-error';
        } else { msg.textContent = d.detail || 'Falhou'; msg.className = 'admin-error'; }
      } catch (e) { msg.textContent = 'Falha na requisição'; msg.className = 'admin-error'; }
      localAddBtn.disabled = false; localAddBtn.textContent = 'Adicionar';
    });
  }
  ...
  if (ollamaBtn) {
    ollamaBtn.addEventListener('click', async () => {
      ...
      const msg = _endpointMsg('local');
      if (msg) {
        msg.innerHTML = '<span style="font-size:11px;opacity:0.55;">Ollama pronto para testar.</span>';
        msg.className = '';
      }
    });
  }
  ...
  if (discoverBtn) {
    discoverBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        txt.textContent = 'Escaneando portas 8000-8020 e 11434 por servidores de modelo...';
        txt.style.cssText = 'font-size:12px;opacity:0.7;';
        wrap.appendChild(txt);
        msg.appendChild(wrap);
        discoverBtn._wp = wp;
      } catch(e) { msg.textContent = 'Escaneando...'; }
      try {
        const res = await fetch('/api/discover');
        const data = await res.json();
        const items = data.items || [];
        if (!items.length) {
          msg.textContent = 'Nenhum servidor de modelo encontrado. Verifique se vLLM, llama.cpp, SGLang ou Ollama está em execução. Usuários de Docker podem precisar vincular o Ollama a uma interface acessível e confiável.';
          msg.className = 'admin-error';
        } else {
          ...
          const parts = [`${items.length} servidor${items.length !== 1 ? 'es' : ''} encontrado${items.length !== 1 ? 's' : ''} com ${totalModels} modelo${totalModels !== 1 ? 's' : ''}`];
          if (added) parts.push(`${added} novo${added !== 1 ? 's' : ''} adicionado${added !== 1 ? 's' : ''}`);
          if (skipped) parts.push(`${skipped} já adicionado${skipped !== 1 ? 's' : ''}`);
          msg.innerHTML = parts.join(' — ');
          msg.className = 'admin-success';
          loadEndpoints();
        }
      } catch (e) {
        msg.textContent = 'Falha no escaneamento: ' + e.message;
        msg.className = 'admin-error';
      }
      if (discoverBtn._wp) { discoverBtn._wp.destroy(); discoverBtn._wp = null; }
      discoverBtn.disabled = false;
      discoverBtn.innerHTML = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" style="vertical-align:-1px;margin-right:4px;"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>Buscar Servidores';
    });
  }
...
const _GOOGLE_OAUTH_HELP = `Para obter credenciais do Google OAuth:
1. Vá para console.cloud.google.com
2. Clique no menu de projetos (canto superior esquerdo) > Novo projeto > nomeie-o > Criar
3. APIs e Serviços > Biblioteca > ative a API necessária (Gmail, Calendário, Drive, etc.)
4. APIs e Serviços > Tela de consentimento OAuth > configure (Externo, nome do app + email)
5. Em Audiência, clique Adicionar usuários > adicione seu email Google como usuário de teste
6. APIs e Serviços > Credenciais > + Criar credenciais > ID do cliente OAuth > Aplicativo para desktop
7. Copie o ID do cliente e o Segredo do cliente nos campos acima
8. Após adicionar o servidor, clique em Autorizar para entrar com o Google
9. Se acessar remotamente: entre, copie a URL da página de erro e cole-a de volta`;
...
const MCP_PRESETS = [
  { name: "Gmail",           command: "npx", args: ["-y", "@gongrzhe/server-gmail-autoauth-mcp"],      env: { GOOGLE_CLIENT_ID: "", GOOGLE_CLIENT_SECRET: "" },
    ...
    help: `Configuração:
1. Vá para console.cloud.google.com > crie ou selecione um projeto
2. APIs e Serviços > Biblioteca > pesquise "Gmail API" > Ativar
3. APIs e Serviços > Tela de consentimento OAuth > configure (Externo é aceitável)
4. Em Audiência, adicione seu endereço Gmail como usuário de teste
5. APIs e Serviços > Credenciais > + Criar credenciais > ID do cliente OAuth
6. Tipo de aplicação: Aplicativo para desktop > Criar
7. Copie o ID do cliente e o Segredo do cliente nos campos acima
8. Clique em Adicionar servidor, depois clique no botão Autorizar
9. Entre com o Google, copie a URL da página de erro, cole-a de volta` },
  { name: "Email (IMAP/SMTP)", command: "npx", args: ["-y", "@codefuturist/email-mcp", "stdio"],        env: { MCP_EMAIL_ADDRESS: "", MCP_EMAIL_PASSWORD: "", MCP_EMAIL_IMAP_HOST: "", MCP_EMAIL_SMTP_HOST: "" },
    providerDropdown: {
      ...
      options: [
        ...
        { name: "Personalizado", imap: "",                    smtp: "" },
      ],
    },
    help: "Funciona com qualquer provedor de email IMAP/SMTP.\n1. Escolha seu provedor na lista suspensa (ou selecione Personalizado)\n2. Informe seu endereço de email e senha (ou senha de app)\n3. Clique em Adicionar servidor" },
  { name: "CalDAV (Radicale/Nextcloud)", command: "npx", args: ["-y", "caldav-mcp"],                     env: { CALDAV_BASE_URL: "http://localhost:5232", CALDAV_USERNAME: "", CALDAV_PASSWORD: "" },
    help: "Funciona com qualquer servidor CalDAV (Radicale, Nextcloud, etc.).\n1. Informe a URL do seu servidor CalDAV (ex. http://localhost:5232)\n2. Informe seu nome de usuário e senha\n3. Clique em Adicionar servidor" },
  { name: "Google Calendar", command: "npx", args: ["-y", "@cocal/google-calendar-mcp"],                 env: { GOOGLE_OAUTH_CREDENTIALS: "" },
    help: `Configuração:
1. Vá para console.cloud.google.com > crie/selecione um projeto
2. APIs e Serviços > Biblioteca > ative a API do Google Calendar
3. APIs e Serviços > Credenciais > + Criar credenciais > ID do cliente OAuth
4. Tipo de aplicação: Aplicativo para desktop > Criar
5. Clique em "Baixar JSON" na credencial que você acabou de criar
6. Defina Credenciais do Google OAuth como o caminho completo do arquivo JSON baixado` },
  { name: "Google Drive",    command: "npx", args: ["-y", "@modelcontextprotocol/server-gdrive"],        env: {},
    help: "Google Drive usa OAuth baseado em navegador na primeira execução. Nenhuma variável de ambiente necessária — apenas clique em Adicionar e autorize quando solicitado." },
  { name: "GitHub",          command: "npx", args: ["-y", "@modelcontextprotocol/server-github"],        env: { GITHUB_PERSONAL_ACCESS_TOKEN: "" },
    help: "1. Vá para github.com > Configurações > Configurações do desenvolvedor > Tokens de acesso pessoal > Tokens refinados\n2. Gere um novo token com as permissões de repositório necessárias\n3. Cole-o como Token de acesso pessoal do Github" },
  { name: "Slack",           command: "npx", args: ["-y", "@modelcontextprotocol/server-slack"],         env: { SLACK_BOT_TOKEN: "", SLACK_TEAM_ID: "" },
    help: "1. Vá para api.slack.com/apps > Criar novo app > Do zero\n2. Adicione escopos do token do bot (channels:read, chat:write, etc.)\n3. Instale no espaço de trabalho, copie o token OAuth do usuário bot (xoxb-...)\n4. O ID do time está na URL do seu espaço de trabalho ou nas configurações de administrador do Slack" },
  { name: "Notion",          command: "npx", args: ["-y", "@notionhq/notion-mcp-server"],               env: { OPENAPI_MCP_HEADERS: "" },
    help: "1. Vá para notion.so/my-integrations\n2. Crie uma nova integração\n3. Copie o segredo da integração interna\n4. Compartilhe as páginas/bases de dados do Notion que você quer acessíveis com a integração\n5. Para os cabeçalhos do MCP do Openapi informe:\n   {\"Authorization\": \"Bearer YOUR_SECRET\", \"Notion-Version\": \"2022-06-28\"}" },
  { name: "Linear",          command: "npx", args: ["-y", "mcp-linear"],                                env: { LINEAR_API_KEY: "" },
    help: "1. Vá para linear.app > Configurações > API\n2. Crie uma chave de API pessoal\n3. Cole-a como chave de API do Linear" },
  { name: "Brave Search",    command: "npx", args: ["-y", "@modelcontextprotocol/server-brave-search"], env: { BRAVE_API_KEY: "" },
    help: "1. Vá para brave.com/search/api\n2. Inscreva-se em um plano gratuito (2000 consultas/mês)\n3. Copie sua chave de API" },
  { name: "Browser (Playwright)", command: "npx", args: ["-y", "@playwright/mcp@latest", "--headless"],  env: {},
    help: "Automação de navegador via Playwright. O AI pode navegar em páginas, clicar, preencher formulários e ler conteúdo.\nExecuta sem interface gráfica por padrão. Remova --headless de Args para ver a janela do navegador.\nA primeira execução instala o Chromium automaticamente." },
  { name: "Filesystem",      command: "npx", args: ["-y", "@modelcontextprotocol/server-filesystem", "/home"], env: {},
    help: "Edite o campo Args para alterar o diretório ao qual o servidor tem acesso." },
  ...
  { name: "Postgres",        command: "npx", args: ["-y", "@modelcontextprotocol/server-postgres", "postgresql://user:pass@localhost/db"], env: {},
    help: "Substitua a string de conexão no campo Args pela sua URL de conexão real do Postgres." },
  { name: "Todoist",         command: "npx", args: ["-y", "todoist-mcp-server"],                         env: { TODOIST_API_TOKEN: "" },
    help: "1. Vá para todoist.com > Configurações > Integrações > Desenvolvedor\n2. Copie seu token de API" },
];
// ── Built-in tools management ──
const TOOL_META = {
  bash:              { name: 'Shell',            desc: 'Executar comandos bash',           cat: 'Código',       ctx: '~200' },
  python:            { name: 'Python',           desc: 'Executar scripts Python',              cat: 'Código',       ctx: '~200' },
  read_file:         { name: 'Ler arquivo',        desc: 'Ler arquivos do disco',            cat: 'Código',       ctx: '~150' },
  write_file:        { name: 'Escrever arquivo',       desc: 'Escrever/criar arquivos',              cat: 'Código',       ctx: '~150' },
  web_search:        { name: 'Busca na web',       desc: 'Pesquisar na web via SearXNG',      cat: 'Busca',     ctx: '~300' },
  search_chats:      { name: 'Buscar conversas',     desc: 'Pesquisar histórico de conversas',     cat: 'Busca',     ctx: '~150' },
  create_document:   { name: 'Criar documento',  desc: 'Criar novos documentos',            cat: 'Documentos',  ctx: '~200' },
  update_document:   { name: 'Atualizar documento',  desc: 'Modificar documentos existentes',       cat: 'Documentos',  ctx: '~200' },
  edit_document:     { name: 'Editar documento',    desc: 'Localizar e substituir em documentos',     cat: 'Documentos',  ctx: '~200' },
  suggest_document:  { name: 'Sugerir alterações',  desc: 'Propor edições de documentos',          cat: 'Documentos',  ctx: '~200' },
  manage_documents:  { name: 'Gerenciar documentos', desc: 'Listar, excluir, organizar docs',     cat: 'Documentos',  ctx: '~150' },
  generate_image:    { name: 'Gerar imagem',   desc: 'Criar imagens via IA',            cat: 'Mídia',      ctx: '~150' },
  manage_memory:     { name: 'Memória',           desc: 'Salvar e recuperar memórias',        cat: 'Conhecimento',  ctx: '~200' },
  manage_skills:     { name: 'Habilidades',           desc: 'Aprender e usar procedimentos',        cat: 'Conhecimento',  ctx: '~200' },
  manage_rag:        { name: 'RAG / Docs',       desc: 'Consultar documentos indexados',         cat: 'Conhecimento',  ctx: '~150' },
  chat_with_model:   { name: 'Chat com modelo',  desc: 'Conversar com outro modelo IA',        cat: 'Multi-Agente', ctx: '~200' },
  second_opinion:    { name: 'Segunda opinião',   desc: 'Obter a perspectiva de outro modelo',       cat: 'Multi-Agente', ctx: '~150' },
  pipeline:          { name: 'Pipeline',         desc: 'Fluxos de trabalho IA com várias etapas',         cat: 'Multi-Agente', ctx: '~200' },
  ask_teacher:       { name: 'Perguntar ao professor',      desc: 'Consultar um modelo mais capaz',      cat: 'Multi-Agente', ctx: '~150' },
  send_to_session:   { name: 'Enviar para sessão',  desc: 'Enviar mensagem para outro chat',    cat: 'Sessões',   ctx: '~100' },
  create_session:    { name: 'Criar sessão',   desc: 'Iniciar uma nova sessão de chat',        cat: 'Sessões',   ctx: '~100' },
  list_sessions:     { name: 'Listar sessões',    desc: 'Procurar sessões existentes',        cat: 'Sessões',   ctx: '~100' },
  manage_session:    { name: 'Gerenciar sessão',   desc: 'Renomear, arquivar, configurar',      cat: 'Sessões',   ctx: '~100' },
  list_models:       { name: 'Listar modelos',      desc: 'Mostrar modelos disponíveis',           cat: 'Sistema',     ctx: '~100' },
  ui_control:        { name: 'Controle de UI',       desc: 'Alterar tema, layout, configurações',  cat: 'Sistema',     ctx: '~150' },
  manage_tasks:      { name: 'Tarefas',            desc: 'Agendar tarefas automatizadas',        cat: 'Sistema',     ctx: '~150' },
  api_call:          { name: 'Chamada de API',         desc: 'Fazer requisições HTTP',              cat: 'Sistema',     ctx: '~200' },
  manage_endpoints:  { name: 'Endpoints',        desc: 'Adicionar/remover endpoints de modelo',      cat: 'Sistema',     ctx: '~100' },
  manage_mcp:        { name: 'Servidores MCP',      desc: 'Gerenciar conexões MCP',          cat: 'Sistema',     ctx: '~100' },
  manage_webhooks:   { name: 'Webhooks',         desc: 'Configurar eventos de webhook',        cat: 'Sistema',     ctx: '~100' },
  manage_tokens:     { name: 'Tokens de API',       desc: 'Gerenciar tokens de acesso à API',        cat: 'Sistema',     ctx: '~100' },
  manage_settings:   { name: 'Configurações',         desc: 'Alterar configurações do app',             cat: 'Sistema',     ctx: '~100' },
};
...
async function loadBuiltinTools() {
  ...
  try {
    ...
    if (!tools.length) { list.innerHTML = '<div class="admin-empty">Nenhuma ferramenta encontrada</div>'; return; }
    ...
    for (const cat of catOrder) {
      ...
      html += `<div class="admin-tool-category">
        <div class="admin-tool-cat-header" data-tool-cat="${catId}" style="cursor:pointer;display:flex;align-items:center;justify-content:space-between;">
          ...
          <span style="display:flex;align-items:center;gap:6px;" class="admin-tool-cat-right">
            <span class="admin-tool-cat-count" style="font-size:10px;opacity:0.5;">${enabledCount}/${totalCount} ativadas</span>
      ...
      for (const t of items) {
        ...
        <div class="admin-tool-row">
          ...
          <span class="admin-tool-ctx" title="Tokens de contexto aproximados">${esc(t.ctx)}</span>
          <label class="admin-switch" style="flex-shrink:0;">
    ...
    list.innerHTML = '<div class="admin-empty">Falha ao carregar ferramentas</div>';
  }
}
...
async function loadMcpServers() {
  ...
  try {
    ...
    if (!servers.length) { list.innerHTML = '<div class="admin-empty">Nenhum servidor MCP configurado</div>'; return; }
    list.innerHTML = servers.map(s => {
      ...
      const toolInfo = s.status === 'connected' ? `${s.enabled_tool_count}/${s.tool_count} ferramentas ativadas` : '';
      const statusText = s.needs_oauth ? 'Requer autorização' : s.status === 'connected' ? `Conectado (${toolInfo})` : s.status === 'error' ? `Erro: ${s.error || 'desconhecido'}` : 'Desconectado';
      ...
      return `<div class="admin-user-row" data-adm-mcp-id="${s.id}">
        <div style="display:flex;align-items:center;justify-content:space-between;${hasTools ? 'cursor:pointer;' : ''}padding:4px 0;" data-adm-mcp-header="${s.id}">
          <div class="admin-user-info" style="flex:1;flex-wrap:wrap;gap:0.3rem;">
            ...
            ${hasTools ? `<span style="font-size:10px;opacity:0.4;">Clique para gerenciar ferramentas</span>` : ''}
          </div>
          <div style="display:flex;gap:4px;align-items:center;">
            ${s.needs_oauth ? `<a href="/api/mcp/oauth/authorize/${s.id}" target="_blank" class="admin-btn-sm" style="background:var(--red);color:#fff;text-decoration:none;padding:3px 10px;border-radius:4px;font-size:11px;font-weight:600;">Autorizar</a>` : ''}
            <button class="admin-btn-sm" data-adm-mcp-reconnect="${s.id}">Reconectar</button>
            <button class="admin-btn-delete" style="border-color:${s.is_enabled ? 'color-mix(in srgb, var(--red) 30%, transparent)' : 'color-mix(in srgb, var(--fg) 30%, transparent)'};color:${s.is_enabled ? 'var(--red)' : 'var(--fg)'};" data-adm-mcp-toggle="${s.id}" data-adm-mcp-enable="${!s.is_enabled}">${s.is_enabled ? 'Desativar' : 'Ativar'}</button>
            <button class="admin-btn-delete" data-adm-mcp-delete="${s.id}">Excluir</button>
    ...
    list.querySelectorAll('[data-adm-mcp-reconnect]').forEach(btn => {
      btn.addEventListener('click', async () => {
        const msg = el('adm-mcpMsg'); msg.textContent = 'Reconectando...'; msg.className = '';
        try {
          const res = await fetch(`/api/mcp/servers/${btn.dataset.admMcpReconnect}/reconnect`, { method: 'POST', credentials: 'same-origin' });
          const data = await res.json();
          msg.textContent = data.connected ? `Reconectado (${data.tool_count} ferramentas)` : `Falhou: ${data.error || 'desconhecido'}`;
          msg.className = data.connected ? 'admin-success' : 'admin-error';
          loadMcpServers();
        } catch (e) { msg.textContent = 'Falha: ' + e.message; msg.className = 'admin-error'; }
      });
    });
    ...
    list.querySelectorAll('[data-adm-mcp-delete]').forEach(btn => {
      btn.addEventListener('click', async () => {
        if (!await uiModule.styledConfirm('Excluir este servidor MCP?', { confirmText: 'Excluir', danger: true })) return;
    ...
    list.querySelectorAll('[data-adm-mcp-id]').forEach(row => {
      ...
      row.addEventListener('click', async (e) => {
        ...
        if (!_toolsLoaded && isOpen) {
          _toolsLoaded = true;
          panel.innerHTML = '<span style="opacity:0.5;font-size:11px;">Carregando ferramentas...</span>';
          try {
            const res = await fetch(`/api/mcp/servers/${sid}/tools`, { credentials: 'same-origin' });
            const tools = await res.json();
            if (!tools.length) { panel.innerHTML = '<span style="opacity:0.5;font-size:11px;">Nenhuma ferramenta</span>'; return; }
            const disabled = new Set(tools.filter(t => t.is_disabled).map(t => t.name));
            panel.innerHTML = `<div class="mcp-tools-header">
              <span>Ferramentas</span>
              <span style="display:flex;gap:8px;align-items:center;">
                <span class="mcp-tools-count">${tools.length - disabled.size}/${tools.length} ativadas</span>
                <a href="#" data-mcp-select-all="${sid}">Todas</a>
                <a href="#" data-mcp-select-none="${sid}">Nenhuma</a>
              </span>
          ...
          } catch (e) { panel.innerHTML = '<span class="admin-error" style="font-size:11px;">Falha ao carregar ferramentas</span>'; }
        }
      });
    });
  } catch (e) { if (list) list.innerHTML = '<div class="admin-error">Falha ao carregar servidores MCP</div>'; }
}
...
async function _saveMcpToolState(serverId, panel) {
  ...
  try {
    ...
    const countLabel = panel.querySelector('.mcp-tools-count');
    if (countLabel) countLabel.textContent = `${total - disabled.length}/${total} ativadas`;
    // Update badge in the server row
    const row = panel.closest('[data-adm-mcp-id]');
    if (row) {
      const badge = row.querySelector('.admin-badge');
      if (badge) badge.textContent = `Conectado (${total - disabled.length}/${total} ferramentas ativadas)`;
    }
  } catch (e) { /* silent */ }
}
...
function initMcpForm() {
  ...
  el('adm-mcpAddBtn').addEventListener('click', async () => {
    ...
    if (!name) { msg.textContent = 'Nome é obrigatório'; msg.className = 'admin-error'; return; }
    if (transport === 'stdio' && !command) { msg.textContent = 'Comando é obrigatório para stdio'; msg.className = 'admin-error'; return; }
    if (transport === 'sse' && !url) { msg.textContent = 'URL é obrigatória para SSE'; msg.className = 'admin-error'; return; }
    try { JSON.parse(env); } catch { msg.textContent = 'Env deve ser JSON válido'; msg.className = 'admin-error'; return; }
    const fd = new FormData();
    ...
    msg.textContent = 'Adicionando...'; msg.className = '';
    try {
      const res = await fetch('/api/mcp/servers', { method: 'POST', body: fd, credentials: 'same-origin' });
      const data = await res.json();
      if (data.needs_oauth) {
        msg.innerHTML = `Adicionado ${esc(name)} — <a href="/api/mcp/oauth/authorize/${data.id}" target="_blank" style="color:var(--red);font-weight:600;">Autorizar com Google</a> para conectar`;
        msg.className = 'admin-success';
      } else if (data.connected) {
        msg.textContent = `Adicionado ${name} (${data.tool_count} ferramentas descobertas)`; msg.className = 'admin-success';
      } else { msg.textContent = `Adicionado mas conexão falhou: ${data.error || 'desconhecido'}`; msg.className = 'admin-error'; }
...
async function loadRag() {
  try {
    ...
    if (dirs.length === 0) { dirList.innerHTML = '<div class="admin-empty">Nenhum diretório indexado</div>'; }
    else {
      dirList.innerHTML = dirs.map(d => `<div class="admin-rag-item"><span class="admin-rag-item-name" title="${esc(d)}">${esc(d)}</span><button class="admin-btn-delete" data-adm-rag-dir="${esc(d)}">Remover</button></div>`).join('');
      dirList.querySelectorAll('[data-adm-rag-dir]').forEach(btn => {
        btn.addEventListener('click', async () => {
          if (!await uiModule.styledConfirm(`Remover diretório "${btn.dataset.admRagDir}" do RAG?`, { confirmText: 'Remover', danger: true })) return;
          ...
          try {
            ...
            if (res.ok) { ragMsg('Diretório removido'); loadRag(); }
            else { const e = await res.json(); ragMsg(e.detail || 'Falha', true); }
          } catch (e) { ragMsg('Erro: ' + e.message, true); }
        });
      });
    }
    const fileList = el('adm-ragFileList');
    const files = data.files || [];
    if (files.length === 0) { fileList.innerHTML = '<div class="admin-empty">Nenhum arquivo indexado</div>'; }
    else {
      fileList.innerHTML = files.map(f => {
        const size = f.size ? (f.size > 1024 ? (f.size / 1024).toFixed(1) + ' KB' : f.size + ' B') : '';
        return `<div class="admin-rag-item"><span class="admin-rag-item-name" title="${esc(f.path || f.name)}">${esc(f.name)}</span><span class="admin-rag-item-meta">${size}</span><button class="admin-btn-delete" data-adm-rag-file="${esc(f.path || f.name)}">Excluir</button></div>`;
      }).join('');
      fileList.querySelectorAll('[data-adm-rag-file]').forEach(btn => {
        btn.addEventListener('click', async () => {
          if (!await uiModule.styledConfirm(`Excluir "${btn.dataset.admRagFile}" do RAG?`, { confirmText: 'Excluir', danger: true })) return;
          btn.disabled = true; btn.textContent = '...';
          try {
            const res = await fetch('/api/personal/file?filepath=' + encodeURIComponent(btn.dataset.admRagFile), { method: 'DELETE' });
            if (res.ok) { ragMsg('Arquivo removido'); loadRag(); }
            else { const e = await res.json(); ragMsg(e.detail || 'Falha', true); }
          } catch (e) { ragMsg('Erro: ' + e.message, true); }
        });
      });
    ...
    el('adm-ragDirList').innerHTML = '<div class="admin-error">Falha ao carregar</div>';
...
async function ragUpload(files) {
  ...
  ragMsg('Enviando ' + files.length + ' arquivo(s)...', false, true);
  const fd = new FormData();
  for (const f of files) fd.append('files', f);
  try {
    const res = await fetch('/api/personal/upload', { method: 'POST', body: fd });
    const data = await res.json();
    if (data.success) { ragMsg(`Enviados ${data.uploaded.length} arquivo(s), ${data.indexed_count} trechos indexados`); loadRag(); }
    else ragMsg(data.detail || 'Falha ao enviar', true);
  } catch (e) { ragMsg('Erro ao enviar: ' + e.message, true); }
}
...
function initRag() {
  ...
  el('adm-ragAddDirBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) { ragMsg(`Indexados ${data.indexed_count} trechos do diretório`); el('adm-ragDirInput').value = ''; loadRag(); }
      else ragMsg(data.detail || data.message || 'Falha', true);
    } catch (e) { ragMsg('Erro: ' + e.message, true); }
    btn.disabled = false; btn.textContent = 'Adicionar diretório';
  });
  el('adm-ragReloadBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      const data = await res.json();
      ragMsg(`Índice recarregado: ${data.count} documentos`);
      loadRag();
    } catch (e) { ragMsg('Falha ao recarregar: ' + e.message, true); }
    btn.disabled = false; btn.textContent = 'Recarregar índice';
  });
}
...
async function loadTokens() {
  ...
  try {
    ...
    if (!tokens.length) { list.innerHTML = '<div class="admin-empty">Nenhum token de API</div>'; return; }
    list.innerHTML = tokens.map(t => `
      <div class="admin-user-row">
        <div class="admin-user-info" style="flex:1;flex-wrap:wrap;gap:0.3rem;">
          ...
          ${t.owner ? `<span style="font-size:0.75rem;opacity:0.5;">Proprietário: ${esc(t.owner)}</span>` : ''}
          ${t.last_used_at ? `<span style="font-size:0.75rem;opacity:0.5;">Último uso: ${new Date(t.last_used_at).toLocaleDateString()}</span>` : '<span style="font-size:0.75rem;opacity:0.4;">Nunca usado</span>'}
        </div>
        <button class="admin-btn-delete" data-adm-del-token="${t.id}">Revogar</button>
      </div>`).join('');
    list.querySelectorAll('[data-adm-del-token]').forEach(btn => {
      btn.addEventListener('click', async () => {
        if (!await uiModule.styledConfirm('Revogar este token de API? Integrações externas usando-o deixarão de funcionar.', { confirmText: 'Revogar', danger: true })) return;
        await fetch(`/api/tokens/${btn.dataset.admDelToken}`, { method: 'DELETE', credentials: 'same-origin' });
        loadTokens();
      });
    });
  } catch (e) { list.innerHTML = '<div class="admin-error">Falha ao carregar tokens</div>'; }
}
...
function initTokenForm() {
  el('adm-tokenAddBtn').addEventListener('click', async () => {
    ...
    if (!name) { msg.textContent = 'Nome do token é obrigatório'; msg.className = 'admin-error'; return; }
  ...
  el('adm-tokenCopyBtn').addEventListener('click', () => {
    ...
    navigator.clipboard.writeText(val).then(() => {
      el('adm-tokenCopyBtn').textContent = 'Copiado!';
      setTimeout(() => { el('adm-tokenCopyBtn').textContent = 'Copiar'; }, 2000);
    });
  });
}
...
async function loadWebhooks() {
  ...
  try {
    ...
    if (!hooks.length) { list.innerHTML = '<div class="admin-empty">Nenhum webhook configurado</div>'; return; }
    list.innerHTML = hooks.map(w => {
      ...
      const statusBadge = w.last_status_code
        ...
        : '';
      const lastTriggered = w.last_triggered_at ? new Date(w.last_triggered_at).toLocaleString() : 'Nunca';
      const errorText = w.last_error ? `<div style="font-size:0.75rem;color:var(--red);margin-top:0.2rem;">Erro: ${esc(w.last_error.substring(0, 80))}</div>` : '';
      return `
        <div class="admin-ep-item" style="flex-wrap:wrap;">
          <div class="admin-ep-info" style="flex:1;min-width:200px;">
            <div class="admin-ep-name">${esc(w.name)} ${w.is_active ? '' : '<span class="admin-badge admin-badge-off">desativado</span>'} ${w.has_secret ? '<span class="admin-badge">assinado</span>' : ''}</div>
            <div class="admin-ep-detail">${esc(w.url)}</div>
            <div style="margin-top:0.3rem;">${events}</div>
            <div class="admin-ep-detail">Último: ${lastTriggered} ${statusBadge}</div>
            ${errorText}
          </div>
          <div class="admin-ep-actions">
            <button class="admin-btn-sm" data-adm-wh-test="${w.id}">Testar</button>
            <button class="admin-btn-sm" data-adm-wh-toggle="${w.id}">${w.is_active ? 'Desativar' : 'Ativar'}</button>
            <button class="admin-btn-delete" data-adm-wh-delete="${w.id}">Excluir</button>
          </div>
        </div>`;
    }).join('');
    list.querySelectorAll('[data-adm-wh-test]').forEach(btn => {
      btn.addEventListener('click', async () => {
        const msg = el('adm-whMsg'); msg.textContent = 'Enviando teste...'; msg.className = '';
        try {
          const res = await fetch(`/api/webhooks/${btn.dataset.admWhTest}/test`, { method: 'POST', credentials: 'same-origin' });
          msg.textContent = res.ok ? 'Teste enviado!' : 'Teste falhou'; msg.className = res.ok ? 'admin-success' : 'admin-error';
    ...
    list.querySelectorAll('[data-adm-wh-delete]').forEach(btn => {
      btn.addEventListener('click', async () => {
        if (!await uiModule.styledConfirm('Excluir este webhook?', { confirmText: 'Excluir', danger: true })) return;
        await fetch(`/api/webhooks/${btn.dataset.admWhDelete}`, { method: 'DELETE', credentials: 'same-origin' }); loadWebhooks();
      });
    });
  } catch (e) { list.innerHTML = '<div class="admin-error">Falha ao carregar webhooks</div>'; }
}
...
function initWebhookForm() {
  el('adm-whAddBtn').addEventListener('click', async () => {
    ...
    if (!name) { msg.textContent = 'Nome é obrigatório'; msg.className = 'admin-error'; return; }
    if (!url) { msg.textContent = 'URL é obrigatória'; msg.className = 'admin-error'; return; }
    if (!events) { msg.textContent = 'Selecione pelo menos um evento'; msg.className = 'admin-error'; return; }
    const fd = new FormData();
    ...
    try {
      ...
      if (res.ok) { msg.textContent = 'Webhook adicionado'; msg.className = 'admin-success'; el('adm-whName').value = ''; el('adm-whUrl').value = ''; el('adm-whSecret').value = ''; loadWebhooks(); }
      else { const d = await res.json(); msg.textContent = d.detail || 'Falha'; msg.className = 'admin-error'; }
    } catch (e) { msg.textContent = 'Falha: ' + e.message; msg.className = 'admin-error'; }
  });
}
...
/* ── Features ── */
const featureLabels = {
  web_search: 'Busca na web', deep_research: 'Pesquisa profunda',
  memory: 'Memória', document_editor: 'Editor de documentos', rag: 'Base de conhecimento RAG', sensitive_filter: 'Filtro de informações sensíveis',
  gallery: 'Galeria'
};
...
function initCalDAV() {
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      const d = await res.json();
      status.textContent = d.ok ? 'Salvo' : 'Erro';
      status.style.color = d.ok ? 'var(--green)' : 'var(--red)';
    } catch (e) { status.textContent = 'Erro'; status.style.color = 'var(--red)'; }
  ...
  testBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      const d = await res.json();
      status.textContent = d.ok ? `Conectado (${d.calendars} calendários)` : `Falha: ${d.error}`;
      status.style.color = d.ok ? 'var(--green)' : 'var(--red)';
    } catch (e) { status.textContent = 'Erro'; status.style.color = 'var(--red)'; }
...
function initBackup() {
  el('adm-exportDataBtn').addEventListener('click', async () => {
    ...
    btn.disabled = true; btn.textContent = 'Exportando...'; msg.textContent = '';
    try {
      ...
      msg.textContent = 'Exportação baixada.'; msg.className = 'admin-success';
    } catch (e) { msg.textContent = 'Falha na exportação: ' + e.message; msg.className = 'admin-error'; }
    btn.disabled = false; btn.textContent = 'Exportar dados';
  });
  ...
  fileInput.addEventListener('change', async () => {
    ...
    try {
      ...
      if (res.ok && result.ok) {
        msg.textContent = result.message || 'Importação bem-sucedida.'; msg.className = 'admin-success';
      } else {
        msg.textContent = result.message || result.detail || 'Falha na importação'; msg.className = 'admin-error';
      }
    } catch (e) { msg.textContent = 'Falha na importação: ' + e.message; msg.className = 'admin-error'; }
...
function initDangerZone() {
  ...
  const _LABELS = {
    chats: 'conversas', memory: 'entradas de memória', skills: 'habilidades',
    notes: 'notas', tasks: 'tarefas', documents: 'documentos',
    gallery: 'imagens da galeria', calendar: 'eventos do calendário',
  };
  ...
  modalEl.querySelectorAll('[data-wipe-kind]').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      if (!await uiModule.styledConfirm(`Limpar TUDO ${label}? Isto não pode ser desfeito.`, { confirmText: 'Limpar', danger: true })) return;
      if (!await uiModule.styledConfirm(`Realmente limpar todos os ${label}?`, { confirmText: 'Sim, limpar tudo', danger: true })) return;
      btn.disabled = true; const prev = btn.textContent; btn.textContent = 'Limpando…';
      ...
      try {
        ...
        const data = await res.json().catch(() => ({}));
        if (res.ok) {
          if (_wipeMsg) { _wipeMsg.textContent = `Limpou ${data.count ?? 0} ${label}.`; _wipeMsg.className = 'admin-success'; }
        } else {
          if (_wipeMsg) { _wipeMsg.textContent = data.detail || 'Falha'; _wipeMsg.className = 'admin-error'; }
        }
      } catch (e) {
        if (_wipeMsg) { _wipeMsg.textContent = 'Falha na requisição: ' + e.message; _wipeMsg.className = 'admin-error'; }
      }
